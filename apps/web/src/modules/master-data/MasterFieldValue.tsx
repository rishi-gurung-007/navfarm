...
export function formatMasterValue(
  ...
  if (Array.isArray(value) && !value.length)
    return field?.multiple ? 'All stages' : '—';
  ...
  if (
    Array.isArray(value) &&
    value.every((entry) => entry?.attribute_name && 'attribute_value' in entry)
  ) {
    return value
      .map(
        (entry) => `${entry.attribute_name}: ${entry.attribute_value ?? '—'}`,
      )
      .join('\n');
  }
...
export function MasterFieldValue({
  ...
  const entity =
    field?.type === 'select-entity' && field.entityEndpoint && uuid;
  const dependency =
    typeof field?.dependsOn === 'string'
      ? String(record[field.dependsOn] || '')
      : '';
  const endpoint = field?.entityEndpoint?.replace(
    '{value}',
    encodeURIComponent(dependency),
  );
  const usesList = endpoint?.startsWith('/setup/');
  const request =
    entity && (!field?.entityEndpoint?.includes('{value}') || dependency)
      ? usesList
        ? endpoint!
        : `${endpoint!.split('?')[0]}/${encodeURIComponent(String(value))}`
      : '';
  useEffect(() => {
    if (!request) return;
    let cancelled = false;
    api
      ...
      .then((result) => {
        ...
        const row =
          usesList && Array.isArray(data)
            ? data.find(
                (entry) => String(entry[field!.entityValueKey!]) === value,
              )
            : data;
        const label =
          row && !Array.isArray(row)
            ? field?.entityLabelKeys
                ?.map((key) => row[key])
                .filter(
            ...
            : '';
        if (!cancelled)
          setResolved({ request, label: label || 'Reference unavailable' });
      })
      .catch(() => {
        if (!cancelled)
          setResolved({ request, label: 'Reference unavailable' });
      });
    return () => {
      cancelled = true;
    };
