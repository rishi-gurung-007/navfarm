...
export default function CompanyTab({
  ...
  const [ownTab] = useState<
    'profile' | 'address' | 'contact' | 'localization' | 'fiscal' | 'modules'
  >('profile');
  ...
  const targetCompany = isTenantAdmin
    ? selectedCompanyDetails || activeCompany
    : activeCompany;
  ...
  const fetchSetupDetails = async (companyId: string) => {
    ...
    try {
      const details = await api.get(
        `/setup/wizard/company-details/${companyId}`,
      );
      setSetupDetails(details);
    } catch {
      setSetupLoadWarning(
        'Some setup details are temporarily unavailable. Available information is still shown.',
      );
    } finally {
  ...
  useEffect(() => {
    ...
    if (comp) {
      ...
      setAddressForm({
        ...
        country_id:
          setupDetails?.address?.country_id || comp.country_id || 'IND',
      ...
      setContactForm({
        ...
        phone_primary:
          setupDetails?.contact?.phone_primary || comp.phone_primary || '',
      ...
      setFiscalForm({
        fiscal_start_month:
          setupDetails?.fiscal?.fiscal_start_month ||
          comp.financial_year_start ||
          4,
        fiscal_start_day: setupDetails?.fiscal?.fiscal_start_day || 1,
        fiscal_end_day: setupDetails?.fiscal?.fiscal_end_day || 31,
        current_fiscal_year:
          setupDetails?.fiscal?.current_fiscal_year || '2026-27',
        period_type: setupDetails?.fiscal?.period_type || 'MONTHLY',
        accounting_standard:
          setupDetails?.fiscal?.accounting_standard || 'Local GAAP',
        ...
        gst_filing_frequency:
          setupDetails?.fiscal?.gst_filing_frequency || 'MONTHLY',
        tax_audit_applicable:
          setupDetails?.fiscal?.tax_audit_applicable || false,
  ...
  const handleNobToggle = (nobCode: string, nobId: string) => {
    ...
    if (isChecked) {
      ...
      setModulesForm(
        modulesForm.filter(
          (code) => code !== nobCode && !associatedCodes.includes(code),
        ),
      );
    } else {
    ...
    try {
      if (settingsTab === 'profile') {
        ...
        await api.post('/setup/wizard/step-2', {
          ...addressForm,
          company_id: targetCompany.company_id,
          gps_latitude: addressForm.gps_latitude
            ? parseFloat(addressForm.gps_latitude)
            : undefined,
          gps_longitude: addressForm.gps_longitude
            ? parseFloat(addressForm.gps_longitude)
            : undefined,
        });
        ...
        await Promise.all([
          api.post(
            `/setup/wizard/step-4/${targetCompany.company_id}/${default_language_id}`,
          ),
          api.post(
            `/setup/wizard/step-5/${targetCompany.company_id}/${base_currency_id}`,
          ),
          api.post(
            `/setup/wizard/step-6/${targetCompany.company_id}/${encodeURIComponent(default_timezone_id)}/${country_id.toUpperCase()}`,
          ),
        ]);
      } else if (settingsTab === 'fiscal') {
        const isAprilStart =
          parseInt(fiscalForm.fiscal_start_month as any) === 4;
                  ...
                  companies.map((comp, idx) => (
                    <TableRow key={comp.company_id}>
                      ...
                      <TableCell className="p-4">
                        <span className="font-mono text-(--text-primary) bg-(--surface-raised) px-2 py-1 rounded-[var(--radius-xs)] border border-(--border) font-semibold">
                          {comp.company_code}
                        </span>
                      </TableCell>
                      <TableCell className="p-4">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[10px] text-(--text-secondary)">
                            {comp.company_display_name || comp.company_name}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell
                      ...
                      <TableCell className="p-4 text-center">
                        <span
                          className={`px-2 py-0.5 rounded-[var(--radius-xs)] text-[10px] font-semibold ${
                            comp.onboarding_status === 'COMPLETED'
        ...
        >
          <form onSubmit={handleCreateCompany} className="flex flex-col gap-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field
                label={t('ctCompanyCode')}
                htmlFor="create-company-code"
                required
              >
                <Input
              ...
              <Field
                label={t('coFieldDisplayName')}
                htmlFor="create-company-display-name"
              >
                <Input
              ...
              <Field
                label={t('ctClassification')}
                htmlFor="cfg-ct-classification"
              >
                <Select
              ...
              <Field
                label={t('ctPrimaryIndustry')}
                htmlFor="create-industry-type"
                required
              >
                <Input
              ...
              <Field
                label={t('ctOperatingCountry')}
                htmlFor="create-country-id"
                hint={t('ctHintCountryIso')}
              >
                <Input
                  id="create-country-id"
                  value={createForm.country_id}
                  onChange={(e) =>
                    setCreateForm({ ...createForm, country_id: e.target.value })
                  }
                />
              </Field>
              <Field
              ...
              >
                <Input
                  ...
                  onChange={(e) =>
                    setCreateForm({
                      ...createForm,
                      default_timezone_id: e.target.value,
                    })
                  }
                />
              </Field>
              <Field label={t('ctTaxRegIdShort')} htmlFor="create-tax-id">
                <Input
                  ...
                  onChange={(e) =>
                    setCreateForm({ ...createForm, tax_id: e.target.value })
                  }
                />
              </Field>
              <Field
                label={t('ctCorpRegNoShort')}
                htmlFor="create-registration-no"
              >
                <Input
            ...
            <div className="flex flex-col-reverse gap-3 border-t border-(--border) pt-4 sm:flex-row sm:justify-end">
              <Button
                ...
                className="min-h-10 px-4 text-sm cursor-pointer"
              >
                {t('cancel')}
              </Button>
              <Button
                type="submit"
                disabled={creating}
                className="min-h-10 px-5 text-sm cursor-pointer"
              >
  ...
  const currentLogoUrl =
    setupDetails?.company?.company_logo_url ||
    targetCompany?.company_logo_url ||
    '';
    ...
    <div className="flex w-full flex-col gap-4 animate-fade-in">
      ...
      <div className="company-settings-container nf-company-config flex flex-col gap-5">
        ...
        {!targetCompany ? (
          ...
          <div className="text-xs text-(--text-secondary) text-center py-12 animate-pulse">
            {t('ctLoadingSteps')}
          </div>
        ) : (
          <div className="min-w-0">
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="flex items-center gap-4 rounded-[var(--radius-sm)] border border-(--border) bg-(--surface-raised) p-4">
                    <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-[var(--radius-sm)] border border-(--border) bg-(--input-bg)">
                      {currentLogoUrl ? (
                        <img
                          src={
                            currentLogoUrl.startsWith('/')
                              ? `${backendUrl}${currentLogoUrl}`
                              : currentLogoUrl
                          }
                    ...
                    <ReadField
                      ...
                      value={
                        setupDetails?.company?.website ? (
                          <a
                            href={setupDetails.company.website}
                            target="_blank"
                            rel="noreferrer"
                  ...
                  <div className="grid grid-cols-1 gap-x-4 gap-y-5 sm:grid-cols-2">
                    <ReadField
                      ...
                      label={t('ctCompanyCode')}
                      value={
                        setupDetails?.company?.company_code ||
                        targetCompany?.company_code
                      }
                    />
                    <ReadField
                      label={t('ctLegalEntityName')}
                      value={
                        setupDetails?.company?.company_name ||
                    ...
                    <ReadField
                      label={t('coFieldDisplayName')}
                      value={
                        setupDetails?.company?.company_display_name ||
                        targetCompany?.company_display_name
                      }
                    />
                    <ReadField
                      label={t('ctBusinessClassification')}
                      value={
                        setupDetails?.company?.company_type ||
                    ...
                    <ReadField
                      label={t('ctPrimaryIndustry')}
                      value={
                        setupDetails?.company?.industry_type ||
                        targetCompany?.industry_type
                      }
                    />
                    <ReadField
                      mono
                      label={t('ctTaxRegIdFull')}
                      value={
                        setupDetails?.company?.tax_id || targetCompany?.tax_id
                      }
                    />
                    <ReadField
                      label={t('ctTaxRegimeScheme')}
                      value={setupDetails?.company?.tax_regime}
                    />
                    <ReadField
                      mono
                      label={t('ctCorpRegNoFull')}
                      value={
                        setupDetails?.company?.registration_no ||
                        targetCompany?.registration_no
                      }
                    />
                    <ReadField
                      label={t('ctIncorporationDate')}
                      value={setupDetails?.company?.incorporation_date}
                    />
                    <ReadField
                      mono
                      label={t('ctAutoVerifyDomain')}
                      value={setupDetails?.company?.email_domain}
                    />
                    <ReadField
                      mono
                      label={t('ctSupportEmail')}
                      value={setupDetails?.company?.support_email}
                    />
                    <ReadField
                      mono
                      label={t('ctPrimaryPhoneLandline')}
                      value={setupDetails?.company?.phone_primary}
                    />
                    <ReadField
                      ...
                      value={
                        <span className="flex items-center gap-2">
                          <span
                            className="h-4 w-4 rounded-[var(--radius-xs)] border border-(--border)"
                            style={{
                              backgroundColor:
                                setupDetails?.company?.primary_color_hex ||
                                targetCompany?.primary_color_hex ||
                                '#1F4E79',
                            }}
                          />
                ...
                <form onSubmit={handleSaveTab} className="flex flex-col gap-8">
                  <FieldGroup title={t('ctGrpIdentity')}>
                    <Field
                      ...
                      label={t('ctCompanyCode')}
                      htmlFor="profile-company-code"
                      hint={t('ctHintCodeFixed')}
                    >
                      <Input
                    ...
                    <Field
                      ...
                      label={t('coFieldDisplayName')}
                      htmlFor="profile-company-display-name"
                      hint={t('ctHintDisplayName')}
                    >
                      <Input
                  ...
                  <FieldGroup title={t('ctGrpRegistration')}>
                    <Field
                      className="sm:col-span-4"
                      label={t('ctTaxRegIdShort')}
                      htmlFor="profile-tax-id"
                    >
                      <Input
                        id="profile-tax-id"
                    ...
                    <Field
                      className="sm:col-span-4"
                      label={t('ctCorpRegNoShort')}
                      htmlFor="profile-registration-no"
                    >
                      <Input
                    ...
                    <Field
                      className="sm:col-span-4"
                      label={t('ctIncorporationDate')}
                      htmlFor="profile-incorporation-date"
                    >
                      <Input
                  ...
                  <FieldGroup title={t('ctGrpContactWeb')}>
                    ...
                    <Field
                      className="sm:col-span-6"
                      label={t('ctEmailDomainAutoVerify')}
                      htmlFor="profile-email-domain"
                      hint={t('ctHintEmailDomain')}
                    >
                      <Input
                  ...
                  <FieldGroup
                    title={t('ctGrpBranding')}
                    description={t('ctGrpBrandingDesc')}
                  >
                    <Field
                      className="sm:col-span-6"
                      label={t('ctCompanyLogoImage')}
                    >
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-[var(--radius-sm)] border border-dashed border-(--border) bg-(--input-bg)">
                          {profileForm.company_logo_url ? (
                            <img
                              src={
                                profileForm.company_logo_url.startsWith('/')
                                  ? `${backendUrl}${profileForm.company_logo_url}`
                                  : profileForm.company_logo_url
                              }
                    ...
                    <Field
                      className="sm:col-span-6"
                      label={t('ctBrandHexColor')}
                      htmlFor="profile-brand-hex"
                    >
            ...
            {settingsTab === 'address' &&
              (!isEditing ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <ReadField
                    label={t('ctAddressLabelTag')}
                    value={setupDetails?.address?.address_label || 'Primary HQ'}
                  />
                  <ReadField
                    label={t('ctAddressType')}
                    value={setupDetails?.address?.address_type || 'HQ'}
                  />
                  <ReadField
                    label={t('ctAddressLine1')}
                    value={setupDetails?.address?.line1}
                  />
                  <ReadField
                    label={t('ctAddressLine2')}
                    value={setupDetails?.address?.line2}
                  />
                  <ReadField
                    label={t('ctCity')}
                    value={setupDetails?.address?.city}
                  />
                  <ReadField
                    label={t('ctStateProvince')}
                    value={setupDetails?.address?.state_id}
                  />
                  <ReadField
                    mono
                    label={t('ctCountryCode')}
                    value={setupDetails?.address?.country_id}
                  />
                  <ReadField
                    mono
                    label={t('ctPincodePostal')}
                    value={setupDetails?.address?.pincode}
                  />
                  <ReadField
                    mono
                    className="sm:col-span-2"
                    label={t('ctGpsCoordinates')}
                    value={
                      setupDetails?.address?.gps_latitude &&
                      setupDetails?.address?.gps_longitude
                ...
                <form onSubmit={handleSaveTab} className="flex flex-col gap-8">
                  <FieldGroup title={t('ctGrpThisAddress')}>
                    ...
                    >
                      ...
                      >
                        ...
                        <option value="Branch">
                          {t('ctAddrBranchOffice')}
                        </option>
                        <option value="Warehouse">
                          {t('ctAddrWarehouseDepot')}
                        </option>
                  ...
                  <FieldGroup
                    title={t('ctGrpCoordinates')}
                    description={t('ctGrpCoordinatesDesc')}
                  >
                    <Field
                      className="sm:col-span-3"
                      label={t('ctGpsLatitude')}
                      htmlFor="address-gps-lat"
                    >
                      <Input
                        id="address-gps-lat"
                    ...
                    <Field
                      className="sm:col-span-3"
                      label={t('ctGpsLongitude')}
                      htmlFor="address-gps-lng"
                    >
                      <Input
                        id="address-gps-lng"
            ...
            {settingsTab === 'contact' &&
              (!isEditing ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <ReadField
                    label={t('ctContactPersonName')}
                    value={setupDetails?.contact?.full_name}
                  />
                  <ReadField
                    label={t('profileDesignation')}
                    value={setupDetails?.contact?.designation}
                  />
                  <ReadField
                    mono
                    label={t('ctPrimaryPhone')}
                    value={setupDetails?.contact?.phone_primary}
                  />
                  <ReadField
                    mono
                    label={t('ctSecondaryPhone')}
                    value={setupDetails?.contact?.phone_secondary}
                  />
                  <ReadField
                    mono
                    label={t('ctPrimaryEmail')}
                    value={setupDetails?.contact?.email}
                  />
                  <ReadField
                    label={t('ctAlertEmails')}
                    value={
                      setupDetails?.contact?.receives_alerts
                        ? 'Active - Receives ERP threshold notifications'
                        : 'Disabled'
                    }
                  />
                  <ReadField
                    className="sm:col-span-2"
                    label={t('ctExecReportEmails')}
                    value={
                      setupDetails?.contact?.receives_reports
                        ? 'Active - Receives periodic executive summary reports'
                        : 'Disabled'
                    }
                  />
                </div>
                ...
                <form onSubmit={handleSaveTab} className="flex flex-col gap-8">
                  <FieldGroup
                    title={t('ctGrpKeyContact')}
                    description={t('ctGrpKeyContactDesc')}
                  >
                    <Field
                      className="sm:col-span-7"
                      label={t('ctKeyContactFullName')}
                  ...
                  <FieldGroup title={t('ctGrpHowToReach')}>
                    ...
                    <Field
                      className="sm:col-span-5"
                      label={t('ctPrimaryContactPhone')}
                      htmlFor="contact-phone-primary"
                      required
                    >
                      <Input
                    ...
                    <Field
                      className="sm:col-span-5"
                      label={t('ctSecondaryContactPhone')}
                  ...
                  <FieldGroup
                    title={t('ctGrpNotifications')}
                    description={t('ctGrpNotificationsDesc')}
                  >
                    ...
                    <label className="sm:col-span-6 flex cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border border-(--border) bg-(--surface-raised) p-3">
                      <input
                        ...
                        onChange={(e) =>
                          setContactForm({
                            ...contactForm,
                            receives_reports: e.target.checked,
                          })
                        }
                        className="mt-0.5 h-4 w-4 cursor-pointer rounded-[var(--radius-xs)] border-(--border) bg-(--input-bg) text-(--accent)"
                      />
                      <span className="text-sm text-(--text-primary)">
                        {t('ctReceiveExecReports')}
                      </span>
                    </label>
                  </FieldGroup>
            ...
            {settingsTab === 'localization' &&
              (!isEditing ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <ReadField
                    label={t('ctDefaultLanguage')}
                    value={
                      languages.find(
                        (l) =>
                          l.lang_id ===
                          setupDetails?.company?.default_language_id,
                      )?.lang_name || setupDetails?.company?.default_language_id
                    }
                  />
                  <ReadField
                  ...
                  <ReadField
                    mono
                    label={t('ctDefaultTimezone')}
                    value={setupDetails?.company?.default_timezone_id}
                  />
                  <ReadField
                    mono
                    label={t('ctCountryLocaleCode')}
                    value={setupDetails?.company?.country_id}
                  />
                </div>
              ) : (
                <form onSubmit={handleSaveTab} className="flex flex-col gap-8">
                  <FieldGroup title={t('ctGrpLanguageMoney')}>
                    <Field
                      className="sm:col-span-6"
                      label={t('ctDefaultLanguage')}
                      htmlFor="cfg-ct-default-language"
                    >
                      <Select
                        ...
                        onChange={(e) =>
                          setLocalizationForm({
                            ...localizationForm,
                            default_language_id: e.target.value,
                          })
                        }
                      >
                    ...
                    <Field
                      className="sm:col-span-6"
                      label={t('ctBaseCurrency')}
                      htmlFor="cfg-ct-base-currency"
                      hint={t('ctHintBaseCurrency')}
                    >
                      <Select
                        ...
                        onChange={(e) =>
                          setLocalizationForm({
                            ...localizationForm,
                            base_currency_id: e.target.value,
                          })
                        }
                      >
                  ...
                  <FieldGroup title={t('ctGrpPlaceTime')}>
                    <Field
                      className="sm:col-span-5"
                      label={t('ctTimezoneId')}
                      htmlFor="localization-timezone"
                      required
                      hint={t('ctHintTimezoneIana')}
                    >
                      <Input
                    ...
                    <Field
                      className="sm:col-span-3"
                      label={t('ctOperatingCountryCode')}
            ...
            {settingsTab === 'fiscal' &&
              (!isEditing ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <ReadField
                    mono
                    label={t('ctFiscalYearFormat')}
                    value={
                      setupDetails?.fiscal?.fiscal_year_format || 'FY APR-MAR'
                    }
                  />
                  <ReadField
                  ...
                  <ReadField
                    mono
                    label={t('ctCurrentFiscalYear')}
                    value={
                      setupDetails?.fiscal?.current_fiscal_year || '2026-27'
                    }
                  />
                  <ReadField
                    label={t('ctPeriodType')}
                    value={setupDetails?.fiscal?.period_type || 'MONTHLY'}
                  />
                  <ReadField
                    label={t('ctAccountingStandard')}
                    value={
                      setupDetails?.fiscal?.accounting_standard || 'Local GAAP'
                    }
                  />
                  <ReadField
                    label={t('ctDepreciationModel')}
                    value={
                      setupDetails?.fiscal?.depreciation_method ||
                      'SLM (Straight Line)'
                    }
                  />
                  <ReadField
                    label={t('ctInventoryCostingModel')}
                    value={setupDetails?.fiscal?.inventory_valuation || 'FIFO'}
                  />
                  <ReadField
                    label={t('ctTaxFilingFrequency')}
                    value={
                      setupDetails?.fiscal?.gst_filing_frequency || 'MONTHLY'
                    }
                  />
                  <ReadField
                    label={t('ctStatutoryTaxAudit')}
                    value={
                      setupDetails?.fiscal?.tax_audit_applicable
                        ? 'Mandatory Tax Audit Applicable'
                        : 'Not Applicable'
                    }
                  />
                  <ReadField
                    mono
                    label={t('ctDecimalPrecision')}
                    value={`${setupDetails?.fiscal?.decimal_places ?? 2} decimal places`}
                  />
                </div>
              ) : (
                <form onSubmit={handleSaveTab} className="flex flex-col gap-8">
                  <FieldGroup title={t('ctGrpTheYear')}>
                    <Field
                      className="sm:col-span-4"
                      label={t('ctFiscalStartMonth')}
                      htmlFor="cfg-ct-fiscal-start-month"
                    >
                      <Select
                    ...
                    >
                      <Input
                        ...
                        onChange={(e) =>
                          setFiscalForm({
                            ...fiscalForm,
                            current_fiscal_year: e.target.value,
                          })
                        }
                        required
                      />
                    ...
                    <Field
                      className="sm:col-span-3"
                      label={t('ctAccountingPeriodicity')}
                      htmlFor="cfg-ct-accounting-periodicity"
                    >
                      <Select
                  ...
                  <FieldGroup
                    title={t('ctGrpAccountingPolicy')}
                    description={t('ctGrpAccountingPolicyDesc')}
                  >
                    <Field
                      className="sm:col-span-4"
                      label={t('ctAccountingStandard')}
                      htmlFor="cfg-ct-accounting-standard"
                    >
                      <Select
                    ...
                    <Field
                      className="sm:col-span-4"
                      label={t('ctDepreciationModel')}
                      htmlFor="cfg-ct-depreciation-model"
                    >
                      <Select
                      ...
                      >
                        ...
                        <option value="UNITS_OF_PRODUCTION">
                          {t('ctDeprUnitsOfProduction')}
                        </option>
                      </Select>
                    </Field>
                    <Field
                      className="sm:col-span-4"
                      label={t('ctInventoryCostingMethod')}
                      htmlFor="cfg-ct-inventory-costing-method"
                    >
                      <Select
                        ...
                        onChange={(e) =>
                          setFiscalForm({
                            ...fiscalForm,
                            inventory_valuation: e.target.value,
                          })
                        }
                      >
                        <option value="FIFO">{t('ctCostFifo')}</option>
                        <option value="Weighted Average">
                          {t('ctCostWeightedAverage')}
                        </option>
                        <option value="STANDARD COSTING">
                          {t('ctCostStandard')}
                        </option>
                      </Select>
                    </Field>
                    <Field
                      className="sm:col-span-4"
                      label={t('ctDecimalPrecision')}
                      htmlFor="cfg-ct-decimal-precision"
                    >
                      <Select
                  ...
                  <FieldGroup title={t('ctGrpTaxCompliance')}>
                    <Field
                      className="sm:col-span-4"
                      label={t('ctTaxFilingFrequency')}
                      htmlFor="cfg-ct-tax-filing-frequency"
                    >
                      <Select
                        ...
                        onChange={(e) =>
                          setFiscalForm({
                            ...fiscalForm,
                            gst_filing_frequency: e.target.value,
                          })
                        }
                      >
                    ...
                    <label className="sm:col-span-8 flex cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border border-(--border) bg-(--surface-raised) p-3">
                      <input
                        ...
                        onChange={(e) =>
                          setFiscalForm({
                            ...fiscalForm,
                            tax_audit_applicable: e.target.checked,
                          })
                        }
                        className="mt-0.5 h-4 w-4 cursor-pointer rounded-[var(--radius-xs)] border-(--border) bg-(--input-bg) text-(--accent)"
                      />
                      <span className="text-sm text-(--text-primary)">
                        {t('ctStatutoryAuditApplicable')}
                      </span>
                    </label>
                  </FieldGroup>
            ...
            {settingsTab === 'modules' &&
              (!isEditing ? (
                <div className="flex flex-col gap-4">
                  <span className="nf-text-label text-(--text-secondary)">
                    {t('ctActivatedSectors')}
                  </span>
                  {modulesForm.length === 0 ? (
                    <div className="text-xs text-(--text-secondary) bg-(--surface-raised) p-4 rounded-[var(--radius-sm)] border border-(--border)">
                      {t('ctNoModulesEnabled')}
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {nobs.map((n: any) => {
                        ...
                        const activeLobs = associatedLobs.filter((l) =>
                          modulesForm.includes(l.lob_code),
                        );

                        return (
                          <div
                            key={n.nob_id}
                            className="p-4 rounded-[var(--radius-sm)] bg-(--surface-raised) border border-(--border) flex flex-col gap-2"
                          >
                            <div className="flex items-center gap-2 border-b border-(--border) pb-2">
                              <span className="font-semibold text-xs text-(--text-primary)">
                                {n.nob_name}
                              </span>
                              <span className="text-[9px] bg-(--accent)/10 text-(--accent) font-semibold border border-(--accent)/20 px-1.5 py-0.5 rounded-[var(--radius-xs)] font-mono uppercase shrink-0">
                                {t('statusActive')}
                              </span>
                            </div>
                              ...
                              {activeLobs.length === 0 ? (
                                ...
                                <div className="flex flex-wrap gap-1.5 mt-1">
                                  {activeLobs.map((lob) => (
                                    <span
                                      key={lob.lob_id}
                                      className="text-[10px] text-(--text-secondary) bg-(--surface-raised) px-2 py-0.5 rounded-[var(--radius-xs)] border border-(--border)"
                                    >
                                      {lob.lob_name}
                                    </span>
                                  ))}
                    ...
                    {nobs.map((n: any) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {isNobChecked && (
                            <div
                              className="mt-3 pt-3 border-t border-(--border)/80 flex flex-col gap-2"
                              onClick={(e) => e.stopPropagation()}
                            >
                              <span className="text-[10px] font-semibold text-(--accent) uppercase tracking-wider">
                                {t('ctSelectActiveLobs')}
                              </span>
                              {loadingLobs[n.nob_id] ? (
                                <div className="text-[11px] text-(--text-secondary) animate-pulse py-1">
                                  {t('ctLoadingSubSectors')}
                                </div>
                                ...
                                <div className="flex flex-col gap-1.5 mt-1">
                                  {associatedLobs.map((lob: any) => {
                                    const isLobChecked = modulesForm.includes(
                                      lob.lob_code,
                                    );
                                    return (
                                      <label
