...
export default function SchedulerListPanel() {
  ...
  const load = () => {
    ...
    api
      .get(`/scheduler-header?${params.toString()}`)
      .then((r) => setRows(unwrap<Row[]>(r) || []))
      .catch((err: any) =>
        setError(err?.message || 'Could not load schedulers.'),
      )
  ...
  return (
    <div className="flex flex-col gap-4">
            ...
            <TableBody>
              {rows.map((r) => (
                <TableRow
                  ...
                  onClick={() => setViewingId(r.scheduler_id)}
                >
                  <TableCell
                    className="px-3 py-2 font-mono text-[11px] font-semibold text-[var(--accent)]"
                    title={r.scheduler_id}
                  >
                  ...
                  <TableCell className="px-3 py-2">
                    <StatusBadge status={r.scheduler_status} />
                  </TableCell>
                  <TableCell className="px-3 py-2" style={S.sub}>
                    {r.effective_from || '—'}
                  </TableCell>
      ...
      <Dialog
        ...
        title={
          viewingRow
            ? t('schDetailTitle', { batchNo: viewingRow.batch_no || '' })
            : t('schTitle')
        }
        maxWidth="xl"
      >
