...
export default function SchedulerBatchDataEntry() {
  ...
  const saveDay = async () => {
    ...
    try {
      ...
      for (const line of form.lines) {
        ...
        const unchanged = line.entry
          && line.entry.entered_value === value
  ...
  const isWorkingDay = !!form && form.date === (form.backlog[0] ?? form.today);
  const lockedReason = form?.lines.find((l) => !l.editable)?.locked_reason ?? null;
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_290px]">
      <div className="min-w-0 space-y-4">
          ...
          <div className="flex flex-wrap items-end gap-3">
            <label className="min-w-[190px] flex-1">
              ...
              <select
                ...
                className="h-11 w-full rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--surface)] px-3 text-sm text-(--text-primary)"
              >
                {batches.map((b) => (
            ...
            <div>
              ...
              <div className="flex h-11 items-center gap-2 rounded-[var(--radius-sm)] border border-[var(--border)] px-3 text-sm font-semibold text-(--text-primary)">
        ...
        {form && form.backlog.length > 0 && (
          <div className="flex items-start gap-3 rounded-[var(--radius-md)] border border-amber-500/40 bg-amber-500/10 p-4">
            ...
            {form.stages.map((stage) => {
              ...
              return (
                <button
                  ...
                  className={[
                    "min-w-[148px] shrink-0 rounded-[var(--radius-md)] border p-3 text-left transition",
                    active ? "border-[var(--accent)] bg-[var(--accent)]/8" : "border-[var(--border)] bg-[var(--surface)]",
                    stage.scheduled ? "cursor-pointer hover:border-[var(--accent)]" : "cursor-not-allowed opacity-70",
                  ].join(" ")}
                >
                  ...
                  <p className="mt-1 text-xs text-(--text-secondary)">
                    {stage.animal_count == null ? "—" : t("deAnimals", { count: String(stage.animal_count) })}
                  </p>
        ...
        {form && form.lines.length > 0 && (
          <div className="space-y-2">
            {lockedReason && (
              <div className="flex items-center gap-2 rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--surface-muted)] px-3 py-2 text-[13px] text-(--text-secondary)">
              ...
              return (
                <Card key={line.line_id} className="p-4">
                  <div className="flex flex-wrap items-baseline justify-between gap-2">
                    ...
                    {line.item_name && (
                      <span className="text-xs text-(--text-secondary)">{line.item_name}</span>
                    )}
                  </div>
                    ...
                    <label className="min-w-[150px] flex-1">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="h-11 w-full rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--surface)] px-3 text-base text-(--text-primary) disabled:opacity-60"
                        />
                        {line.uom && <span className="text-sm text-(--text-secondary)">{line.uom}</span>}
                      </div>
                    </label>
                    ...
                    {line.lot_required && (
                      <label className="min-w-[130px]">
                        ...
                        <input
                          ...
                          className="h-11 w-full rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--surface)] px-3 text-sm text-(--text-primary) disabled:opacity-60"
                        />
                      </label>
                    )}
            ...
            <div className="flex flex-wrap items-center gap-3 pt-1">
              <Button onClick={saveDay} disabled={saving || form.lines.every((l) => !l.editable)} className="gap-2">
                <Save size={15} />
      ...
      <aside className="space-y-1 lg:sticky lg:top-4 lg:self-start">
        <button
          type="button"
          onClick={() => { setStageId(null); setDate(form?.backlog[0] ?? form?.today ?? ""); }}
          className={[
            "w-full rounded-[var(--radius-sm)] border px-3 py-2.5 text-left text-sm font-semibold transition",
            isWorkingDay
        ...
        {history.length > 0 && (
          <p className="px-3 pt-3 pb-1 text-[11px] font-medium uppercase tracking-wide text-(--text-muted)">
            {t("deHistory")}
          </p>
        )}
        {history.map((day) => (
          <button
            ...
            className={[
              "w-full rounded-[var(--radius-sm)] border px-3 py-2 text-left text-[13px] transition",
              form?.date === day && !isWorkingDay
