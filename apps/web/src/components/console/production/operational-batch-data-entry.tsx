...
const entryKey = (lineId: string, animalId?: string) => (animalId ? `${animalId}:${lineId}` : lineId);
...
const defaultEntryValue = (line: Row) => {
  ...
  if (!isTextCapture(line) && line.expected_qty != null) return String(line.expected_qty);
  return "";
};
...
export default function OperationalBatchDataEntry() {
  ...
  const [minSelectableDate, setMinSelectableDate] = useState<string | null>(null);
  ...
  const [lockInfo, setLockInfo] = useState<{ status: string | null; lockedBy?: string | null; lockedAt?: string | null }>({ status: null });
  ...
  const [batchAnimalRosterLoading, setBatchAnimalRosterLoading] = useState(false);
  ...
  const [animalListStageId, setAnimalListStageId] = useState<string | null>(null);

  // Per-animal manual stage change (ANIMAL_WISE) — reuses the same modal the
  // Animal Register screen already uses.
  const [stageTransitionAnimal, setStageTransitionAnimal] = useState<Row | null>(null);
  ...
  const [bulkStageTransitionForm, setBulkStageTransitionForm] = useState<{ to_stage_id: string; to_location_id: string; reason: string }>({ to_stage_id: "", to_location_id: "", reason: "" });
  const [bulkStageTransitionSaving, setBulkStageTransitionSaving] = useState(false);
  ...
  const [changeStageForm, setChangeStageForm] = useState<{ to_stage_code: string; remarks: string }>({ to_stage_code: "", remarks: "" });
  const [changeStageOptions, setChangeStageOptions] = useState<string[]>([]);
  const [changeStageOptionsLoading, setChangeStageOptionsLoading] = useState(false);
  ...
  const stageActivityLabel = (stage: Row) => {
    ...
    if (types.length === 1) return `${LINE_TYPE_LABELS[types[0]] || types[0]} Activity`;
  ...
  type ActivityBucketKey = "FEED" | "MEDICINE" | "WEIGHT" | "MORTALITY" | "TRANSFER" | "OUTPUT" | "RESOURCE" | "OVERHEAD" | "OBSERVATION";
  const ACTIVITY_BUCKETS: Record<ActivityBucketKey, { title: string; icon: typeof Wheat; color: string }> = {
    FEED: { title: "Feed Consumption", icon: Wheat, color: "#16a34a" },
    MEDICINE: { title: "Medicine / Vaccine Consumption", icon: Syringe, color: "#2563eb" },
    WEIGHT: { title: "Weight & Body Condition", icon: Scale, color: "#7c3aed" },
    MORTALITY: { title: "Mortality", icon: HeartCrack, color: "#dc2626" },
    TRANSFER: { title: "Transfer In / Out", icon: ArrowLeftRight, color: "#ea580c" },
    ...
    OBSERVATION: { title: "Observations", icon: ClipboardList, color: "#475569" },
  };
  const ACTIVITY_BUCKET_ORDER: ActivityBucketKey[] = ["FEED", "MEDICINE", "WEIGHT", "MORTALITY", "TRANSFER", "OUTPUT", "RESOURCE", "OVERHEAD", "OBSERVATION"];
  const bucketOfLine = (line: Row): ActivityBucketKey => {
    if (line.line_type === "CONSUMPTION") return line.item_type === "MEDICINE" ? "MEDICINE" : "FEED";
    if (line.line_type === "DESCRIPTIVE") {
      if (line.kpi_metric === "BODY_WEIGHT" || line.kpi_metric === "BCS_SCORE") return "WEIGHT";
  ...
  const groupLinesByBucket = (lines: Row[]) => {
    ...
    return ACTIVITY_BUCKET_ORDER
      .filter((key) => groups.has(key))
      .map((key) => ({ key, ...ACTIVITY_BUCKETS[key], lines: groups.get(key)! }));
  };
    ...
    api.get(`/batch?companyId=${companyId}&status=ACTIVE&limit=50`)
      .then(async (res) => {
        ...
        if (mapped.length > 0) {
          ...
          const preselect = linkedId && mapped.some((b) => b.id === linkedId) ? linkedId : mapped[0].id;
  ...
  const loadDataEntry = () => {
    ...
    Promise.all([
      ...
      api.get(`/stage`).catch(() => []),
      api.get(`/location?companyId=${getActiveCompanyId()}&limit=200`).catch(() => []),
    ])
      .then(([schedRes, batchRes, stageRes, locationRes]) => {
        const schedData = schedRes?.data ?? schedRes;
        const batchData = batchRes?.data ?? batchRes;
        const isAnimalWise = batchData?.tracking_mode === "ANIMAL_WISE" || Array.isArray(schedData?.stages);
        ...
        if (isAnimalWise) {
          ...
          for (const stage of stages) {
            (stage.animals || []).forEach((animal: Row, animalIdx: number) => {
              for (const line of animal.lines || []) {
                ...
                if (animalIdx === 0) values[entryKey(line.line_id, "__ALL__")] = enteredStr;
              }
            });
          }
          ...
          setSelectedStageId((prev) => {
            const prevStage = prev ? stages.find((s) => s.stage_id === prev) : undefined;
            if (prevStage && prevStage.lock_status !== "LOCKED") return prev;
            const nextOpen = progress.find((p: Row) => p.animal_count > 0 && stages.find((s) => s.stage_id === p.stage_id)?.lock_status !== "LOCKED");
          ...
          const allLocked = activeStages.length > 0 && activeStages.every((p: Row) => stages.find((s) => s.stage_id === p.stage_id)?.lock_status === "LOCKED");
          if (allLocked && !userPickedDateRef.current) {
            const [ly, lm, ld] = selectedDate.split("-").map(Number);
            const dayAfter = new Date(Date.UTC(ly, lm - 1, ld + 1)).toISOString().slice(0, 10);
          ...
          for (const l of lines) values[entryKey(l.line_id)] = defaultEntryValue(l);

          const dayStatus: string | null = schedData?.lock_status || null;
          setLockInfo({ status: dayStatus, lockedBy: schedData?.locked_by || null, lockedAt: schedData?.locked_at || null });

          if (dayStatus === "LOCKED") {
            const [ly, lm, ld] = selectedDate.split("-").map(Number);
            const dayAfter = new Date(Date.UTC(ly, lm - 1, ld + 1)).toISOString().slice(0, 10);
            setMinSelectableDate((prev) => (!prev || dayAfter > prev) ? dayAfter : prev);
        ...
        const weightLine = (schedData?.lines ?? []).find((ln: any) => ln.line_type === "DESCRIPTIVE" && ln.kpi_metric === "BODY_WEIGHT");
        if (weightLine) setAvgWeight(weightLine.already_entered_qty > 0 ? weightLine.already_entered_qty : 0);
  ...
  useEffect(() => {
    if (!currentBatch?.id) { setBatchAnimalRoster([]); return; }
    const companyId = getActiveCompanyId();
    ...
    api.get(`/animal?companyId=${companyId}&currentBatchId=${currentBatch.id}&limit=500`)
      .then((res) => {
        ...
        setBatchAnimalRoster(list.map((a) => ({ ...a, breed_name: a.breed_name || breedLabel(a.breed_id) })));
      })
  ...
  const dataEntryCanSave = (line: Row, animalId?: string) => {
    ...
    if (line.line_type === "TRANSFER" && !dataEntryDestBatches[key]) return false;
    return true;
  };
  ...
  const handleDataEntrySave = async (line: Row, animalId?: string) => {
    ...
    setDataEntryError("");
    try {
      const payload: Row = { line_id: line.line_id, entry_date: selectedDate, draft: true };
      ...
      if (line.line_type === "TRANSFER") payload.destination_batch_id = dataEntryDestBatches[key];
  ...
  const handleDataEntrySaveAllAnimals = async (line: Row) => {
    ...
    setDataEntryError("");
    try {
      const animalIds = (selectedStage.animals || []).map((a: Row) => a.animal_id);
      await Promise.all(animalIds.map((animalId: string) => {
        const payload: Row = { line_id: line.line_id, entry_date: selectedDate, draft: true, animal_id: animalId };
        if (isTextCapture(line)) {
          payload.entered_text = dataEntryTexts[key];
        } else {
          payload.entered_value = Number(dataEntryValues[key]);
  ...
  const handlePostStageDay = async () => {
    ...
    setStageActionError("");
    try {
      await api.post(`/batch/${currentBatch.id}/stage/${selectedStageId}/post-day?date=${selectedDate}`, {});
      loadDataEntry();
    } catch (err: any) {
  ...
  const handleReopenStageDay = async () => {
    ...
    setStageActionError("");
    try {
      await api.post(`/batch/${currentBatch.id}/stage/${selectedStageId}/reopen?date=${selectedDate}`, { reason: reopenReason });
      setReopenBoxOpen(false);
      setReopenReason("");
      loadDataEntry();
    } catch (err: any) {
  ...
  const handleBulkStageTransition = async (animalIds: string[]) => {
    ...
    try {
      ...
      if (failed.length) {
        setBulkStageTransitionError(`Moved ${animalIds.length - failed.length} of ${animalIds.length}. Could not move: ${failed.map((f: Row) => f.reason).join("; ")}`);
      } else {
        setBulkStageTransitionOpen(false);
        setBulkStageTransitionForm({ to_stage_id: "", to_location_id: "", reason: "" });
      }
      loadDataEntry();
      setBatchAnimalRoster((prev) => [...prev]);
    } catch (err: any) {
      setBulkStageTransitionError(err?.message || "Could not move these animals.");
    } finally {
  ...
  const handlePostEntry = async () => {
    ...
    try {
      ...
      setSaveSuccessMsg(`✓ ${selectedDate} posted and locked for batch ${currentBatch.code}. Moved to ${nextDate} for the next entry.`);
      setSelectedDate(nextDate);
      setTimeout(() => setSaveSuccessMsg(""), 5000);
    } catch (err: any) {
      const errMsg = err?.message || err?.error || "Failed to post & lock the daily entry.";
  ...
  const openChangeStage = async () => {
    ...
    try {
      const stageRows = unwrap<Row[]>(await api.get(`/stage?lobId=${currentBatch.lobId}&isActive=true&limit=200`)) || [];
      const codes = Array.from(
        new Set(stageRows.map((s: Row) => s.stage_code).filter((c: string | null) => !!c && c !== currentBatch.currentStageCode))
      ) as string[];
  ...
  const handleChangeStage = async () => {
    ...
    setChangeStageError("");
    try {
      if (!changeStageForm.to_stage_code) throw new Error(t("blErrDestStageRequired"));
  ...
  const renderDataEntryRow = (line: Row, animalId?: string, rowLocked?: boolean, broadcast?: boolean) => {
    const key = entryKey(line.line_id, animalId);
    return (
      <TableRow key={key}>
        ...
        <TableCell className="px-2 py-1.5 w-28">
          {isTextCapture(line) ? (
            <input
              ...
              onChange={(e) => setDataEntryTexts((v) => ({ ...v, [key]: e.target.value }))}
            ...
            <input
              ...
              onChange={(e) => setDataEntryValues((v) => ({ ...v, [key]: e.target.value }))}
              className={inputCls}
              style={S.input}
              disabled={rowLocked}
            />
          ...
          {line.lot_required && (
            <input
              ...
              onChange={(e) => setDataEntryLotNos((v) => ({ ...v, [key]: e.target.value }))}
          ...
          {line.line_type === "TRANSFER" && (
            <select
              ...
              onChange={(e) => setDataEntryDestBatches((v) => ({ ...v, [key]: e.target.value }))}
        ...
        <TableCell className="px-2 py-1.5">
          <button
            onClick={() => (broadcast ? handleDataEntrySaveAllAnimals(line) : handleDataEntrySave(line, animalId))}
            disabled={rowLocked || dataEntrySavingId === key || !dataEntryCanSave(line, animalId)}
  ...
  const renderActivityBoxes = (lines: Row[], animalId: string | undefined, rowLocked: boolean | undefined, broadcast?: boolean) => {
    const boxes = groupLinesByBucket(lines);
    if (!boxes.length) return <p className="text-xs" style={S.muted}>{t("blNoParamsScheduled")}</p>;
    return (
      <div className="flex flex-col gap-3">
        {boxes.map((box) => {
          const Icon = box.icon;
          const uoms = new Set(box.lines.map((l) => l.uom).filter(Boolean));
          const canTotal = uoms.size === 1 && box.lines.every((l) => !isTextCapture(l));
          const total = canTotal
            ? box.lines.reduce((sum, l) => {
                const v = Number(dataEntryValues[entryKey(l.line_id, animalId)]);
                return sum + (Number.isFinite(v) ? v : 0);
              }, 0)
            : null;
          return (
            <div key={box.key} className="rounded-lg border overflow-hidden" style={S.surface}>
              <div className="flex items-center gap-2 px-3 py-2 border-b" style={{ borderColor: "var(--border)" }}>
                <Icon className="h-4 w-4 shrink-0" style={{ color: box.color }} />
                <span className="text-xs font-bold" style={{ color: box.color }}>{box.title}</span>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-left text-xs">
                  {dataEntryTableHeader}
                  <TableBody>{box.lines.map((line) => renderDataEntryRow(line, animalId, rowLocked, broadcast))}</TableBody>
                </table>
              </div>
              {total != null && (
                <div className="flex items-center justify-end gap-1.5 px-3 py-1.5 border-t text-[11px]" style={{ borderColor: "var(--border)" }}>
                  <span style={S.muted}>Total:</span>
                  <span className="font-bold" style={S.primary}>{total.toLocaleString(undefined, { maximumFractionDigits: 2 })} {[...uoms][0]}</span>
                </div>
              )}
            </div>
          );
  ...
  const selectedStage = dataEntryStages.find((s) => s.stage_id === selectedStageId);
  const selectedProgress = dataEntryProgress.find((p) => p.stage_id === selectedStageId);
  const animalListStage = animalListStageId ? dataEntryProgress.find((p) => p.stage_id === animalListStageId) : null;
  const animalListRows = animalListStageId ? batchAnimalRoster.filter((a) => a.current_stage_id === animalListStageId) : [];
    ...
    <div className="space-y-6 animate-fade-in text-[var(--text-primary)]">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-5">
          <div className="flex items-start gap-4 flex-1 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <span className="text-[10px] font-bold uppercase tracking-wider" style={S.muted}>
              ...
              <div className="mt-1.5 flex items-center gap-2.5 flex-wrap">
                ...
                {!isAnimalWise && currentBatch?.lobId && (
                  <Button variant="outline" size="sm" onClick={openChangeStage} className="h-7 text-[11px] gap-1.5 font-semibold shrink-0">
              ...
              <div className="mt-3 flex flex-wrap gap-x-6 gap-y-1.5 text-xs">
                <div className="whitespace-nowrap">
                  ...
                  <span className="font-semibold" style={S.primary}>{currentBatch?.breed || "—"}</span>
                </div>
                <div className="whitespace-nowrap">
                  <span style={S.muted}>{t("bdeType")} </span>
                  <span className="font-semibold" style={S.primary}>{currentBatch?.type || "—"}</span>
                </div>
                <div className="whitespace-nowrap">
                  <span style={S.muted}>{t("bdeStartDate")} </span>
                  <span className="font-semibold" style={S.primary}>{currentBatch?.startDate || "—"}</span>
                </div>
                <div className="whitespace-nowrap">
                  <span style={S.muted}>{isAnimalWise ? "Tracking Mode:" : t("bdeCurrentStage")} </span>
                  {isAnimalWise ? (
                    <span className="font-semibold" style={S.accent}>Animal Wise</span>
                  ) : (
                    <>
                      <span className="font-semibold" style={S.accent}>{currentBatch?.currentStage || "—"}</span>
          ...
          >
            {[
              { label: t("bdeAssigned"), value: currentBatch?.assignedCount ?? 0, color: "var(--text-primary)" },
              { label: t("bdeCurrent"), value: currentHeadCount, color: "var(--success)" },
              { label: t("bdeMortality"), value: batchMortality, color: "var(--danger)" },
              { label: t("bdeTransferred"), value: currentBatch?.transferredCount ?? 0, color: "var(--text-primary)" },
            ].map((kpi) => (
              <div key={kpi.label} className="flex min-w-[82px] flex-col items-center justify-center gap-0.5 px-4 py-2.5" style={{ backgroundColor: "var(--surface-raised)" }}>
                <p className="text-[10px] uppercase font-bold tracking-wide" style={S.muted}>{kpi.label}</p>
      ...
      {activeTab === "overview" && (
        <div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-2xs space-y-5">
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-[var(--text-primary)] mb-3">Batch Summary</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-6 gap-y-3 text-xs">
              <div>
                <p className="text-[10px] uppercase font-bold text-[var(--text-muted)]">{t("bdeBreed")}</p>
                <p className="font-semibold text-[var(--text-primary)]">{currentBatch?.breed || "—"}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase font-bold text-[var(--text-muted)]">{t("bdeType")}</p>
                <p className="font-semibold text-[var(--text-primary)]">{currentBatch?.type || "—"}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase font-bold text-[var(--text-muted)]">{t("bdeStartDate")}</p>
                <p className="font-semibold text-[var(--text-primary)]">{currentBatch?.startDate || "—"}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase font-bold text-[var(--text-muted)]">{isAnimalWise ? "Tracking Mode" : t("bdeCurrentStage")}</p>
                {isAnimalWise ? (
                  <p className="font-semibold text-[var(--accent)]">Animal Wise</p>
                ) : (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-[var(--surface-raised)] p-4 rounded-[var(--radius-md)] border border-[var(--border)]">
            <div className="text-center">
              ...
              <p className="text-lg font-bold text-[var(--text-primary)]">{currentBatch?.assignedCount ?? 0}</p>
            </div>
            <div className="text-center">
              <p className="text-[10px] uppercase font-bold text-[var(--text-muted)]">{t("bdeCurrent")}</p>
              <p className="text-lg font-bold" style={{ color: "var(--success)" }}>{currentHeadCount}</p>
            </div>
            <div className="text-center">
              <p className="text-[10px] uppercase font-bold text-[var(--text-muted)]">{t("bdeMortality")}</p>
              <p className="text-lg font-bold" style={{ color: "var(--danger)" }}>{batchMortality}</p>
            </div>
            <div className="text-center">
              <p className="text-[10px] uppercase font-bold text-[var(--text-muted)]">{t("bdeTransferred")}</p>
              <p className="text-lg font-bold text-[var(--text-primary)]">{currentBatch?.transferredCount ?? 0}</p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs bg-[var(--surface-raised)] p-4 rounded-[var(--radius-md)] border border-[var(--border)]">
                <div>
                  ...
                  {locked ? (
                    <span className="font-bold inline-flex items-center gap-1" style={{ color: "var(--accent)" }}>
                      <Lock className="h-3.5 w-3.5" /> Posted & Locked
                    </span>
                  ) : (
                    <span className="font-bold text-[var(--text-primary)]">Draft — not yet posted</span>
                  )}
                  {locked && lockInfo.lockedAt && (
                    <span className="text-[11px] text-[var(--text-muted)] ml-2">since {String(lockInfo.lockedAt).slice(0, 16).replace("T", " ")}</span>
                  )}
                </div>
              </div>
            </div>
      ...
      <div className="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] p-4 flex flex-wrap items-center justify-between gap-4 shadow-2xs">
        ...
        {!isAnimalWise && (
          ...
          >
            {posting ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Lock className="w-3.5 h-3.5" />}
            {locked ? "Posted & Locked" : posting ? "Posting…" : "Post Entry"}
          </Button>
        )}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {(selectedStage.animals || []).length > 0 && (
              <select
                ...
                style={S.input}
              >
                <option value="ALL">All animals in this stage ({selectedStage.animal_count})</option>
                {selectedStage.animals.map((a: Row) => (
                  <option key={a.animal_id} value={a.animal_id}>{a.animal_code}</option>
                ))}
              </select>
            )}
            {selectedStage.lock_status === "LOCKED" ? (
              <span className="text-xs font-semibold inline-flex items-center gap-1.5 px-3 py-1.5" style={{ color: "var(--success, #16a34a)" }}>
                <Lock className="h-3.5 w-3.5" /> Posted & Locked
              </span>
            ) : (
              <Button
                ...
                className="nf-btn-primary text-xs h-8 gap-1.5 font-semibold"
              >
                {stageActionBusy ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Lock className="w-3.5 h-3.5" />}
      ...
      {locked && !isAnimalWise && (
        <div
          ...
          style={{ backgroundColor: "var(--surface-raised)", color: "var(--accent)", borderColor: "var(--border)" }}
        >
          <Lock className="h-4 w-4 shrink-0" />
          <span>
            {selectedDate} has been posted and locked{lockInfo.lockedAt ? ` (${String(lockInfo.lockedAt).slice(0, 16).replace("T", " ")})` : ""}.
            Fields below are read-only — pick a different date to enter new data.
          </span>
        </div>
      )}
      ...
      {isAnimalWise ? (
        <div className="flex flex-col gap-4">
          ...
          {dataEntryProgress.length > 0 && (
            <div className="w-full rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] p-5 overflow-x-auto shadow-2xs">
              <div className="relative" style={{ minWidth: `${Math.max(780, dataEntryProgress.length * 108)}px` }}>
                ...
                <div className="flex items-start justify-between relative z-10">
                  {dataEntryProgress.map((p) => {
                    ...
                    const stageLocked = dataEntryStages.find((s) => s.stage_id === p.stage_id)?.lock_status === "LOCKED";
                    return (
                      <div
                        key={p.stage_id}
                        role="button"
                      ...
                      >
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); if (hasAnimals) setAnimalListStageId(p.stage_id); }}
                          ...
                          title={hasAnimals ? "View animal list" : undefined}
                        >
                        ...
                        <div className="mt-2.5 text-center flex flex-col items-center w-full px-0.5">
                          <p
                            className={`text-xs tracking-tight truncate w-full ${
                              selected ? "text-[var(--accent)] font-bold" : stageLocked ? "text-[var(--text-primary)] font-semibold" : "text-[var(--text-secondary)] font-medium"
                            }`}
                            title={p.stage_name}
                          >
                            {p.stage_code}
                          ...
                          <div className="h-5 mt-1.5 flex items-center justify-center">
                            {stageLocked ? (
                              <span className="px-1.5 py-0.5 rounded-full bg-[var(--success-muted)] text-[var(--success)] border border-[var(--success)]/20 text-[9px] font-semibold">POSTED</span>
                            ) : hasAnimals ? (
                              <span className="px-2 py-0.5 rounded-full bg-[var(--accent)] text-white text-[9px] font-bold uppercase tracking-wider shadow-2xs">ACTIVE</span>
                            ) : (
                              <span className="text-[9px]" style={S.muted}>—</span>
                            )}
                          </div>
                        </div>
                      </div>
          ...
          {!dataEntryLoading && !selectedStageId ? (
            <InlineAlert variant="info">{t("blNoParamsScheduled")}</InlineAlert>
          ) : selectedProgress && !selectedStage ? (
            <InlineAlert variant="info">No animals are currently in {selectedProgress.stage_code}.</InlineAlert>
          ) : selectedStage ? (
            <div className="rounded-lg border p-3" style={S.surface}>
              <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
                <span className="font-semibold" style={S.primary}>{stageActivityLabel(selectedStage)}</span>
                <span className="text-[11px]" style={S.muted}>
                  {selectedStage.animal_count} animal(s) · Day {selectedStage.day_of_stage ?? "—"} of stage
                </span>
              </div>

              {stageActionError && <div className="mb-2"><InlineAlert>{stageActionError}</InlineAlert></div>}

              <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-lg border px-3 py-2" style={S.surface}>
                {selectedStage.lock_status === "LOCKED" ? (
                  <>
                    <div className="flex flex-col gap-0.5">
                      <span className="text-xs font-semibold" style={{ color: "var(--success, #16a34a)" }}>✓ POSTED — 🔒 LOCKED</span>
                      <span className="text-[11px]" style={S.muted}>
                        Posted {selectedStage.locked_at ? new Date(selectedStage.locked_at).toLocaleString() : ""}
                      </span>
                    </div>
                    {!reopenBoxOpen ? (
                      <button type="button" onClick={() => setReopenBoxOpen(true)} className="rounded-lg border px-3 py-1.5 text-xs font-semibold" style={S.surface}>
                        Request Reopen
                      </button>
                    ) : (
                      <div className="flex w-full flex-col gap-1.5 sm:w-auto sm:flex-row sm:items-center">
                        ...
                        <div className="flex gap-1.5">
                          <button
                            ...
                            disabled={stageActionBusy || !reopenReason.trim()}
                            className="rounded-lg px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50"
                            style={{ backgroundColor: "var(--danger)" }}
                          >
                            {stageActionBusy ? t("blSaving") : "Confirm Reopen"}
                          </button>
                          <button type="button" onClick={() => setReopenBoxOpen(false)} className="rounded-lg border px-3 py-1.5 text-xs font-semibold" style={S.surface}>
                            {t("blCancel")}
                          </button>
                        </div>
                      </div>
                  ...
                  <>
                    <div className="flex flex-col gap-0.5">
                      {selectedStage.lock_status === "REOPENED" ? (
                        <>
                          <span className="text-xs font-semibold" style={{ color: "var(--danger)" }}>REOPENED for correction</span>
                          <span className="text-[11px]" style={S.muted}>Reason: {selectedStage.reopen_reason}</span>
                        </>
                      ) : (
                        <span className="text-xs" style={S.muted}>Not yet posted for this date.</span>
                      )}
                    </div>
                  </>
                )}
              ...
              {entryScope === "ALL" ? (() => {
                ...
                return (
                  <div>
                    {selectedStage.lock_status !== "LOCKED" && stageAnimals.length > 0 && (
                      <div className="mb-2 flex items-center justify-end">
                        <button
                          type="button"
                          onClick={() => { setBulkStageTransitionError(""); setBulkStageTransitionOpen(true); }}
                    ...
                    {renderActivityBoxes(templateAnimal.lines || [], "__ALL__", selectedStage.lock_status === "LOCKED", true)}
                  </div>
                );
              })() : (() => {
                const animal = (selectedStage.animals || []).find((a: Row) => a.animal_id === entryScope);
                if (!animal) return null;
                const rosterAnimal = batchAnimalRoster.find((a) => a.animal_id === animal.animal_id);
                return (
                  <div>
                    <div className="mb-2 flex items-center justify-between gap-2">
                      <p className="font-mono text-[11px] font-semibold" style={S.sub}>{animal.animal_code}</p>
                      {rosterAnimal && selectedStage.lock_status !== "LOCKED" && (
                        <button
                          type="button"
                          onClick={() => setStageTransitionAnimal(rosterAnimal)}
                          className="text-[10px] font-semibold hover:underline"
                          style={S.accent}
                        >
                          Change Stage
                    ...
                    {renderActivityBoxes(animal.lines || [], animal.animal_id, selectedStage.lock_status === "LOCKED")}
                  </div>
                );
              })()}
            </div>
        ...
        <InlineAlert variant="info">No scheduler is linked to this batch&apos;s current stage — activities will appear here once one is created in the Scheduler module.</InlineAlert>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-2xs">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="nf-text-label mb-1 block text-[var(--text-muted)]">{t("bdeAvgWeightKg")}</label>
              <input
                type="number"
                step="0.1"
                value={avgWeight}
            ...
            <div>
              <label className="nf-text-label mb-1 block text-[var(--text-muted)]">{t("bdeAdg")}</label>
              <input
                type="number"
                step="0.01"
                value={weightGain}
            ...
            <div>
              <label className="nf-text-label mb-1 block text-[var(--text-muted)]">{t("bdeBcsRange")}</label>
              <input
                type="text"
                value={bcsScore}
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            />
            <Button size="sm" onClick={handleSaveWeightSample} disabled={savingWeight || avgWeight <= 0} className="nf-btn-primary text-xs h-8 gap-1.5">
              {savingWeight ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />}
              {t("blSave")}
            </Button>
          </div>
        </div>
        ...
        <div className="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-2xs">
          ...
          <div className="mt-2 flex justify-end">
            <Button size="sm" onClick={handleSaveNotes} disabled={savingNotes || !generalNotes.trim()} className="nf-btn-primary text-xs h-8 gap-1.5">
      ...
      {animalListStageId && (
        ...
        >
          {batchAnimalRosterLoading ? (
            <div className="flex items-center justify-center py-6"><Loader2 className="h-5 w-5 animate-spin" style={S.accent} /></div>
          ) : animalListRows.length === 0 ? (
            <p className="text-xs py-4 text-center" style={S.muted}>No animals found for this stage.</p>
          ) : (
            <div className="max-h-96 overflow-auto rounded-[var(--radius-sm)] border" style={S.surface}>
              <table className="w-full border-collapse text-left text-xs">
                ...
                <TableBody>
                  {animalListRows.map((a) => (
                    <TableRow key={a.animal_id}>
                      ...
                      <TableCell className="px-3 py-1.5" style={S.sub}>{a.gender === "F" ? "Female" : "Male"}</TableCell>
                      <TableCell className="px-3 py-1.5" style={S.sub}>{a.breed_name || "—"}</TableCell>
      ...
      <AnimalStageTransitionModal
        ...
        onSuccess={() => { loadDataEntry(); setBatchAnimalRoster((prev) => [...prev]); }}
      ...
      {bulkStageTransitionOpen && selectedStage && (
        <Dialog
          ...
          footer={
            <>
              <Button variant="outline" size="sm" onClick={() => setBulkStageTransitionOpen(false)}>{t("blCancel")}</Button>
              <Button
                size="sm"
                onClick={() => handleBulkStageTransition((selectedStage.animals || []).map((a: Row) => a.animal_id))}
                disabled={bulkStageTransitionSaving || !bulkStageTransitionForm.to_stage_id}
                className="nf-btn-primary"
              >
                {bulkStageTransitionSaving ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : "Move"}
              </Button>
            </>
          }
        >
          <div className="space-y-3 text-xs pt-1">
            {bulkStageTransitionError && <InlineAlert>{bulkStageTransitionError}</InlineAlert>}
            <div>
              <label className="font-semibold block mb-1">Destination Stage</label>
              <select
                value={bulkStageTransitionForm.to_stage_id}
                onChange={(e) => setBulkStageTransitionForm((f) => ({ ...f, to_stage_id: e.target.value }))}
                className={`${inputCls} nf-select w-full`}
              >
                <option value="">Select a stage…</option>
                {stageMasterList.map((s: Row) => <option key={s.stage_id} value={s.stage_id}>{s.stage_code} — {s.stage_name}</option>)}
              </select>
            </div>
            <div>
              <label className="font-semibold block mb-1">Destination Location (optional)</label>
              <select
                value={bulkStageTransitionForm.to_location_id}
                onChange={(e) => setBulkStageTransitionForm((f) => ({ ...f, to_location_id: e.target.value }))}
                className={`${inputCls} nf-select w-full`}
              >
                <option value="">Keep current location</option>
                {locations.map((l: Row) => <option key={l.location_id} value={l.location_id}>{l.location_name}</option>)}
              </select>
            </div>
            <div>
              <label className="font-semibold block mb-1">Reason (required to override a minimum-duration hold)</label>
              <input
                type="text"
                value={bulkStageTransitionForm.reason}
                onChange={(e) => setBulkStageTransitionForm((f) => ({ ...f, reason: e.target.value }))}
      ...
      {changeStageOpen && (
        <Dialog
          ...
          footer={
            <>
              <Button variant="outline" size="sm" onClick={() => setChangeStageOpen(false)}>{t("blCancel")}</Button>
              <Button size="sm" onClick={handleChangeStage} disabled={changeStageSaving || changeStageOptions.length === 0 || !changeStageForm.to_stage_code} className="nf-btn-primary">
                {changeStageSaving ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : t("blTransferStage")}
              </Button>
            </>
          }
        >
          <div className="space-y-3 text-xs pt-1">
            ...
            <div>
              ...
              {changeStageOptionsLoading ? (
                <div className="flex items-center gap-2 text-[var(--text-muted)]"><Loader2 className="h-3.5 w-3.5 animate-spin" /> Loading stages…</div>
              ) : (
                <select
                  value={changeStageForm.to_stage_code}
                  onChange={(e) => setChangeStageForm((f) => ({ ...f, to_stage_code: e.target.value }))}
                  className={`${inputCls} nf-select w-full`}
                >
                  <option value="">Select a stage…</option>
                  {changeStageOptions.map((code) => <option key={code} value={code}>{code}</option>)}
                </select>
              )}
            </div>
            <div>
              <label className="font-semibold block mb-1">Remarks (required to override a minimum-duration hold)</label>
              <input
                type="text"
                value={changeStageForm.remarks}
                onChange={(e) => setChangeStageForm((f) => ({ ...f, remarks: e.target.value }))}
                className="nf-input w-full"
              />
            </div>
          </div>
