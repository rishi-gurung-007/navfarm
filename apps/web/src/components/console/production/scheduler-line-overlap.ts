...
export interface LookupOptions {
  items?:
    Array<{ item_id?: string; item_name?: string; [key: string]: any }> | any[];
  resources?:
    | Array<{
        resource_id?: string;
...
export function doLinesTargetSameSubject(
  ...
  if (type === 'DESCRIPTIVE') {
    if (a.kpi_metric && b.kpi_metric) {
      return (
        a.kpi_metric.trim().toLowerCase() === b.kpi_metric.trim().toLowerCase()
      );
    }
    return (
      !!a.activity_name &&
...
export function findConflictingSchedulerLine(
  ...
  for (const existing of existingLines) {
    ...
    if (candidate.line_type === 'CONSUMPTION') {
      const matchedItem =
        candidate.item_id && lookups?.items
    ...
    if (candidate.line_type === 'RESOURCE') {
      const matchedRes =
        candidate.resource_id && lookups?.resources
          ? lookups.resources.find(
              (r) => r.resource_id === candidate.resource_id,
            )
          : null;
      subjectName =
        matchedRes?.resource_name ||
    ...
    if (candidate.line_type === 'DESCRIPTIVE') {
      subjectName =
        candidate.kpi_metric || candidate.activity_name || 'This metric';
      return {
        conflictingLine: existing,
