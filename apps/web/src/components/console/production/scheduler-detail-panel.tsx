...
export default function SchedulerDetailPanel({
  schedulerId,
  onChanged,
  items: itemsProp,
  ...
  const load = () => {
    ...
    setError('');
    api
      .get(`/scheduler-header/${schedulerId}`)
      .then((r) => setHeader(unwrap<Row>(r)))
      .catch((err: any) =>
        setError(err?.message || 'Could not load the scheduler.'),
      )
  ...
  useEffect(() => {
    ...
    if (!resourcesProp)
      api
        ...
        .catch(() => setResources([]));
    api
      .get(`/activity?isActive=true&limit=500`)
      .then((r) => setActivities(unwrap<Row[]>(r) || []))
      .catch(() => setActivities([]));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [header?.scheduler_id]);

  const itemLabel = (id: string) =>
    items.find((i) => i.item_id === id)?.item_name || id;
  const resourceLabel = (id: string) =>
    resources.find((r) => r.resource_id === id)?.resource_name || id;
  ...
  const runStatusChange = async (status: string) => {
    ...
    setError('');
    try {
      await api.put(`/scheduler-header/${header.scheduler_id}/status`, {
  ...
  const openEditLine = (line: Row) => {
    ...
    setLineForm({
      ...emptyLine(),
      ...line,
      custom_days: (line.custom_days || [])
        .map((d: Row) => d.day_number)
        .join(', '),
  ...
  const saveLine = async () => {
    ...
    const otherLines = (header.lines || []).filter(
      (l: Row) => l.line_id !== editingLineId,
    );
    const conflict = findConflictingSchedulerLine(lineForm, otherLines, {
      items,
      resources,
    });
    if (conflict) {
    ...
    try {
      ...
      const payload: Row = {
        ...
        day_of_week:
          lineForm.day_of_week === '' ? null : Number(lineForm.day_of_week),
      ...
      if (['CONSUMPTION', 'OUTPUT', 'TRANSFER'].includes(lineForm.line_type)) {
        ...
        payload.standard_qty =
          lineForm.standard_qty === '' ? null : Number(lineForm.standard_qty);
      ...
      if (editingLineId) {
        await api.put(
          `/scheduler-header/${header.scheduler_id}/lines/${editingLineId}`,
          payload,
        );
      } else {
        await api.post(
          `/scheduler-header/${header.scheduler_id}/lines`,
          payload,
        );
      }
      setAddingLine(false);
  ...
  const deactivateLine = async (line: Row) => {
    ...
    try {
      await api.delete(
        `/scheduler-header/${header.scheduler_id}/lines/${line.line_id}`,
      );
      load();
      onChanged?.();
    } catch (err: any) {
  ...
  if (loading && !header) {
    return (
      <div className="flex justify-center py-8">
        <Loader2 className="h-5 w-5 animate-spin" style={S.muted} />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4 text-xs">
      ...
      <div
        className="grid grid-cols-2 gap-3 rounded-[var(--radius-md)] border p-3.5 shadow-2xs sm:grid-cols-4"
        style={S.raised}
      >
        <div>
          <p
          ...
          <StatusBadge status={header.scheduler_status} className="mt-1" />
        </div>
        <div>
          <p
            className="text-[10px] font-bold uppercase tracking-wider"
        ...
        <div>
          <p
            ...
            style={S.muted}
          >
            {t('scLabelDataEntryLevel')}
          </p>
          <p className="font-semibold mt-0.5" style={S.primary}>
            {header.data_entry_level || '—'}
          </p>
        </div>
        <div>
          <p
        ...
        <div>
          <p
            ...
            style={S.muted}
          >
            {t('scLabelAutoGenerated')}
          </p>
          <p className="font-semibold mt-0.5" style={S.primary}>
            {header.auto_generated ? t('yes') : t('no')}
          </p>
        </div>
                ...
                {header.silo_stock.map((s: Row) => (
                  <TableRow key={s.item_id}>
                    <TableCell
                      ...
                      style={S.primary}
                    >
                      {s.item_description || s.item_code}
                    </TableCell>
            ...
            {(header.lines || []).length === 0 ? (
              ...
              (header.lines || []).map((line: Row) => (
                <TableRow
                  key={line.line_id}
                  style={
                    line.is_active === false ? { opacity: 0.5 } : undefined
                  }
                >
                  ...
                  >
                    {line.activity_name}
                    {line.is_mandatory && (
                      <span className="ml-1 text-[10px]" style={S.muted}>
                        *
                      </span>
                    )}
                  </TableCell>
                  <TableCell className="px-3 py-2" style={S.sub}>
                    {line.occurrence}
                    {line.occurrence === 'WEEKLY' && line.day_of_week
                      ? ` (${line.day_of_week})`
                      : ''}
                  </TableCell>
                  <TableCell className="px-3 py-2" style={S.sub}>
                    {line.item_id ? (
                      ...
                      line.resource_name || resourceLabel(line.resource_id)
                    ) : (
                      line.kpi_metric || '—'
                    )}
                  </TableCell>
                  <TableCell
                    ...
                    style={S.primary}
                  >
                    {line.standard_qty ?? line.std_value ?? '—'}{' '}
                    {line.qty_basis
                      ? `(${line.qty_basis})`
                      : line.output_basis
                        ? `(${line.output_basis})`
      ...
      {addingLine && (
        ...
        >
          <p
            ...
            style={S.sub}
          >
            {editingLineId ? t('scEditLineTitle') : t('scAddLineTitle')}
          </p>
          {lineError && (
            <div className="mb-2">
              <InlineAlert>{lineError}</InlineAlert>
            </div>
          )}
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            ...
            <select
              ...
              onChange={(e) =>
                setLineForm((f: Row) => ({ ...f, occurrence: e.target.value }))
              }
              className={`${inputCls} nf-select`}
              style={S.input}
            >
            ...
            <input
              type="number"
              min={1}
              placeholder={t('scPlaceholderStartDay')}
              value={lineForm.start_day}
              onChange={(e) =>
                setLineForm((f: Row) => ({ ...f, start_day: e.target.value }))
              }
              className={inputCls}
              style={S.input}
            />
            <input
              type="number"
              min={1}
              placeholder={t('scPlaceholderEndDay')}
            ...
            {lineForm.occurrence === 'WEEKLY' && (
              <input
                ...
                min={1}
                max={7}
                placeholder={t('scPlaceholderDayOfWeek')}
            ...
            {lineForm.occurrence === 'CUSTOM' && (
              <input
                placeholder={t('scPlaceholderCustomDays')}
            ...
            {['CONSUMPTION', 'OUTPUT', 'TRANSFER'].includes(lineType) && (
              <>
                ...
                >
                  ...
                  {items.map((i) => (
                    <option key={i.item_id} value={i.item_id}>
                      {i.item_name} {i.uom_primary ? `(${i.uom_primary})` : ''}
                    </option>
                  ))}
                </select>
                <input
                  placeholder={t('scPlaceholderItemDescription')}
                  value={lineForm.item_description}
                  onChange={(e) =>
                    setLineForm((f: Row) => ({
                      ...f,
                ...
                <input
                  type="number"
                  step="any"
                  placeholder={t('scPlaceholderStandardQty')}
            ...
            {lineType === 'DESCRIPTIVE' && (
              <>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setLineForm((f: Row) => ({ ...f, kpi_uom: e.target.value }))
                  }
                  className={inputCls}
                  style={S.input}
                />
                ...
                <input
                  type="number"
                  step="any"
                  placeholder={t('scPlaceholderLowerLimit')}
                ...
                <input
                  type="number"
                  step="any"
                  placeholder={t('scPlaceholderUpperLimit')}
            ...
            {lineType === 'OVERHEAD' && (
              <input
                placeholder={t('scPlaceholderOverheadCategory')}
          ...
          <div className="mt-3 flex gap-2">
            <Button
              size="sm"
              onClick={saveLine}
              disabled={savingLine || !lineForm.activity_name}
              className="nf-btn-primary"
            >
              {savingLine ? t('scSaving') : t('scSaveLine')}
            </Button>
            <Button
              size="sm"
              variant="outline"
