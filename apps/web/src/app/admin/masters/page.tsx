...
export default function AdminMastersPage() {
  ...
  const [nobForm, setNobForm] = useState({
    nob_code: '',
    nob_name: '',
    description: '',
  ...
  useEffect(() => {
    ...
    if (!token || !user || user.userType !== 'SYSTEM_ADMIN') {
      router.replace('/');
      return;
    }
    loadAll();
  ...
  const loadLobs = async (nobId: string) => {
    ...
    try {
      ...
      setLoadingLobs((p) => ({ ...p, [nobId]: false }));
    }
  };

  const handleExpandNob = (nobId: string) => {
    if (expandedNob === nobId) {
      setExpandedNob(null);
      return;
    }
  ...
  const handleSaveNob = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to create NOB.');
    } finally {
      setSavingNob(false);
    }
  };
  ...
  const handleSaveLob = async (e: React.FormEvent, nobId: string) => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to create LOB.');
    } finally {
      setSavingLob(false);
    }
  };
  ...
  const loadStates = async (countryId: string) => {
    ...
    try {
      ...
      setLoadingStates((p) => ({ ...p, [countryId]: false }));
    }
  };
  ...
  const handleSaveCostingMethod = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to create costing method.');
    } finally {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 px-4 pb-4 sm:px-6 sm:pb-6 xl:px-8 xl:pb-8">
      <PageHeader title={t('masterData')} description={t('admMastersDesc')} />
      ...
      <div>
        <div className="flex overflow-x-auto border-b" style={S.border}>
          ...
          {tabBtn(
            'costingMethods',
            `Costing Methods (${costingMethods.length})`,
          )}
        </div>
        ...
        <div className="pt-6">
          ...
          {activeTab === 'nobs' && (
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={() => setShowNobForm(!showNobForm)}
              ...
              <Dialog
                open={showNobForm}
                onClose={() => setShowNobForm(false)}
                title={t('admAddNobTitle')}
                description={t('admAddNobDesc')}
                maxWidth="md"
              >
                <form
                  onSubmit={handleSaveNob}
                ...
                >
                  {[
                    ...
                    { k: 'nob_name', l: 'NOB Name', p: 'Poultry Farming' },
                    {
                      k: 'description',
                      l: 'Description',
                      p: 'Broiler & layer farming',
                    },
                  ].map(({ k, l, p }) => (
                    <div
                      key={k}
                      className={k === 'description' ? 'col-span-2' : ''}
                    >
                      <Field label={l}>
                        <input
                          required
                          value={(nobForm as any)[k]}
                          onChange={(e) =>
                            setNobForm({ ...nobForm, [k]: e.target.value })
                          }
                          placeholder={p}
                          className={inputCls}
                          style={S.input}
                  ...
                  <Field label={t('blLabelCostingMethod')}>
                    <select
                      value={nobForm.default_costing_method}
                      onChange={(e) =>
                        setNobForm({
                          ...nobForm,
                          default_costing_method: e.target.value,
                        })
                      }
              ...
              <div className="border-t" style={S.border}>
                {nobs.length === 0 && (
                  <p className="text-sm text-center py-8" style={S.muted}>
                    {t('admNoNobs')}
                  </p>
                )}
                {nobs.map((nob) => {
                    ...
                    <div key={nob.nob_id} className="border-b" style={S.border}>
                      ...
                      {isExp && (
                        ...
                        >
                          ...
                          <Dialog
                            open={showLobForm === nob.nob_id}
                            onClose={() => setShowLobForm(null)}
                            title={t('admAddLobTitle')}
                            description={`Add an operating line beneath ${nob.nob_name}.`}
                            maxWidth="md"
                          >
                            <form
                              onSubmit={(e) => handleSaveLob(e, nob.nob_id)}
                              className="grid grid-cols-2 gap-4"
                            >
                              {[
                                { k: 'lob_code', l: 'LOB Code', p: 'BROILER' },
                                {
                                  k: 'lob_name',
                                  l: 'LOB Name',
                                  p: 'Broiler Production',
                                },
                              ].map(({ k, l, p }) => (
                                <Field key={k} label={l}>
                                  <input
                                    required
                                    value={(lobForm as any)[k]}
                                    onChange={(e) =>
                                      setLobForm({
                                        ...lobForm,
                                        [k]: e.target.value,
          ...
          {activeTab === 'currencies' && (
            <div className="space-y-4">
              ...
              <Dialog
                open={showCurrForm}
                onClose={() => setShowCurrForm(false)}
                title={t('admAddCurrencyTitle')}
                description={t('admAddCurrencyDesc')}
                maxWidth="md"
              >
                <form
                ...
                >
                  {[
                    ...
                    <Field key={k} label={l}>
                      <input
                        required
                        value={(currForm as any)[k]}
                        onChange={(e) =>
                          setCurrForm({ ...currForm, [k]: e.target.value })
                        }
                        placeholder={p}
                        className={inputCls}
                        style={S.input}
                  ...
                  <Field label={t('admSymbolPosition')}>
                    <select
                      value={currForm.symbol_position}
                      onChange={(e) =>
                        setCurrForm({
                          ...currForm,
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                ...
                {currencies.map((curr) => (
                  ...
                  >
                    ...
                    <div className="text-xs font-semibold" style={S.primary}>
                      {curr.iso_code}
                    </div>
                    <div
                    ...
                    {curr.is_system_default && (
                      <span
                        className="mt-1.5 inline-block text-[10px] font-semibold px-2 py-0.5 rounded-[var(--radius-xs)]"
                        style={{
          ...
          {activeTab === 'languages' && (
            <div className="space-y-4">
              ...
              <Dialog
                open={showLangForm}
                onClose={() => setShowLangForm(false)}
                title={t('admAddLanguageTitle')}
                description={t('admAddLanguageDesc')}
                maxWidth="md"
              >
                <form
                ...
                >
                  {[
                    ...
                    <Field key={k} label={l}>
                      <input
                        required
                        value={(langForm as any)[k]}
                        onChange={(e) =>
                          setLangForm({ ...langForm, [k]: e.target.value })
                        }
                        placeholder={p}
                        className={inputCls}
                        style={S.input}
                  ...
                  <Field label={t('admScriptDirection')}>
                    <select
                      value={langForm.script}
                      onChange={(e) =>
                        setLangForm({ ...langForm, script: e.target.value })
                      }
                      className={`${inputCls} nf-select`}
                      style={S.input}
                    >
                  ...
                  <TableBody>
                    ...
                    {languages.map((lang) => (
                      <TableRow key={lang.lang_code}>
                        ...
                        <TableCell>
                          <span
                            className="text-[11px] font-semibold border rounded-[var(--radius-xs)] px-2 py-0.5"
                            style={S.raised}
                          >
                            {lang.script}
                          </span>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <p
                  ...
                  style={S.muted}
                >
                  {t('admSupportedTimezones')}
                </p>
                <button
                  onClick={() => setShowTzForm(!showTzForm)}
                  className="flex items-center gap-1.5 text-xs font-semibold"
                  style={S.accent}
                >
                  <Plus className="w-3.5 h-3.5" />
              ...
              <Dialog
                open={showTzForm}
                onClose={() => setShowTzForm(false)}
                title={t('admAddTimezoneTitle')}
                description={t('admAddTimezoneDesc')}
                maxWidth="md"
              >
                <form
                ...
                >
                  <Field label={t('admIanaCode')}>
                    <input
                      required
                      value={tzForm.tz_code}
                      onChange={(e) =>
                        setTzForm({ ...tzForm, tz_code: e.target.value })
                      }
                  ...
                  <Field label={t('wzDisplayName')}>
                    <input
                      required
                      value={tzForm.tz_name}
                      onChange={(e) =>
                        setTzForm({ ...tzForm, tz_name: e.target.value })
                      }
                      placeholder={t('admPhIndiaStandardTime')}
                      className={inputCls}
                      style={S.input}
                    />
                  </Field>
                  <Field label={t('admUtcOffset')}>
                    <input
                      required
                      value={tzForm.utc_offset}
                      onChange={(e) =>
                        setTzForm({ ...tzForm, utc_offset: e.target.value })
                      }
                      placeholder="+05:30"
                      className={inputCls}
                      style={S.input}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                ...
                {timezones.map((tz) => (
                  ...
                  >
                    ...
                    <div className="text-xs font-mono" style={S.accent}>
                      {tz.tz_code}
                    </div>
                    <div
                      className="text-[10px] mt-0.5 truncate"
          ...
          {activeTab === 'countries' && (
            <div className="space-y-4">
              ...
              <Dialog
                open={showCountryForm}
                onClose={() => setShowCountryForm(false)}
                title={t('admAddCountryTitle')}
                description={t('admAddCountryDesc')}
                maxWidth="md"
              >
                <form
                ...
                >
                  {[
                    ...
                    <Field key={k} label={l}>
                      <input
                        required={k !== 'phone_code' && k !== 'flag_emoji'}
                        value={(countryForm as any)[k]}
                        onChange={(e) =>
                          setCountryForm({
                            ...countryForm,
                            [k]: e.target.value,
                        ...
                        >
                          <div className="flex items-center justify-between mb-3">
                            <p
                              ...
                              style={S.muted}
                            >
                              {t('admStatesProvinces')}
                            </p>
                            <button
                              onClick={() =>
                                setShowStateForm(
                                  showStateForm === country.country_id
                                    ? null
                                    : country.country_id,
                                )
                              }
                          ...
                          <Dialog
                            open={showStateForm === country.country_id}
                            onClose={() => setShowStateForm(null)}
                            title={t('admAddStateTitle')}
                            description={`Add a state/province beneath ${country.country_name}.`}
                            maxWidth="md"
                          >
                            <form
                              onSubmit={(e) =>
                                handleSaveState(e, country.country_id)
                              }
                              className="grid grid-cols-2 gap-4"
                            >
                              ...
                              <Field label={t('admStateName')}>
                                <input
                                  ...
                                  placeholder={t('admPhMaharashtra')}
                                  className={inputCls}
                                  style={S.input}
                                />
                              </Field>
                          ...
                          ) : (states[country.country_id] || []).length ===
                            0 ? (
                            <p className="text-xs" style={S.muted}>
                              {t('admNoStates')}
                            </p>
          ...
          {activeTab === 'costingMethods' && (
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <button
                  onClick={() => setShowCostingForm(!showCostingForm)}
                  className="flex items-center gap-1.5 text-xs font-semibold"
                  style={S.accent}
                >
                  <Plus className="w-3.5 h-3.5" />
                  {t('admAddCostingMethod')}
                </button>
              </div>

              <Dialog
                open={showCostingForm}
                onClose={() => setShowCostingForm(false)}
                title={t('admAddCostingTitle')}
                description={t('admAddCostingDesc')}
                maxWidth="md"
              >
                <form
                  onSubmit={handleSaveCostingMethod}
                  className="grid grid-cols-2 gap-4"
                >
                  <Field label={t('admMethodCode')}>
                    <input
                      required
                      value={costingForm.method_code}
                      onChange={(e) =>
                        setCostingForm({
                          ...costingForm,
                          method_code: e.target.value.toUpperCase(),
                        })
                      }
                      placeholder="WEIGHTED_AVG"
                      className={inputCls}
                      style={S.input}
                    />
                  </Field>
                  <Field label={t('admMethodName')}>
                    <input
                      ...
                      placeholder={t('admPhWeightedAverage')}
                      className={inputCls}
                      style={S.input}
                    />
                  </Field>
                  <Field label={t('admAutoPostVariance')}>
                    <select
                      value={costingForm.variance_auto}
                      onChange={(e) =>
                        setCostingForm({
                          ...costingForm,
                          variance_auto: e.target.value,
                        })
                      }
                      className={`${inputCls} nf-select`}
                      style={S.input}
                  ...
                  <div className="col-span-2 flex flex-col-reverse gap-3 border-t border-(--border) pt-5 sm:flex-row sm:justify-end">
                    ...
                    <button
                      type="button"
                      onClick={() => setShowCostingForm(false)}
                      className="h-11 rounded-[var(--radius-sm)] border border-(--border) bg-(--surface) px-5 text-sm text-(--text-secondary) hover:bg-(--surface-raised)"
                    >
                      {t('cancel')}
                    </button>
