...
export default function ConsoleLayout({
  ...
  useEffect(() => {
    ...
    if (storedUser.userType === 'SYSTEM_ADMIN') {
    ...
    if (initialActiveId && initialActiveId !== homeId) {
      const patched = updateStoredUser({
        companyId: initialActiveId,
        company_id: initialActiveId,
      });
  ...
  const checkOnboardingStatus = async (
    ...
    if (
      storedUser.userType === 'STANDARD_USER' ||
      storedUser.userType === 'OPERATIONAL_ADMIN'
    ) {
      setIsOnboarded(true);
      setCheckingOnboard(false);
      setReady(true);
    ...
    try {
      ...
      if (filtered.length === 0) {
        ...
        if (comp.onboarding_status === 'COMPLETED') {
          ...
          const steps = await api.get(
            `/setup/wizard/status/${comp.company_id}`,
          );
          setWizardSteps(steps);
          const firstPending = steps.find(
            (s: any) => s.status !== 'COMPLETED' && s.isMandatory,
          );
      ...
      const staleSession =
        status === 401 ||
        message.includes('Tenant connection context') ||
  ...
  if (!isOnboarded) {
    return (
      ...
      >
        <header
          className="h-14 flex items-center px-4 sm:px-6 shrink-0 border-b"
          style={{
        ...
        >
          ...
          <span
            className="ml-2 hidden text-xs font-semibold uppercase tracking-widest px-2 py-0.5 rounded-[var(--radius-xs)] sm:inline-flex"
            style={{
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button
              ...
              className="text-sm flex h-10 items-center gap-1.5 rounded-[var(--radius-sm)] px-2 sm:px-3"
              style={{ color: 'var(--text-secondary)' }}
            >
  ...
  if (activeScope === 'TENANT') {
    ...
    navItems = [
      {
        label: t('lobDashboard', { lob: tLob(activeLob) }),
        href: '/dashboard',
        icon: LayoutDashboard,
      },
      {
      ...
      <ContextNavProvider>
        {(contextNav) => (
          <AppShell
            ...
            profileItems={PROFILE_ITEMS.map((key) => ({
              label: t(key),
              href:
                key === 'account' ? '/account/profile' : '/account/settings',
            }))}
