...
const categoryField = itemConfig.fields.find(
  (field) => field.key === 'category_id',
)!;
const subCategoryField = itemConfig.fields.find(
  (field) => field.key === 'sub_category',
)!;
...
const creepFeed = {
  category_id: 'cat-creep',
  category_code: 'CREEP_FEED',
  category_name: 'Creep Feed',
};
...
describe('Item classification cascade', () => {
  ...
  it('hides Sub Category until a Category is chosen', () => {
    expect(
      parentFilterState(subCategoryField, { item_type: 'FEED' }, {}),
  ...
  it('offers Sub Category once its parent Category has children', () => {
    expect(
      parentFilterState(
        subCategoryField,
        { item_type: 'FEED', category_id: 'cat-feed' },
        { [CHILDREN_OF_FEED]: [creepFeed] },
      ),
    ).toEqual(expect.objectContaining({ hidden: false, empty: false }));
  });
  ...
  it('leaves a field with no requiresParent alone', () => {
    const uomPrimary = itemConfig.fields.find(
      (field) => field.key === 'uom_primary',
    )!;
    expect(parentFilterState(uomPrimary, {}, {})).toEqual({
      hidden: false,
      empty: false,
    });
  });
