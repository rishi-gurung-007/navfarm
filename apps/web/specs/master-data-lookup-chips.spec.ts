...
const chipKeys = (key: string) =>
  lookupMastersFor(config(key)).map((c) => c.key);
...
describe('master-data lookup chips', () => {
  ...
  it("orders a master's lookups by field position, never by registry position", () => {
    for (const c of MASTER_DATA_CONFIGS) {
      ...
      const registry = MASTER_DATA_CONFIGS.map((x) => x.key).filter((k) =>
        chips.includes(k),
      );
      ...
      fields: [
        {
          key: 'f',
          label: 'F',
          type: 'select-entity' as const,
          entityEndpoint: uomConversion.apiBase,
        },
      ],
    };
    expect(lookupMastersFor(probe).map((c) => c.key)).toEqual([
      'uom-conversion',
    ]);
  });
