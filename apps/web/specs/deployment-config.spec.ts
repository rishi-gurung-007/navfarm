...
describe('Windows production deployment configuration', () => {
  ...
  afterEach(() => {
    ...
    if (originalUpstream === undefined)
      delete process.env.NAVFARM_API_UPSTREAM_URL;
  ...
  it('rejects credentials or path data in the private upstream URL', async () => {
    process.env.NAVFARM_API_MODE = 'proxy';
    process.env.NAVFARM_API_UPSTREAM_URL =
      'http://user:secret@127.0.0.1:2877/private';

    const config = createNextConfig('phase-production-build');

    await expect(config.rewrites()).rejects.toThrow(
      /must not contain credentials|must be an origin/,
    );
  });
  ...
  it('keeps the web production port explicit in its Nx target', () => {
    ...
    expect(webPackage.nx.targets.start.options.command).toBe(
      'next start --hostname 0.0.0.0 --port 3002',
    );
  });
});
