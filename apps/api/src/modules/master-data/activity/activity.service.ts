...
export class ActivityService {
  ...
  async findAll(query: QueryActivityDto, tenantId: string) {
    ...
    if (query.nobId)
      conditions.push(or(eq(table.nob_id, query.nobId), isNull(table.nob_id))!);
    if (query.lobId)
      conditions.push(or(eq(table.lob_id, query.lobId), isNull(table.lob_id))!);
    ...
    if (query.search)
      conditions.push(
        or(
          like(table.activity_code, `%${query.search}%`),
          like(table.activity_name, `%${query.search}%`),
        )!,
      );
    return this.db
      .select()
  ...
  async create(dto: CreateActivityDto, tenantId: string, user?: any) {
    ...
    const [dup] = await this.db
      .select({ activity_id: table.activity_id })
      .from(table)
      .where(
        and(
          eq(table.tenant_id, tenantId),
          ...masterScopeConditions(this.cls, table, companyId || undefined),
          eq(table.activity_code, code),
        ),
      )
      .limit(1);
    if (dup)
      throw new ConflictException(
        `Activity code '${code}' already exists in this workspace.`,
      );
