...
export class CreateActivityDto {
  ...
  @Matches(/^[A-Z][A-Z0-9_]{0,49}$/, {
    message:
      'activity_code must be uppercase letters/digits/underscore, starting with a letter',
  })
  @MaxLength(50)
  ...
  @IsOptional()
  @IsIn(OCCURRENCES)
  default_occurrence?: (typeof OCCURRENCES)[number];
  @IsOptional() @IsIn(QTY_BASES) default_qty_basis?: (typeof QTY_BASES)[number];
  @IsOptional()
  @IsIn(OUTPUT_BASES)
  default_output_basis?: (typeof OUTPUT_BASES)[number];
  @IsOptional() @IsString() @MaxLength(50) default_kpi_metric?: string;
  @IsOptional()
  @IsIn(CAPTURE_PERS)
  default_capture_per?: (typeof CAPTURE_PERS)[number];
...
export class UpdateActivityDto extends PartialType(
  OmitType(CreateActivityDto, ['activity_code'] as const),
) {}
