...
export class BreedService {
  ...
  private async resolveBreedCode(
    dto: CreateBreedDto,
    tenantId: string,
    ...
    if (!seriesCode) {
      if (!dto.breed_code) {
        throw new BadRequestException(
          'Enter a breed code or configure a breed number series.',
        );
      }
    ...
    const code = await generateCompositeCode({
      parentCode: location.location_code,
      prefix: series.prefix || dto.breed_type,
      seqLength: series.seq_length,
      fetchSiblingCodes: () =>
        executor
          .select({ code: schema.breedMaster.breed_code })
    ...
    if (code.length > 255)
      throw new BadRequestException(
        'Generated breed code exceeds 255 characters. Shorten the location code or breed prefix.',
      );
    return code;
  }

  private async requireRootFarm(
    locationId: string,
    tenantId: string,
    companyId: string | null,
    executor = this.db,
    ...
    if (
      ...
      throw new BadRequestException(
        'Breed location must be an active, first-level farm without a parent in this workspace.',
      );
    }
    return location;
  }
  ...
  async createSpecies(
    ...
    const speciesCode = await this.numberSeriesService.resolveNewCode(
      'SPECIES',
      dto.species_code,
      tenantId,
      companyId,
    ...
    if (existing.length > 0) {
      throw new ConflictException(
        `Species with code '${dto.species_code}' already exists.`,
      );
    }
  ...
  async findOneSpecies(id: string) {
    const [species] = await this.db
      ...
      .where(
        and(
          eq(schema.speciesMaster.species_id, id),
          isNull(schema.speciesMaster.deleted_at),
        ),
      )
      .limit(1);
  ...
  async findAllSpecies(query: QuerySpeciesDto, tenantId: string) {
    ...
    const conditions: any[] = [eq(schema.speciesMaster.tenant_id, tenantId)];

    conditions.push(
      ...masterScopeConditions(this.cls, schema.speciesMaster, query.companyId),
    );
    ...
    conditions.push(
      ...listFilterConditions(schema.speciesMaster, query.filter),
    );
    ...
    return this.db
      ...
      .orderBy(
        listOrderBy(
          schema.speciesMaster,
          query,
          schema.speciesMaster.species_code,
        ),
      )
      .limit(limit)
      .offset(offset);
  ...
  async updateSpecies(
    ...
    if (
      dto.species_code &&
      dto.species_code.toUpperCase() !== species.species_code
    ) {
      const duplicateConditions = [
        eq(schema.speciesMaster.tenant_id, tenantId),
      ...
      if (species.company_id) {
        duplicateConditions.push(
          eq(schema.speciesMaster.company_id, species.company_id),
        );
      } else {
        duplicateConditions.push(isNull(schema.speciesMaster.company_id));
      }
    ...
    if (dto.species_code !== undefined)
      updates.species_code = dto.species_code.toUpperCase();
  ...
  async removeSpecies(id: string, tenantId: string, userPayload?: any) {
    ...
    return {
      success: true,
      message: `Species '${species.species_name}' has been soft-deleted.`,
    };
  }
  ...
  async createBreed(dto: CreateBreedDto, tenantId: string, userPayload?: any) {
    ...
    const resolvedNobLob = await this.nobLobResolution.resolve(
      tenantId,
      companyId,
      {
        nob_id: dto.nob_id,
    ...
    const newBreed = await this.db.transaction(async (tx) => {
      const breedCode = await this.resolveBreedCode(
        dto,
        tenantId,
        companyId,
        tx,
      ...
      if (existing.length > 0) {
        throw new ConflictException(
          `Breed with code '${breedCode}' already exists.`,
        );
      }
  ...
  async findAllBreeds(query: QueryBreedDto, tenantId: string) {
    // No isNull(deleted_at) filter — list view shows both Active/Inactive states (toggle switch) so a blocked row can be found again and restored.
    const conditions: any[] = [eq(schema.breedMaster.tenant_id, tenantId)];

    conditions.push(
      ...masterScopeConditions(this.cls, schema.breedMaster, query.companyId),
    );
    if (query.speciesId) {
    ...
    if (query.lobId) {
      ...
      conditions.push(
        or(
          eq(schema.breedMaster.lob_id, query.lobId),
          isNull(schema.breedMaster.lob_id),
        )!,
      );
    }
    if (query.breedType) {
    ...
    return this.db
      ...
      .where(and(...conditions))
      .orderBy(
        listOrderBy(schema.breedMaster, query, schema.breedMaster.breed_code),
      )
      .limit(limit)
      .offset(offset);
  }
  ...
  async updateBreed(
    ...
    if (dto.location_id)
      await this.requireRootFarm(dto.location_id, tenantId, breed.company_id);
    ...
    if (dto.breed_code && dto.breed_code.toUpperCase() !== breed.breed_code) {
      ...
      if (breed.company_id) {
        duplicateConditions.push(
          eq(schema.breedMaster.company_id, breed.company_id),
        );
      } else {
        duplicateConditions.push(isNull(schema.breedMaster.company_id));
      }
      ...
      if (existing.length > 0) {
        throw new ConflictException(
          `Breed with code '${dto.breed_code}' already exists.`,
        );
      }
    }
    ...
    if (dto.breed_code !== undefined)
      updates.breed_code = dto.breed_code.toUpperCase();
    ...
    if (dto.avg_growth_rate_g_day !== undefined)
      updates.avg_growth_rate_g_day =
        dto.avg_growth_rate_g_day?.toString() || null;
    if (dto.avg_fcr !== undefined)
      updates.avg_fcr = dto.avg_fcr?.toString() || null;
    if (dto.avg_mortality_pct !== undefined)
      updates.avg_mortality_pct = dto.avg_mortality_pct?.toString() || null;
    if (dto.avg_lay_rate_pct !== undefined)
      updates.avg_lay_rate_pct = dto.avg_lay_rate_pct?.toString() || null;
    if (dto.incubation_days !== undefined)
      updates.incubation_days = dto.incubation_days;
    if (dto.gestation_days !== undefined)
      updates.gestation_days = dto.gestation_days;
    if (dto.avg_litter_size !== undefined)
      updates.avg_litter_size = dto.avg_litter_size?.toString() || null;
    if (dto.mature_age_months !== undefined)
      updates.mature_age_months = dto.mature_age_months;
    if (dto.productive_life_months !== undefined)
      updates.productive_life_months = dto.productive_life_months;
    if (dto.premature_years !== undefined)
      updates.premature_years = dto.premature_years?.toString() || null;
    if (dto.avg_yield_per_unit !== undefined)
      updates.avg_yield_per_unit = dto.avg_yield_per_unit?.toString() || null;
    if (dto.lactation_days !== undefined)
      updates.lactation_days = dto.lactation_days;
    if (dto.residual_value_pct !== undefined)
      updates.residual_value_pct = dto.residual_value_pct?.toString() || null;
    if (dto.productive_life_cycles !== undefined)
      updates.productive_life_cycles = dto.productive_life_cycles;
    if (dto.avg_litter_size_born !== undefined)
      updates.avg_litter_size_born =
        dto.avg_litter_size_born?.toString() || null;
    if (dto.avg_litter_size_weaned !== undefined)
      updates.avg_litter_size_weaned =
        dto.avg_litter_size_weaned?.toString() || null;
    if (dto.avg_weaning_weight_kg !== undefined)
      updates.avg_weaning_weight_kg =
        dto.avg_weaning_weight_kg?.toString() || null;
    if (dto.farrowing_rate_pct !== undefined)
      updates.farrowing_rate_pct = dto.farrowing_rate_pct?.toString() || null;
    if (dto.boar_doses_per_week !== undefined)
      updates.boar_doses_per_week = dto.boar_doses_per_week?.toString() || null;
    if (dto.boar_productive_life_months !== undefined)
      updates.boar_productive_life_months = dto.boar_productive_life_months;
    if (dto.age_labels !== undefined)
      updates.age_labels = JSON.stringify(dto.age_labels);
  ...
  async removeBreed(id: string, tenantId: string, userPayload?: any) {
    ...
    return {
      success: true,
      message: `Breed '${breed.breed_name}' has been soft-deleted.`,
    };
  }
  ...
  private async assertItemExists(itemId: string) {
    const [item] = await this.db
      ...
      .where(
        and(
          eq(schema.itemMaster.item_id, itemId),
  ...
  async createLifecycleStage(
    dto: CreateBreedLifecycleStageDto,
    tenantId: string,
    userPayload?: any,
  ) {
    ...
    const lifecycleCode = await this.numberSeriesService.resolveOptionalCode(
    ...
    const newLifecycleStage = {
      ...
      feed_qty_per_head_per_day_kg:
        dto.feed_qty_per_head_per_day_kg?.toString() || null,
      ...
      medication_protocol: dto.medication_protocol
        ? JSON.stringify(dto.medication_protocol)
        : null,
      vaccination_protocol: dto.vaccination_protocol
        ? JSON.stringify(dto.vaccination_protocol)
        : null,
      resource_requirements: dto.resource_requirements
        ? JSON.stringify(dto.resource_requirements)
        : null,
      kpi_thresholds: dto.kpi_thresholds
        ? JSON.stringify(dto.kpi_thresholds)
        : null,
  ...
  async findOneLifecycleStage(id: string) {
    const [lifecycleStage] = await this.db
      ...
      .leftJoin(
        schema.stageMaster,
        eq(schema.breedLifecycleStages.stage_id, schema.stageMaster.stage_id),
      )
      .leftJoin(
        schema.breedMaster,
        eq(schema.breedLifecycleStages.breed_id, schema.breedMaster.breed_id),
      )
    ...
    if (!lifecycleStage) {
      throw new NotFoundException(
        `Breed lifecycle stage with ID '${id}' not found.`,
      );
    }
  ...
  async findAllLifecycleStages(
    query: QueryBreedLifecycleStageDto,
    tenantId: string,
  ) {
    ...
    if (scopeConditions.length) {
      conditions.push(
        inArray(
          schema.breedLifecycleStages.breed_id,
          this.db
            .select({ id: schema.breedMaster.breed_id })
            .from(schema.breedMaster)
    ...
    if (query.breedId)
      conditions.push(eq(schema.breedLifecycleStages.breed_id, query.breedId));
    if (query.stageId)
      conditions.push(eq(schema.breedLifecycleStages.stage_id, query.stageId));
    if (query.search) {
  ...
  async updateLifecycleStage(
    id: string,
    dto: UpdateBreedLifecycleStageDto,
    tenantId: string,
    userPayload?: any,
  ) {
    ...
    if (dto.stage_id) {
      ...
      if (!stage) {
        throw new NotFoundException(
          `Stage with ID '${dto.stage_id}' not found.`,
        );
      }
    }
    if (dto.feed_item_id) await this.assertItemExists(dto.feed_item_id);
    ...
    const lifecycleCode = await this.numberSeriesService.editedCode(
      'BREED_LIFECYCLE_STAGE',
      dto.lifecycle_code,
      lifecycleStage.lifecycle_code,
      tenantId,
      null,
    );
    ...
    if (dto.feed_qty_per_head_per_day_kg !== undefined)
      updates.feed_qty_per_head_per_day_kg =
        dto.feed_qty_per_head_per_day_kg?.toString() || null;
    if (dto.feed_wastage_pct !== undefined)
      updates.feed_wastage_pct = dto.feed_wastage_pct?.toString() || null;
    if (dto.std_body_weight_kg !== undefined)
      updates.std_body_weight_kg = dto.std_body_weight_kg?.toString() || null;
    if (dto.std_adg_gpd !== undefined)
      updates.std_adg_gpd = dto.std_adg_gpd?.toString() || null;
    if (dto.std_fcr !== undefined)
      updates.std_fcr = dto.std_fcr?.toString() || null;
    if (dto.std_mortality_rate_pct !== undefined)
      updates.std_mortality_rate_pct =
        dto.std_mortality_rate_pct?.toString() || null;
    if (dto.output_item_id !== undefined)
      updates.output_item_id = dto.output_item_id;
    if (dto.output_uom !== undefined) updates.output_uom = dto.output_uom;
    if (dto.std_output_qty !== undefined)
      updates.std_output_qty = dto.std_output_qty?.toString() || null;
    if (dto.medication_protocol !== undefined)
      updates.medication_protocol = JSON.stringify(dto.medication_protocol);
    if (dto.vaccination_protocol !== undefined)
      updates.vaccination_protocol = JSON.stringify(dto.vaccination_protocol);
    if (dto.resource_requirements !== undefined)
      updates.resource_requirements = JSON.stringify(dto.resource_requirements);
    if (dto.kpi_thresholds !== undefined)
      updates.kpi_thresholds = JSON.stringify(dto.kpi_thresholds);
    if (dto.kpi_lower_limit !== undefined)
      updates.kpi_lower_limit = dto.kpi_lower_limit?.toString() || null;
    if (dto.kpi_upper_limit !== undefined)
      updates.kpi_upper_limit = dto.kpi_upper_limit?.toString() || null;
    if (dto.alert_severity !== undefined)
      updates.alert_severity = dto.alert_severity;
  ...
  async removeLifecycleStage(id: string, tenantId: string, userPayload?: any) {
    ...
    return {
      success: true,
      message: 'Breed lifecycle stage has been deactivated.',
    };
  }
  ...
  async restoreLifecycleStage(id: string, tenantId: string, userPayload?: any) {
    ...
    if (lifecycleStage.is_active) {
      return {
        success: true,
        message: 'Breed lifecycle stage is already active.',
      };
    }
    ...
    return {
      success: true,
      message: 'Breed lifecycle stage has been restored.',
    };
  }
}
