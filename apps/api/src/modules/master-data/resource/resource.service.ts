...
export class ResourceService {
  ...
  async create(dto: CreateResourceDto, tenantId: string, userPayload?: any) {
    ...
    await this.numberSeriesService.ensureCompanySeries(
      ...
      async () => {
        const rows = await this.db
          .select({ code: schema.resourceMaster.resource_code })
          .from(schema.resourceMaster)
          .where(
            and(
              eq(schema.resourceMaster.tenant_id, tenantId),
              companyCondition(
                schema.resourceMaster.company_id,
                dto.company_id,
              ),
            ),
          );
    ...
    const newResource = {
      ...
      maintenance_cost_per_service:
        dto.maintenance_cost_per_service?.toString() || null,
  ...
  async findOne(id: string) {
    const [resource] = await this.db
      ...
      .where(
        and(
          eq(schema.resourceMaster.resource_id, id),
  ...
  async findAll(query: QueryResourceDto, tenantId: string) {
    ...
    const conditions: any[] = [eq(schema.resourceMaster.tenant_id, tenantId)];

    conditions.push(
      ...masterScopeConditions(
        this.cls,
        schema.resourceMaster,
        query.companyId,
      ),
    );
    if (query.resourceType) {
      conditions.push(
        eq(schema.resourceMaster.resource_type, query.resourceType),
      );
    }
    if (query.nobId) {
    ...
    if (query.maintenanceDueWithinDays !== undefined) {
      ...
      conditions.push(
        and(
          ...
          lte(
            schema.resourceMaster.next_maintenance_date,
    ...
    conditions.push(
      ...listFilterConditions(schema.resourceMaster, query.filter),
    );
    ...
    return runMasterList(
      ...
      conditions,
      query,
      schema.resourceMaster.resource_code,
    );
  }
  ...
  async update(
    ...
    if (
      dto.resource_code &&
      dto.resource_code.toUpperCase() !== resource.resource_code
    ) {
      throw new ConflictException(
        'Resource Code is generated from the company-wide RESOURCE sequence and cannot be changed.',
      );
    }
    ...
    if (dto.resource_name !== undefined)
      updates.resource_name = dto.resource_name;
    if (dto.resource_type !== undefined)
      updates.resource_type = dto.resource_type;
    if (dto.resource_sub_type !== undefined)
      updates.resource_sub_type = dto.resource_sub_type;
    ...
    if (dto.capacity !== undefined)
      updates.capacity = dto.capacity?.toString() || null;
    if (dto.capacity_uom !== undefined) updates.capacity_uom = dto.capacity_uom;
    if (dto.unit !== undefined) updates.unit = dto.unit;
    if (dto.cost_rate !== undefined)
      updates.cost_rate = dto.cost_rate?.toString() || null;
    ...
    if (dto.asset_serial_no !== undefined)
      updates.asset_serial_no = dto.asset_serial_no;
    if (dto.purchase_date !== undefined)
      updates.purchase_date = dto.purchase_date;
    if (dto.warranty_expiry_date !== undefined)
      updates.warranty_expiry_date = dto.warranty_expiry_date;
    if (dto.maintenance_frequency_days !== undefined)
      updates.maintenance_frequency_days = dto.maintenance_frequency_days;
    if (dto.maintenance_cost_per_service !== undefined)
      updates.maintenance_cost_per_service =
        dto.maintenance_cost_per_service?.toString() || null;
    if (dto.maintenance_vendor !== undefined)
      updates.maintenance_vendor = dto.maintenance_vendor;
  ...
  async remove(id: string, tenantId: string, userPayload?: any) {
    ...
    return {
      success: true,
      message: `Resource '${resource.resource_name}' has been soft-deleted.`,
    };
  }
  ...
  async createMaintenanceLog(
    ...
    if (!resource.company_id)
      throw new BadRequestException(
        'Maintenance is recorded against a company resource, not a tenant template.',
      );
  ...
  async findOneMaintenanceLog(logId: string) {
    const [log] = await this.db
      ...
      .where(
        and(
          eq(schema.resourceMaintenanceLog.log_id, logId),
    ...
    if (!log) {
      throw new NotFoundException(
        `Maintenance log with ID '${logId}' not found.`,
      );
    }
  ...
  async updateMaintenanceLog(
    ...
    if (dto.maintenance_date !== undefined)
      updates.maintenance_date = dto.maintenance_date;
    if (dto.maintenance_type !== undefined)
      updates.maintenance_type = dto.maintenance_type;
