...
export class ItemTypeService {
  ...
  private async resolveTypeCode(
    dto: CreateItemTypeDto,
    tenantId: string,
    ...
    if (!seriesCode) {
      if (!dto.type_code) {
        throw new BadRequestException(
          'type_code is required — no number series is configured for item types.',
        );
      }
  ...
  async create(dto: CreateItemTypeDto, tenantId: string, userPayload?: any) {
    ...
    if (existing.length > 0) {
      throw new ConflictException(
        `Item type with code '${typeCode}' already exists in this scope.`,
      );
    }
  ...
  async findOne(id: string) {
    const [itemType] = await this.db
      ...
      .where(
        and(
          eq(schema.itemTypeMaster.item_type_id, id),
          isNull(schema.itemTypeMaster.deleted_at),
        ),
      )
      .limit(1);
  ...
  async findAll(query: QueryItemTypeDto, tenantId: string) {
    ...
    const conditions: any[] = [eq(schema.itemTypeMaster.tenant_id, tenantId)];

    conditions.push(
      ...masterScopeConditions(
        this.cls,
        schema.itemTypeMaster,
        query.companyId,
      ),
    );
    ...
    conditions.push(
      ...listFilterConditions(schema.itemTypeMaster, query.filter),
    );
    ...
    return runMasterList(
      ...
      conditions,
      query,
      schema.itemTypeMaster.type_code,
    );
  }
  ...
  async update(
    ...
    if (
      itemType.is_system &&
      (dto.is_active === false ||
        (dto.status !== undefined && dto.status.toUpperCase() !== 'ACTIVE'))
    ) {
      throw new ConflictException(
        `Item type '${itemType.type_code}' is a system type and cannot be deactivated.`,
      );
    }
    ...
    if (dto.code_prefix !== undefined)
      updates.code_prefix = dto.code_prefix.toUpperCase();
  ...
  async remove(id: string, tenantId: string, userPayload?: any) {
    ...
    if (itemType.is_system) {
      throw new ConflictException(
        `Item type '${itemType.type_code}' is a system type and cannot be deleted.`,
      );
    }
    ...
    if (itemType.company_id)
      usageConditions.push(
        eq(schema.itemMaster.company_id, itemType.company_id),
      );
    ...
    if (item) {
      throw new ConflictException(
        `Item type '${itemType.type_code}' is already used by an item and cannot be deleted.`,
      );
    }
    ...
    return {
      success: true,
      message: `Item Type '${itemType.type_name}' has been soft-deleted.`,
    };
  }
