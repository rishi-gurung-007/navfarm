...
export class DiseaseService {
   ...
   */
  private async resolveDiseaseCode(
    ...
    if (!seriesCode) {
      if (!dto.disease_code) {
        throw new BadRequestException(
          'disease_code is required — no number series is configured for diseases.',
        );
      }
  ...
  async create(dto: CreateDiseaseDto, tenantId: string, userPayload?: any) {
    ...
    if (existing.length > 0) {
      throw new ConflictException(
        `Disease definition with code '${diseaseCode}' already exists in this company.`,
      );
    }
  ...
  async findOne(id: string) {
    const [disease] = await this.db
      ...
      .where(
        and(
          eq(schema.diseaseMaster.disease_id, id),
          isNull(schema.diseaseMaster.deleted_at),
        ),
      )
      .limit(1);
  ...
  async findAll(query: QueryDiseaseDto, tenantId: string) {
    ...
    const conditions: any[] = [eq(schema.diseaseMaster.tenant_id, tenantId)];

    conditions.push(
      ...masterScopeConditions(this.cls, schema.diseaseMaster, query.companyId),
    );
    ...
    conditions.push(
      ...listFilterConditions(schema.diseaseMaster, query.filter),
    );
    ...
    return runMasterList(
      this.db,
      schema.diseaseMaster,
      conditions,
      query,
      schema.diseaseMaster.disease_code,
    );
  }
  ...
  async update(
    ...
    if (
      dto.disease_code &&
      dto.disease_code.toUpperCase() !== disease.disease_code
    ) {
      const existing = await this.db
        .select()
        .from(schema.diseaseMaster)
        .where(
          and(
            eq(schema.diseaseMaster.tenant_id, tenantId),
            companyCondition(
              schema.diseaseMaster.company_id,
              disease.company_id,
            ),
            eq(
              schema.diseaseMaster.disease_code,
              dto.disease_code.toUpperCase(),
            ),
      ...
      if (existing.length > 0) {
        throw new ConflictException(
          `Disease definition with code '${dto.disease_code}' already exists in this company.`,
        );
      }
    }
    ...
    if (dto.disease_code !== undefined)
      updates.disease_code = dto.disease_code.toUpperCase();
    if (dto.disease_name !== undefined) updates.disease_name = dto.disease_name;
    if (dto.scientific_name !== undefined)
      updates.scientific_name = dto.scientific_name;
    if (dto.symptoms !== undefined) updates.symptoms = dto.symptoms;
    if (dto.treatment_guideline !== undefined)
      updates.treatment_guideline = dto.treatment_guideline;
  ...
  async remove(id: string, tenantId: string, userPayload?: any) {
    ...
    return {
      success: true,
      message: `Disease '${disease.disease_name}' has been soft-deleted.`,
    };
  }
