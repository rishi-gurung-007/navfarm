...
export class FeedFormulaController {
  ...
  async create(@Body() dto: CreateFeedFormulaDto, @Req() req: any) {
    ...
    const result = await this.feedFormulaService.create(
      dto,
      tenantId,
      req.user,
    );
  ...
  @ApiOperation({
    summary:
      'Fetch details of a single Feed Formula by UUID (including ingredients)',
  })
  ...
  @ApiOperation({
    summary: 'Update header details of an existing Feed Formula',
  })
  @ApiParam({ name: 'id', description: 'Formula UUID' })
  async update(
    @Param('id') id: string,
    @Body() dto: UpdateFeedFormulaDto,
    @Req() req: any,
  ) {
    const tenantId = req.user?.tenantId || req['tenantId'];
    const result = await this.feedFormulaService.update(
      id,
      dto,
      tenantId,
      req.user,
  ...
  @ApiOperation({
    summary: 'Deactivate (soft-delete) a Feed Formula and its ingredients',
  })
  @ApiParam({ name: 'id', description: 'Formula UUID' })
  ...
  @RequirePermission('MASTER_DATA', 'FEED_FORMULA', 'edit')
  @ApiOperation({
    summary: 'Restore a soft-deleted Feed Formula and its ingredients',
  })
  @ApiParam({ name: 'id', description: 'Formula UUID' })
  async restore(@Param('id') id: string, @Req() req: any) {
    const tenantId = req.user?.tenantId || req['tenantId'];
    const result = await this.feedFormulaService.restore(
      id,
      tenantId,
      req.user,
    );
