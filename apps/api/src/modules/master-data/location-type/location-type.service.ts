...
import {
  ...
  QueryLocationTypeDto,
  UpdateLocationTypeDto,
} from './dto/location-type.dto';
...
export class LocationTypeService {
  ...
  private async assertPrefixAvailable(
    prefix: string,
    tenantId: string,
    ...
    if (excludeId)
      conditions.push(
        ne(schema.locationTypeMaster.location_type_id, excludeId),
      );
    conditions.push(
      companyCondition(schema.locationTypeMaster.company_id, companyId),
    );
    const [duplicate] = await this.db
      .select({ id: schema.locationTypeMaster.location_type_id })
      .from(schema.locationTypeMaster)
      .where(and(...conditions))
      .limit(1);
    if (duplicate)
      throw new ConflictException(
        `Location prefix '${prefix}' is already used by another type in this scope.`,
      );
  }

  private async assertParentTypesExist(
    parentTypes: string[],
    ownType: string,
    tenantId: string,
    companyId: string | null,
  ) {
    if (parentTypes.includes(ownType))
      throw new ConflictException(
        'A Location Type cannot allow itself as a parent.',
      );
    for (const parentType of parentTypes) {
      ...
      if (!parent)
        throw new NotFoundException(
          `Allowed parent Location Type '${parentType}' does not exist.`,
        );
    }
  }
  ...
  async create(dto: CreateLocationTypeDto, tenantId: string, user?: any) {
    ...
    const typeCode = await this.numberSeriesService.resolveNewCode(
      'LOCATION_TYPE',
      dto.type_code,
      tenantId,
      companyId,
    ...
    const [duplicate] = await this.db
      ...
      .where(
        and(
          ...
          companyId
            ? eq(schema.locationTypeMaster.company_id, companyId)
    ...
    if (duplicate)
      throw new ConflictException(
        `Location type '${typeCode}' already exists in this scope.`,
      );
    ...
    await this.assertParentTypesExist(
      parentTypes,
      typeCode,
      tenantId,
      companyId,
    ...
    const row = {
      ...
      type_code: typeCode,
      type_name: dto.type_name.trim(),
      code_prefix: prefix,
      allowed_parent_types: parentTypes,
      is_system: false,
      is_active: true,
      status: 'ACTIVE',
    ...
    await this.auditService.log({
      ...
      userId: user?.userId,
      action: 'CREATE',
      entityName: 'location_type_master',
      entityId: row.location_type_id,
      newValues: row,
    });
  ...
  async findAll(query: QueryLocationTypeDto, tenantId: string) {
    ...
    conditions.push(
      ...masterScopeConditions(
        this.cls,
        schema.locationTypeMaster,
        query.companyId,
      ),
    );
    if (query.isActive !== undefined)
      conditions.push(eq(schema.locationTypeMaster.is_active, query.isActive));
    if (query.search)
      conditions.push(
        or(
          like(schema.locationTypeMaster.type_code, `%${query.search}%`),
          like(schema.locationTypeMaster.type_name, `%${query.search}%`),
        )!,
      );
    conditions.push(
      ...listFilterConditions(schema.locationTypeMaster, query.filter),
    );
    const rows = await this.db
      ...
      .orderBy(
        sql`${schema.locationTypeMaster.company_id} IS NULL`,
        listOrderBy(
          schema.locationTypeMaster,
          query,
          schema.locationTypeMaster.type_code,
        ),
      )
    ...
    rows.forEach((row) => {
      if (!effective.has(row.type_code)) effective.set(row.type_code, row);
    });
  ...
  async update(
    ...
    if (dto.code_prefix !== undefined) {
      ...
      if (prefix !== existing.code_prefix)
        await this.assertPrefixAvailable(
          prefix,
          tenantId,
          existing.company_id,
          id,
    ...
    await this.db.transaction(async (tx) => {
      await tx
        .update(schema.locationTypeMaster)
        .set(updates)
        .where(eq(schema.locationTypeMaster.location_type_id, id));
      if (dto.code_prefix !== undefined || dto.type_name !== undefined) {
        await tx
          .update(schema.noSeriesMaster)
          .set({
            ...(dto.code_prefix !== undefined
              ? { prefix: dto.code_prefix.toUpperCase() }
              : {}),
            ...(dto.type_name !== undefined
              ? { series_name: `${dto.type_name.trim()} Location` }
              : {}),
            updated_by: user?.userId || null,
          })
          .where(
            and(
              ...
              eq(
                ...
                seriesCodeFor(existing.type_code),
              ),
              ...
              ...(existing.company_id
                ? [eq(schema.noSeriesMaster.company_id, existing.company_id)]
                : []),
            ),
          );
      }
  ...
  async remove(id: string, tenantId: string, user?: any) {
    ...
    if (existing.is_system) {
      throw new ConflictException(
        `Location type '${existing.type_code}' is a core type and cannot be deleted.`,
      );
    }
    ...
    if (existing.company_id)
      usageConditions.push(
        eq(schema.locationMaster.company_id, existing.company_id),
      );
    const [location] = await this.db
      .select({ id: schema.locationMaster.location_id })
      .from(schema.locationMaster)
      .where(and(...usageConditions))
      .limit(1);
    if (location)
      throw new ConflictException(
        `Location type '${existing.type_code}' is already used by a location and cannot be deleted.`,
      );
  ...
  async restore(id: string, tenantId: string, user?: any) {
    const [existing] = await this.db
      ...
      .limit(1);
    if (!existing)
      throw new NotFoundException(`Location type '${id}' not found.`);
    await this.db
      .update(schema.locationTypeMaster)
      .set({
        is_active: true,
