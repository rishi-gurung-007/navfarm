...
export class SupplierService {
  ...
  private assertVendorTypeRequirements(
    vendorType: string,
    healthCertUrl?: string | null,
    breedingFarmCode?: string | null,
  ) {
    if (
      vendorType === 'ANIMAL_SUPPLIER' &&
      (!healthCertUrl || !breedingFarmCode)
    ) {
      throw new BadRequestException(
        'ANIMAL_SUPPLIER vendors require both health_cert_url and breeding_farm_code.',
      );
    }
    if (vendorType === 'BREEDING_FARM' && !breedingFarmCode) {
      throw new BadRequestException(
        'BREEDING_FARM vendors require breeding_farm_code.',
      );
    }
  }
  ...
  private maskSupplier(supplier: typeof schema.supplierMaster.$inferSelect) {
    ...
    if (bank_account_no_enc) {
      try {
        bank_account_last4 = EncryptionService.maskLast4(
          this.encryptionService.decrypt(bank_account_no_enc),
        );
      } catch {
  ...
  async create(dto: CreateSupplierDto, tenantId: string, userPayload?: any) {
    ...
    this.assertVendorTypeRequirements(
      vendorType,
      dto.health_cert_url,
      dto.breeding_farm_code,
    );
    ...
    await this.numberSeriesService.ensureCompanySeries(
      ...
      async () => {
        const rows = await this.db
          .select({ code: schema.supplierMaster.supplier_code })
          .from(schema.supplierMaster)
          .where(
            and(
              eq(schema.supplierMaster.tenant_id, tenantId),
              companyCondition(
                schema.supplierMaster.company_id,
                dto.company_id,
              ),
            ),
          );
    ...
    const newSupplier = {
      ...
      bank_account_no_enc: dto.bank_account_no
        ? this.encryptionService.encrypt(dto.bank_account_no)
        : null,
    ...
    await this.auditService.log({
      ...
      newValues: {
        ...newSupplier,
        bank_account_no_enc: newSupplier.bank_account_no_enc
          ? '[REDACTED]'
          : null,
      },
    });
  ...
  async findOne(id: string) {
    const [supplier] = await this.db
      ...
      .where(
        and(
          eq(schema.supplierMaster.supplier_id, id),
  ...
  async findAll(query: QuerySupplierDto, tenantId: string) {
    ...
    const conditions: any[] = [eq(schema.supplierMaster.tenant_id, tenantId)];

    conditions.push(
      ...masterScopeConditions(
        this.cls,
        schema.supplierMaster,
        query.companyId,
      ),
    );
    ...
    conditions.push(
      ...listFilterConditions(schema.supplierMaster, query.filter),
    );
    ...
    const rows = await this.db
      ...
      .orderBy(
        listOrderBy(
          schema.supplierMaster,
          query,
          schema.supplierMaster.supplier_code,
        ),
      )
      .limit(limit)
      .offset(offset);
  ...
  async update(
    ...
    if (
      dto.supplier_code &&
      dto.supplier_code.toUpperCase() !== supplier.supplier_code
    ) {
      throw new ConflictException(
        'Supplier Code is generated from the company-wide SUPPLIER sequence and cannot be changed.',
      );
    }
    ...
    const effectiveHealthCertUrl =
      dto.health_cert_url !== undefined
        ? dto.health_cert_url
        : supplier.health_cert_url;
    const effectiveBreedingFarmCode =
      dto.breeding_farm_code !== undefined
        ? dto.breeding_farm_code
        : supplier.breeding_farm_code;
    this.assertVendorTypeRequirements(
      effectiveVendorType,
      effectiveHealthCertUrl,
      effectiveBreedingFarmCode,
    );
    ...
    if (dto.health_cert_url !== undefined)
      updates.health_cert_url = dto.health_cert_url;
    if (dto.breeding_farm_code !== undefined)
      updates.breeding_farm_code = dto.breeding_farm_code;
    if (dto.bank_account_no !== undefined)
      updates.bank_account_no_enc = dto.bank_account_no
        ? this.encryptionService.encrypt(dto.bank_account_no)
        : null;
    ...
    await this.auditService.log({
      ...
      newValues: {
        ...updates,
        bank_account_no_enc:
          updates.bank_account_no_enc !== undefined ? '[REDACTED]' : undefined,
      },
    });
  ...
  async approve(id: string, tenantId: string, userPayload?: any) {
    ...
    if (supplier.is_approved) {
      throw new BadRequestException(
        `Supplier '${supplier.supplier_name}' is already approved.`,
      );
    }
  ...
  async remove(id: string, tenantId: string, userPayload?: any) {
    ...
    return {
      success: true,
      message: `Supplier '${supplier.supplier_name}' has been soft-deleted.`,
    };
  }
