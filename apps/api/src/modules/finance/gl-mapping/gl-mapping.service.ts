...
export class GlMappingService {
  ...
  async create(dto: CreateGlMappingDto, tenantId: string, userPayload?: any) {
    ...
    if (dto.item_category_id) {
      ...
      if (!category) {
        throw new NotFoundException(
          `Item Category with ID '${dto.item_category_id}' not found.`,
        );
      }
    }
    ...
    if (dto.valuation_method) {
      ...
      if (!method) {
        throw new NotFoundException(
          `Costing method '${dto.valuation_method}' not found.`,
        );
      }
    }
    ...
    if (dto.debit_gl_account_id) {
      ...
      if (!debitGl) {
        throw new NotFoundException(
          `Debit G/L Account with ID '${dto.debit_gl_account_id}' not found.`,
        );
      }
    }
    ...
    if (dto.credit_gl_account_id) {
      ...
      if (!creditGl) {
        throw new NotFoundException(
          `Credit G/L Account with ID '${dto.credit_gl_account_id}' not found.`,
        );
      }
    }
    ...
    const queryConditions = [
      ...
      eq(
        schema.glMappingMaster.transaction_type,
        dto.transaction_type.toUpperCase(),
      ),
    ...
    if (dto.item_category_id) {
      queryConditions.push(
        eq(schema.glMappingMaster.item_category_id, dto.item_category_id),
      );
    } else {
    ...
    if (dto.valuation_method) {
      queryConditions.push(
        eq(schema.glMappingMaster.valuation_method, dto.valuation_method),
      );
    } else {
    ...
    if (existing.length > 0) {
      throw new ConflictException(
        `G/L Mapping rule for transaction type '${dto.transaction_type}' with these same category/NOB/LOB/stage/valuation-method dimensions already exists.`,
      );
    }
    ...
    const mappingCode = await this.numberSeriesService.resolveOptionalCode(
      'GL_MAPPING',
      dto.mapping_code,
      tenantId,
      dto.company_id,
  ...
  async findOne(id: string) {
    const [mapping] = await this.db
      ...
      .where(
        and(
          eq(schema.glMappingMaster.mapping_id, id),
          isNull(schema.glMappingMaster.deleted_at),
        ),
      )
      .limit(1);
    ...
    if (!mapping) {
      throw new NotFoundException(
        `G/L Mapping rule with ID '${id}' not found.`,
      );
    }
  ...
  async findAll(query: QueryGlMappingDto, tenantId: string) {
    ...
    const conditions: any[] = [eq(schema.glMappingMaster.tenant_id, tenantId)];

    conditions.push(
      ...masterScopeConditions(
        this.cls,
        schema.glMappingMaster,
        query.companyId,
      ),
    );
    if (query.itemCategoryId) {
      conditions.push(
        eq(schema.glMappingMaster.item_category_id, query.itemCategoryId),
      );
    }
    if (query.nobId) {
    ...
    if (query.valuationMethod) {
      conditions.push(
        eq(schema.glMappingMaster.valuation_method, query.valuationMethod),
      );
    }
    if (query.transactionType) {
      conditions.push(
        eq(
          schema.glMappingMaster.transaction_type,
          query.transactionType.toUpperCase(),
        ),
      );
    }
    ...
    conditions.push(
      ...listFilterConditions(schema.glMappingMaster, query.filter),
    );
    ...
    return runMasterList(
      ...
      conditions,
      query,
      schema.glMappingMaster.mapping_code,
    );
  }

  async update(
    id: string,
    dto: UpdateGlMappingDto,
    tenantId: string,
    userPayload?: any,
  ) {
    ...
    const valuationMethod =
      dto.valuation_method !== undefined
        ...
        : mapping.valuation_method;

    if (
      dto.item_category_id &&
      dto.item_category_id !== mapping.item_category_id
    ) {
      const [category] = await this.db
        .select()
        .from(schema.itemCategoryMaster)
    ...
    if (
      dto.valuation_method &&
      dto.valuation_method !== mapping.valuation_method
    ) {
      const [method] = await this.db
        .select()
        .from(schema.costingMethodConfig)
    ...
    if (
      dto.debit_gl_account_id &&
      dto.debit_gl_account_id !== mapping.debit_gl_account_id
    ) {
      const [debitGl] = await this.db
        .select()
        .from(schema.glAccountMaster)
    ...
    if (
      dto.credit_gl_account_id &&
      dto.credit_gl_account_id !== mapping.credit_gl_account_id
    ) {
      const [creditGl] = await this.db
        .select()
        .from(schema.glAccountMaster)
    ...
    if (
      ...
      (dto.valuation_method !== undefined &&
        dto.valuation_method !== mapping.valuation_method) ||
      (dto.transaction_type !== undefined &&
        dto.transaction_type.toUpperCase() !== mapping.transaction_type)
    ) {
      const queryConditions = [
        eq(schema.glMappingMaster.tenant_id, tenantId),
      ...
      if (categoryId) {
        queryConditions.push(
          eq(schema.glMappingMaster.item_category_id, categoryId),
        );
      } else {
        queryConditions.push(isNull(schema.glMappingMaster.item_category_id));
      }
      ...
      if (valuationMethod) {
        queryConditions.push(
          eq(schema.glMappingMaster.valuation_method, valuationMethod),
        );
      } else {
        queryConditions.push(isNull(schema.glMappingMaster.valuation_method));
      }
      ...
      if (existing.length > 0) {
        throw new ConflictException(
          `G/L Mapping rule for transaction type '${transType}' with these same category/NOB/LOB/stage/valuation-method dimensions already exists.`,
        );
      }
    }
    ...
    const mappingCode = await this.numberSeriesService.editedCode(
      'GL_MAPPING',
      dto.mapping_code,
    ...
    if (dto.item_category_id !== undefined)
      updates.item_category_id = dto.item_category_id;
    ...
    if (dto.transaction_type !== undefined)
      updates.transaction_type = dto.transaction_type.toUpperCase();
    if (dto.debit_gl_account_id !== undefined)
      updates.debit_gl_account_id = dto.debit_gl_account_id;
    if (dto.credit_gl_account_id !== undefined)
      updates.credit_gl_account_id = dto.credit_gl_account_id;
  ...
  async remove(id: string, tenantId: string, userPayload?: any) {
    ...
    return {
      success: true,
      message: 'G/L Mapping rule soft-deleted successfully.',
    };
  }
