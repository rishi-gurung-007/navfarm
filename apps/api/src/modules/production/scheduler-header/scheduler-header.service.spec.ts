  ...
  describe('createForStage', () => {
    it('returns the existing header instead of creating a duplicate for the same (batch, stage)', async () => {
      mockDbSelect
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest
                .fn()
                .mockResolvedValue([{ scheduler_id: 'sched-existing' }]),
            }),
          }),
        ...
        }) // findOne: lines (no line ids -> custom days skipped)
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest
              .fn()
        ...
        }) // findOne: batch_no lookup (no location_id -> silo stock skipped)
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest
              .fn()
        ...
        }); // findOne: stage_name lookup (no breed_id -> breed lookup skipped)

      const result = await service.createForStage(
        'batch-1',
        'stage-gest',
        'tenant-123',
      );
    ...
    it("creates a header and a CONSUMPTION line from the breed's feed lifecycle standard", async () => {
      ...
      mockDbSelect
        ...
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest.fn().mockResolvedValue([batch]),
            }),
          }),
        }) // batch lookup
        .mockReturnValueOnce({
        ...
        }) // priorHeader check — none, so effective_from = batch.start_date and animal_count falls through to opening_quantity
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockResolvedValue([{ liveCount: 0 }]),
          }),
        }) // live animal_register count — none tracked, falls back to opening_quantity
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest.fn().mockResolvedValue([
                {
                  ...
                  feed_qty_per_head_per_day_kg: '2.2000',
                  std_mortality_rate_pct: null,
                  output_item_id: null,
                  std_output_qty: null,
                  std_body_weight_kg: null,
                  medication_protocol: null,
                  vaccination_protocol: null,
                },
              ]),
            }),
          }),
        ...
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest
                .fn()
                .mockResolvedValue([{ item_name: 'Gestation Feed 14%' }]),
            }),
          }),
        }) // feed item name lookup (item_description)
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest.fn().mockResolvedValue([
                {
                  scheduler_id: 'new-sched',
                  batch_id: 'batch-1',
                  stage_id: 'stage-gest',
        ...
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest.fn().mockResolvedValue([{ batch_no: 'BAT-0001' }]),
            }),
          }),
        }) // findOne: batch_no lookup (no location_id -> silo stock skipped)
        .mockReturnValueOnce({
        ...
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockResolvedValue([
              {
                item_id: 'item-feed',
                item_name: 'Gestation Feed 14%',
                uom_primary: 'KG',
                withdrawal_days: null,
              },
            ]),
          }),
        }); // findOne: allItemIds lookup
      ...
      const headerInsert = insertedValues.one?.find(
        (v) => v.batch_id === 'batch-1' && v.stage_id === 'stage-gest',
      );
      ...
      expect(lineInsert).toEqual(
        expect.objectContaining({
          ...
          stage_id: 'stage-gest',
          item_id: 'item-feed',
          item_description: 'Gestation Feed 14%',
          standard_qty: '2.2000',
          qty_basis: 'PER_HEAD',
        }),
      );
  ...
  describe('createManualHeader', () => {
    it('creates a manual scheduler_header with auto_generated=false', async () => {
      ...
      mockDbSelect
        ...
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest
                .fn()
                .mockResolvedValue([{ ...batch, lob_id: 'lob-1' }]),
            }),
          }),
        }) // batch lookup
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest
                .fn()
                .mockResolvedValue([{ ...stage, lob_id: 'lob-1' }]),
            }),
          }),
        }) // stage lookup
        .mockReturnValueOnce({
        ...
        }) // generateLinesFromLifecycle: breed_lifecycle_stages (none)
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest.fn().mockResolvedValue([
        ...
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest
                .fn()
                .mockResolvedValue([{ breed_name: 'Large White' }]),
            }),
          }),
        }) // findOne: breed
        .mockReturnValueOnce({
  ...
  describe('addLine', () => {
    it('rejects a CONSUMPTION line missing item_id/standard_qty/qty_basis', async () => {
      ...
      await expect(
        service.addLine(
          'sched-1',
          { line_type: 'CONSUMPTION', activity_name: 'Evening Feed' } as any,
          'tenant-123',
        ),
    ...
    it('rejects a line with the same item and overlapping time period', async () => {
      mockDbSelect
        ...
        .mockReturnValueOnce({
          from: jest.fn().mockReturnValue({
            where: jest.fn().mockReturnValue({
              limit: jest.fn().mockResolvedValue([{ stage_name: 'Weaner' }]),
            }),
          }),
        }) // findOne: stage
        .mockReturnValueOnce({
