...
import {
  CreateSchedulerHeaderDto,
  UpdateSchedulerHeaderDto,
  CreateSchedulerLineDto,
  UpdateSchedulerLineDto,
  UpdateSchedulerHeaderStatusDto,
  QuerySchedulerHeaderDto,
...
function parseProtocolEntries(
  ...
  return value
    .map((entry) => {
      ...
      if (!Number.isFinite(day) || typeof name !== 'string' || !name.trim())
        return null;
...
export class SchedulerHeaderService {
  ...
  private async generateLinesFromLifecycle(
    ...
    if (lifecycle.feed_item_id && lifecycle.feed_qty_per_head_per_day_kg) {
      const [feedItem] = await this.db
        .select({ item_name: schema.itemMaster.item_name })
        .from(schema.itemMaster)
        .where(eq(schema.itemMaster.item_id, lifecycle.feed_item_id))
        .limit(1);
      lines.push({
        line_id: randomUUID(),
        ...
        activity_name: `${stage.stage_name} Feed`,
        stage_id: stageId,
        occurrence: 'DAILY',
        start_day: 1,
        end_day: null,
        is_mandatory: true,
        source: 'AUTO',
        lifecycle_ref_id: lifecycle.lifecycle_id,
        nob_id: batch.nob_id,
        lob_id: batch.lob_id,
        item_id: lifecycle.feed_item_id,
        item_description: feedItem?.item_name ?? null,
        standard_qty: lifecycle.feed_qty_per_head_per_day_kg,
        qty_basis: 'PER_HEAD',
        allow_qty_edit: true,
        lot_required: true,
      });
    ...
    if (lifecycle.std_mortality_rate_pct) {
      lines.push({
        ...
        activity_name: `${stage.stage_name} Mortality`,
        stage_id: stageId,
        occurrence: 'DAILY',
        start_day: 1,
        end_day: null,
        ...
        kpi_uom: KPI_METRIC_UOM.MORTALITY_COUNT,
        capture_per: 'TOTAL',
    ...
    if (lifecycle.output_item_id && lifecycle.std_output_qty) {
      const [outputItem] = await this.db
        .select({ item_name: schema.itemMaster.item_name })
        .from(schema.itemMaster)
        .where(eq(schema.itemMaster.item_id, lifecycle.output_item_id))
        .limit(1);
      lines.push({
        line_id: randomUUID(),
        scheduler_id: schedulerId,
        line_seq: seq++,
        line_type: 'OUTPUT',
        activity_name: `${stage.stage_name} Output`,
        stage_id: stageId,
        occurrence: 'ONCE',
        start_day: stage.typical_duration_days || 1,
        end_day: null,
        is_mandatory: false,
        source: 'AUTO',
        lifecycle_ref_id: lifecycle.lifecycle_id,
        nob_id: batch.nob_id,
        lob_id: batch.lob_id,
        item_id: lifecycle.output_item_id,
        item_description: outputItem?.item_name ?? null,
        standard_qty: lifecycle.std_output_qty,
      ...
      lines.push({
        ...
        line_seq: seq++,
        line_type: 'DESCRIPTIVE',
        activity_name: `${stage.stage_name} Body Weight Check`,
        stage_id: stageId,
        occurrence: 'WEEKLY',
        start_day: 7,
        end_day: null,
        ...
        kpi_uom: KPI_METRIC_UOM.BODY_WEIGHT,
        capture_per: 'AVERAGE',
        std_value: lifecycle.std_body_weight_kg,
        alert_severity: 'WARNING',
      });
    }

    const protocolEntries: Array<{ entry: ProtocolEntry; label: string }> = [
      ...parseProtocolEntries(lifecycle.medication_protocol, 'medicine').map(
        (entry) => ({ entry, label: 'Medication' }),
      ),
      ...parseProtocolEntries(lifecycle.vaccination_protocol, 'vaccine').map(
        (entry) => ({ entry, label: 'Vaccination' }),
      ),
    ];
    for (const { entry, label } of protocolEntries) {
      const item = await this.findItemByName(
        batch.tenant_id,
        batch.company_id,
        entry.name,
      );
      ...
      lines.push({
        ...
        line_seq: seq++,
        line_type: 'CONSUMPTION',
        activity_name: `${label}: ${entry.name}`,
        stage_id: stageId,
        occurrence: 'CUSTOM',
        start_day: entry.day,
        end_day: entry.day,
    ...
    const customDayRows = Object.entries(customDaysByLine).flatMap(
      ([lineId, days]) =>
        days.map((day) => ({
    ...
    if (customDayRows.length)
      await this.db
        .insert(schema.schedulerLineCustomDays)
        .values(customDayRows);
  }
  ...
  async createForStage(
    ...
    const [stage] = await this.db
      ...
      .where(eq(schema.stageMaster.stage_id, stageId))
      .limit(1);
    ...
    const [priorHeader] = await this.db
      .select({
      ...
      .orderBy(desc(schema.schedulerHeader.created_at))
      .limit(1);
    const effectiveFrom = priorHeader
      ? toDateOnly(new Date())
      : batch.start_date;
    const effectiveTo = stage.typical_duration_days
      ? addDays(effectiveFrom, stage.typical_duration_days)
      : null;
    const locationId =
  ...
  async generateForBatchCurrentStage(
    batchId: string,
    tenantId: string,
    userPayload?: { userId?: string },
  ) {
    ...
    if (!batch.stage_id) {
      throw new BadRequestException(
        'This batch has not transferred into a Stage Master stage yet — transfer it into a stage first.',
      );
    }
   ...
   */
  async createManualHeader(
    dto: CreateSchedulerHeaderDto,
    tenantId: string,
    userPayload?: { userId?: string },
  ) {
    const [existing] = await this.db
      ...
      .where(
        and(
          eq(schema.schedulerHeader.batch_id, dto.batch_id),
          eq(schema.schedulerHeader.stage_id, dto.stage_id),
        ),
      )
      .limit(1);
    if (existing) {
      throw new ConflictException(
        `A scheduler already exists for batch '${dto.batch_id}' and stage '${dto.stage_id}'.`,
      );
    }
    ...
    const [batch] = await this.db
      .select()
      .from(schema.batchHeader)
      .where(eq(schema.batchHeader.batch_id, dto.batch_id))
      .limit(1);
    if (!batch)
    ...
    const [stage] = await this.db
      .select()
      .from(schema.stageMaster)
      .where(eq(schema.stageMaster.stage_id, dto.stage_id))
      .limit(1);
    if (!stage)
      throw new NotFoundException(`Stage '${dto.stage_id}' not found.`);

    if (stage.lob_id !== batch.lob_id) {
      throw new BadRequestException(
        `Stage '${stage.stage_name}' does not belong to the batch's Line of Business.`,
      );
    }
    ...
    if (!effectiveFrom) {
      const [priorHeader] = await this.db
        ...
        .from(schema.schedulerHeader)
        .where(eq(schema.schedulerHeader.batch_id, dto.batch_id))
        .limit(1);
      effectiveFrom = priorHeader
        ? toDateOnly(new Date())
        : batch.start_date || toDateOnly(new Date());
    }
    const effectiveTo =
      dto.effective_to ||
    ...
    if (animalCount == null) {
      ...
      const [{ liveCount }] = await this.db
        ...
        .where(
          and(
            eq(schema.animalRegister.current_batch_id, dto.batch_id),
            eq(schema.animalRegister.is_active, true),
          ),
        );
      if (liveCount > 0) {
        ...
        animalCount =
          priorHeader?.animal_count != null
            ? Number(priorHeader.animal_count)
            : Number(batch.closing_quantity ?? batch.opening_quantity ?? 0);
      }
    }
    ...
    if (dto.lines && dto.lines.length > 0) {
      for (let i = 0; i < dto.lines.length; i++) {
        this.assertLineTypeFields(
          dto.lines[i].line_type,
          dto.lines[i],
          dto.lines[i] as unknown as Record<string, unknown>,
        );
      ...
      for (const line of dto.lines) {
        ...
        await this.db.insert(schema.schedulerLine).values({
          ...
          standard_qty:
            line.standard_qty != null ? line.standard_qty.toString() : null,
          ...
          lower_alert_limit:
            line.lower_alert_limit != null
              ? line.lower_alert_limit.toString()
              : null,
          upper_alert_limit:
            line.upper_alert_limit != null
              ? line.upper_alert_limit.toString()
              : null,
          ...
          estimated_cost:
            line.estimated_cost != null ? line.estimated_cost.toString() : null,
      ...
      await this.generateLinesFromLifecycle(
        schedulerId,
        batch,
        dto.stage_id,
        stage,
    ...
    await this.auditService.log({
      ...
      newValues: {
        ...
        auto_generated: false,
        status,
        lines_count: dto.lines?.length || 0,
      },
    });
  ...
  async updateHeader(
    ...
    if (dto.data_entry_level !== undefined)
      updates.data_entry_level = dto.data_entry_level;
    ...
    if (dto.animal_count !== undefined)
      updates.animal_count = dto.animal_count.toString();
    if (dto.notes !== undefined) updates.notes = dto.notes;

    await this.db
      .update(schema.schedulerHeader)
      .set(updates)
      .where(eq(schema.schedulerHeader.scheduler_id, id));
  ...
  private async loadSiloStock(locationId: string | null) {
    ...
    return rows
      .map((r) => ({ ...r, on_hand_qty: Number(r.on_hand_qty) }))
  ...
  async findOne(id: string) {
    ...
    const lines = await this.db
      ...
      .where(eq(schema.schedulerLine.scheduler_id, id));
    ...
    const [batch] = await this.db
      .select({ batch_no: schema.batchHeader.batch_no })
      .from(schema.batchHeader)
      .where(eq(schema.batchHeader.batch_id, header.batch_id))
      .limit(1);
    const [breed] = header.breed_id
      ? await this.db
          .select({ breed_name: schema.breedMaster.breed_name })
          .from(schema.breedMaster)
          .where(eq(schema.breedMaster.breed_id, header.breed_id))
          .limit(1)
      : [null];
    const [stage] = await this.db
      .select({ stage_name: schema.stageMaster.stage_name })
      .from(schema.stageMaster)
      .where(eq(schema.stageMaster.stage_id, header.stage_id))
      .limit(1);
    const [location] = header.location_id
      ? await this.db
          .select({ location_name: schema.locationMaster.location_name })
          .from(schema.locationMaster)
          .where(eq(schema.locationMaster.location_id, header.location_id))
          .limit(1)
      : [null];
    ...
    const allItemIds = [
      ...new Set(
        lines
          .map((l) => l.item_id)
          .filter((item_id): item_id is string => !!item_id),
      ),
    ];
    const itemMap = new Map<
      string,
      { uom: string; item_name: string; withdrawal_days: number | null }
    >();
    if (allItemIds.length) {
      const items = await this.db
        ...
        .from(schema.itemMaster)
        .where(inArray(schema.itemMaster.item_id, allItemIds));
      for (const it of items) {
        itemMap.set(it.item_id, {
          uom: it.uom_primary || 'PCS',
          item_name: it.item_name,
          withdrawal_days: it.withdrawal_days,
        });
      }
    }
    ...
    const allResourceIds = [
      ...new Set(
        lines
          .map((l) => l.resource_id)
          .filter((res_id): res_id is string => !!res_id),
      ),
    ];
    ...
    if (allResourceIds.length) {
      const resources = await this.db
        ...
        .where(inArray(schema.resourceMaster.resource_id, allResourceIds));
    ...
    return {
      ...
      lines: lines.map((line) => {
        ...
        return {
          ...
          item_description:
            line.item_description || itemInfo?.item_name || null,
          withdrawal_days: itemInfo?.withdrawal_days ?? null,
          resource_name: line.resource_id
            ? (resourceMap.get(line.resource_id) ?? null)
            : null,
  ...
  async findAllForCompany(query: QuerySchedulerHeaderDto, tenantId: string) {
    ...
    if (query.companyId)
      conditions.push(eq(schema.schedulerHeader.company_id, query.companyId));
    if (query.status)
      conditions.push(
        eq(schema.schedulerHeader.scheduler_status, query.status),
      );
    if (query.lobId)
      conditions.push(eq(schema.schedulerHeader.lob_id, query.lobId));
    ...
    const breedNameById = breedIds.length
      ? new Map(
          (
            await this.db
              .select({
    ...
    const stageNameById = stageIds.length
      ? new Map(
          (
            await this.db
              .select({
    ...
    const locationIds = [
      ...new Set(
        rows.map((r) => r.location_id).filter((x): x is string => !!x),
      ),
    ];
    const locationNameById = locationIds.length
      ? new Map(
          (
            await this.db
              .select({
    ...
    const lineCounts = await this.db
      .select({
        scheduler_id: schema.schedulerLine.scheduler_id,
        total: count(),
      })
      .from(schema.schedulerLine)
      .where(
        inArray(
          ...
          rows.map((r) => r.scheduler_id),
        ),
      )
      .groupBy(schema.schedulerLine.scheduler_id);
    const lineCountById = new Map(
      lineCounts.map((l) => [l.scheduler_id, Number(l.total)]),
    );
    ...
    let result = rows.map((r) => ({
      ...
      location_name: r.location_id
        ? (locationNameById.get(r.location_id) ?? null)
        : null,
    ...
    if (query.search) {
      ...
      result = result.filter((r) =>
        (r.batch_no || '').toLowerCase().includes(needle),
      );
    }
  ...
  async updateStatus(
    id: string,
    dto: UpdateSchedulerHeaderStatusDto,
    tenantId: string,
    userPayload?: { userId?: string },
  ) {
    const header = await this.findOne(id);
    if (
      dto.scheduler_status === 'ACTIVE' &&
      header.scheduler_status === 'DRAFT' &&
      !userPayload?.userId
    ) {
      throw new BadRequestException(
        'Activating a scheduler requires an approving user.',
      );
    }
  ...
  private assertLineTypeFields(
    line_type: string,
    dto: CreateSchedulerLineDto | UpdateSchedulerLineDto,
    effective: Record<string, unknown>,
  ) {
    const spec = LINE_TYPE_FIELDS[line_type];
    if (!spec)
      throw new BadRequestException(`Unknown line_type '${line_type}'.`);
    ...
    if (missing.length) {
      throw new ConflictException(
        `${line_type} lines require: ${missing.join(', ')}.`,
      );
    }
    ...
    if (
      dto.occurrence === 'CUSTOM' &&
      !(dto.custom_days && dto.custom_days.length)
    ) {
      throw new ConflictException(
        'Occurrence CUSTOM requires at least one custom day.',
      );
    }
  ...
  private formatPeriod(line: Record<string, any>): string {
    ...
    const end =
      line.end_day !== null && line.end_day !== undefined && line.end_day !== ''
        ? `Day ${line.end_day}`
        : 'Stage close';
    ...
    if (line.occurrence === 'CUSTOM') {
      const days = Array.isArray(line.custom_days)
        ? line.custom_days
            .map((d: any) =>
              typeof d === 'object' && d !== null ? d.day_number : d,
            )
            .join(', ')
        : line.custom_days || '';
  ...
  private doLinesTargetSameSubject(
    a: Record<string, any>,
    b: Record<string, any>,
  ): boolean {
    ...
    if (type === 'CONSUMPTION') {
      ...
      return (
        !!a.activity_name &&
        a.activity_name.trim().toLowerCase() ===
          b.activity_name?.trim().toLowerCase()
      );
    }
    if (type === 'OUTPUT') {
      if (a.item_id && b.item_id) return a.item_id === b.item_id;
    ...
    if (type === 'DESCRIPTIVE') {
      if (a.kpi_metric && b.kpi_metric)
        return (
          a.kpi_metric.trim().toLowerCase() ===
          b.kpi_metric.trim().toLowerCase()
        );
      return (
        !!a.activity_name &&
        a.activity_name.trim().toLowerCase() ===
    ...
    if (type === 'OVERHEAD') {
      if (a.overhead_category && b.overhead_category)
        return (
          a.overhead_category.trim().toLowerCase() ===
          b.overhead_category.trim().toLowerCase()
        );
      return (
        !!a.activity_name &&
        a.activity_name.trim().toLowerCase() ===
  ...
  private parseCustomDays(raw: unknown): number[] {
    if (Array.isArray(raw)) {
      return raw
        .map((d) =>
          typeof d === 'object' && d !== null && 'day_number' in d
            ? Number((d as any).day_number)
            : Number(d),
        )
  ...
  private checkDaysOverlap(
    ...
    const customA =
      occA === 'CUSTOM' ? this.parseCustomDays(a.custom_days) : [];
    const customB =
      occB === 'CUSTOM' ? this.parseCustomDays(b.custom_days) : [];
    ...
    if (occA === 'CUSTOM' && occB === 'ONCE') {
      return customA.length
        ? customA.includes(startB)
        : startB >= startA && startB <= endA;
    }
    if (occB === 'CUSTOM' && occA === 'ONCE') {
      return customB.length
        ? customB.includes(startA)
  ...
  private resolveKpiUom(
    kpiMetric: string | null | undefined,
    suppliedUom: string | null | undefined,
  ): string | null {
    if (kpiMetric && KPI_METRIC_UOM[kpiMetric])
      return KPI_METRIC_UOM[kpiMetric];
    return suppliedUom ?? null;
  }

  async addLine(
    headerId: string,
    dto: CreateSchedulerLineDto,
    tenantId: string,
    userPayload?: { userId?: string },
  ) {
    ...
    const maxSeq = header.lines.reduce(
      (max, l) => Math.max(max, l.line_seq),
      0,
    );
    const lineId = randomUUID();
    await this.db.insert(schema.schedulerLine).values({
  ...
  async updateLine(
    ...
    if (!line)
      throw new NotFoundException(
        `Line '${lineId}' not found on this scheduler.`,
      );
    ...
    if ('kpi_metric' in updates || 'kpi_uom' in updates) {
      updates.kpi_uom = this.resolveKpiUom(
        (updates.kpi_metric as string) ?? line.kpi_metric,
        (updates.kpi_uom as string) ?? line.kpi_uom,
      );
    }

    if (Object.keys(updates).length) {
      await this.db
        .update(schema.schedulerLine)
        .set(updates)
    ...
    if (dto.custom_days) {
      await this.db
        ...
        .where(eq(schema.schedulerLineCustomDays.line_id, lineId));
