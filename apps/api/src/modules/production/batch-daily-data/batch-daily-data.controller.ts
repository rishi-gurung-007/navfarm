...
export class BatchDailyDataController {
  ...
  @ApiOperation({
    summary:
      'Post one scheduler_line entry for a date — dispatches to inventory/GL/animal-transfer per line_type, same day+line upserts',
  })
  @ApiParam({ name: 'batchId', description: 'Batch UUID' })
  async postEntry(
    @Param('batchId') batchId: string,
    @Body() dto: CreateBatchDailyDataDto,
    @Req() req: any,
  ) {
    const tenantId = req.user?.tenantId || req['tenantId'];
    const result = await this.batchDailyDataService.postEntry(
      batchId,
      dto,
      tenantId,
      req.user,
  ...
  @ApiOperation({
    summary: 'List entries already recorded for this batch on a given date',
  })
  @ApiParam({ name: 'batchId', description: 'Batch UUID' })
  ...
  async findForDate(
    ...
    const tenantId = req.user?.tenantId || req['tenantId'];
    const result = await this.batchDailyDataService.findForDate(
      batchId,
      date,
      tenantId,
    );
