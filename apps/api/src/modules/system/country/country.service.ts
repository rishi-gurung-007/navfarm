...
export class CountryService {
  ...
  async listCountries(query: QueryCountryDto = {}) {
    ...
    conditions.push(
      ...listFilterConditions(schema.countryMaster, query.filter),
    );
    return runMasterList(
      this.db,
      schema.countryMaster,
      conditions,
      query,
      schema.countryMaster.iso2,
    );
  }
  ...
  async createCountry(data: any) {
    ...
    await this.db
      .insert(schema.countryMaster)
      .values({ ...data, country_id: countryId });
    const [newCountry] = await this.db
      .select()
      .from(schema.countryMaster)
      .where(eq(schema.countryMaster.country_id, countryId))
      .limit(1);
    return newCountry;
  }
  ...
  async updateCountry(id: string, data: any) {
    await this.db
      .update(schema.countryMaster)
      .set(data)
      .where(eq(schema.countryMaster.country_id, id));
  ...
  async listStates(countryId: string) {
    ...
    return this.db
      ...
      .where(
        and(
          eq(schema.stateProvince.country_id, countryId),
          eq(schema.stateProvince.is_active, true),
        ),
      );
  }
  ...
  async createState(countryId: string, data: any) {
    ...
    await this.db
      .insert(schema.stateProvince)
      .values({ ...data, state_id: stateId, country_id: countryId });
    const [newState] = await this.db
      .select()
      .from(schema.stateProvince)
      .where(eq(schema.stateProvince.state_id, stateId))
      .limit(1);
    return newState;
  }
