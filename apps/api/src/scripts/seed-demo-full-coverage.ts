...
export async function seedFullCoverage() {
  ...
  try {
    ...
    if (!comp1)
      throw new Error('No company found — run db-seed-dev-tenant first.');
    ...
    const [c1Admin] = await db
      ...
      .limit(1);
    if (!tAdmin)
      throw new Error('No TENANT_ADMIN found — run db-seed-dev-tenant first.');
    ...
    const [lob] = await db
      ...
      .limit(1);
    if (!nob || !lob)
      throw new Error('NOB LIVESTOCK / LOB LVS_PIGGERY not found.');
    ...
    if (!area1)
      throw new Error(
        'No operational area found — run db-seed-dev-tenant first.',
      );

    const batches1 = await db
      .select()
      .from(schema.batchHeader)
      .where(eq(schema.batchHeader.company_id, comp1Id));
    const batches2 = await db
      .select()
      .from(schema.batchHeader)
      .where(eq(schema.batchHeader.company_id, comp2Id));
    ...
    if (!batch1a || !batch1b || !batch2a || !batch2b)
      throw new Error('Expected batches not found.');
    ...
    const [warehouse1] = await db
      ...
      .where(
        and(
          eq(schema.locationMaster.company_id, comp1Id),
          inArray(schema.locationMaster.location_type, ['STORE', 'SILO']),
        ),
      )
      .limit(1);
    const [warehouse2] = await db
      ...
      .where(
        and(
          eq(schema.locationMaster.company_id, comp2Id),
          inArray(schema.locationMaster.location_type, ['STORE', 'SILO']),
        ),
      )
      .limit(1);
    ...
    const glMap1 = new Map(
      glAll
        .filter((g) => g.company_id === comp1Id)
        .map((g) => [g.account_code, g.gl_account_id]),
    );
    const glMap2 = new Map(
      glAll
        .filter((g) => g.company_id === comp2Id)
        .map((g) => [g.account_code, g.gl_account_id]),
    );

    const costCentersAll = await db.select().from(schema.costCenterMaster);
    const ccMap1 = new Map(
      costCentersAll
        .filter((c) => c.company_id === comp1Id)
        .map((c) => [c.cost_center_code, c.cost_center_id]),
    );
    const ccMap2 = new Map(
      costCentersAll
        .filter((c) => c.company_id === comp2Id)
        .map((c) => [c.cost_center_code, c.cost_center_id]),
    );

    console.log(
      `Context: Tenant ${tenantId} — APEXBREED (${comp1Id}) / HIGHLAND (${comp2Id})`,
    );
    ...
    await run('Section 1: HIGHLAND animal register', async () => {
      const existing = await db
        ...
        .where(eq(schema.animalRegister.company_id, comp2Id));
      ...
      if (!piglet || !finisher)
        throw new Error('HIGHLAND bio items not found.');
      ...
      const animalCfgs = [
        ...
        },
        {
          ...
          gender: 'M',
          tag: 'GRW-DR-104',
          rfid: '982000412881104',
          cost: '7800.0000',
          item: growPen ? finisher : piglet,
          loc: growPen,
        },
        {
          code: 'PIG-2026-0105',
      ...
      for (const a of animalCfgs) {
        ...
        const row: typeof schema.animalRegister.$inferInsert = {
          ...
          current_batch_id:
            a.type === 'PIGLET' ? batch2a!.batch_id : batch2a!.batch_id,
          status: 'ACTIVE',
          is_active: true,
    ...
    await run('Section 2: Batch transfer (APEXBREED)', async () => {
      const existing = await db
        ...
        .where(eq(schema.batchTransfer.company_id, comp1Id))
        .limit(1);
      if (existing.length > 0) {
        console.log('  transfer already exists, skipping.');
        return;
      }
      const gilts = await db
        .select()
        .from(schema.animalRegister)
        .where(
          and(
            eq(schema.animalRegister.company_id, comp1Id),
            eq(schema.animalRegister.animal_type, 'GILT'),
          ),
        );
      const moving = gilts.slice(0, 2);
      if (moving.length === 0)
        throw new Error('No GILT animals found on APEXBREED to transfer.');

      for (const g of moving) {
        await db
          .update(schema.animalRegister)
          .set({ current_batch_id: batch1a!.batch_id })
          .where(eq(schema.animalRegister.animal_id, g.animal_id));
      }
      ...
      await db.insert(schema.batchTransfer).values({
        ...
        transfer_value: d4(
          moving.reduce(
            (s, a) => s + Number(a.book_value || a.acquisition_cost || 0),
            0,
          ),
        ),
        reason: 'STAGE_PROGRESSION',
        remarks:
          'Gilts moved from gestation cohort into farrowing/lactation cohort.',
        status: 'POSTED',
        posted_at: tsMinus(5),
      ...
      for (const g of moving) {
        ...
        await db
          .update(schema.animalRegister)
          .set({ current_batch_id: batch1b!.batch_id })
          .where(eq(schema.animalRegister.animal_id, g.animal_id));
      }
      console.log(
        `  transferred ${moving.length} animals ${batch1a!.batch_no} -> ${batch1b!.batch_no}.`,
      );
    });
    ...
    for (const [compId, admin, areaId, tag] of [
      ...
      await run(
        `Section 3: FARM_SUPERVISOR role + operational admin (${tag})`,
        async () => {
          const [role] = await db
            .select()
            ...
            .where(
              and(
                eq(schema.roleMaster.company_id, compId),
                eq(schema.roleMaster.role_code, 'FARM_SUPERVISOR'),
              ),
            )
            .limit(1);
          ...
          if (!role) {
            ...
            await db.insert(schema.roleMaster).values({
              ...
              role_description:
                'Day-to-day farm supervision — batch/feed operations plus visibility into settings',
          ...
          if (!existingUser) {
            ...
            await db.insert(schema.userMaster).values({
              ...
              full_name:
                tag === 'apex'
                  ? 'Meera Nair (Farm Supervisor)'
                  : 'Suresh Rathi (Farm Supervisor)',
              email,
            ...
            console.log(
              `  created ${email} (OPERATIONAL_ADMIN, FARM_SUPERVISOR).`,
            );
          } else {
    ...
    // ═══════════════════════════════════════════════════════════════════
    for (const [compId, admin] of [
      [comp1Id, c1Admin],
      [comp2Id, c2Admin],
    ] as const) {
      await run(
        `Section 3.5: Default roles + admin assignment (${compId === comp1Id ? 'apex' : 'highland'})`,
        async () => {
          const existingDefaults = await db
            ...
            .where(
              and(
                eq(schema.roleMaster.company_id, compId),
                eq(schema.roleMaster.is_system_role, true),
              ),
            )
            .limit(1);
          ...
          for (const u of [tAdmin, admin]) {
            const [existingAssign] = await db
              ...
              .where(eq(schema.userRoleAssignment.user_id, u.user_id))
              .limit(1);
            if (!existingAssign) {
              await db.insert(schema.userRoleAssignment).values({
                assign_id: randomUUID(),
    ...
    async function postJournal(
      ...
      if (!debitId || !creditId)
        throw new Error(`GL accounts ${debitCode}/${creditCode} not found.`);
      ...
      await db.insert(schema.journalLine).values([
        {
          ...
          journal_id: journalId,
          line_no: 1,
          gl_account_id: debitId,
          debit_amount: d4(amount),
        ...
        },
        {
          ...
          journal_id: journalId,
          line_no: 2,
          gl_account_id: creditId,
          debit_amount: '0.0000',
          credit_amount: d4(amount),
          description,
          nob_id: nobId,
          lob_id: lobId,
        },
    ...
    for (const cfg of [
      {
        ...
        batch: batch1a,
        tag: 'APX',
        rawCode: 'RAW-MAIZE-CORN',
        feedCode: 'FEED-GEST-SOW',
        medCode: 'MED-IRON-DEX',
      },
      {
        compId: comp2Id,
        admin: c2Admin,
        ...
        tag: 'HGH',
        rawCode: 'RAW-MAIZE-CORN',
        feedCode: 'FEED-WEAN-GROW',
        medCode: 'MED-IVERMECTIN',
      },
    ] as const) {
      await run(`Section 4/5: Inventory + finance (${cfg.tag})`, async () => {
        const existing = await db
          ...
          .where(eq(schema.goodsReceipt.company_id, cfg.compId))
          .limit(1);
        if (existing.length > 0) {
        ...
        if (!rawItem || !feedItem || !medItem)
          throw new Error(`Items not found for ${cfg.tag}.`);

        // Second small warehouse so a transfer has a real destination.
        const feedmillCode = `WH-${cfg.tag}-FEEDMILL`;
        const [feedmillWh] = await db
          .select()
          .from(schema.locationMaster)
          .where(
            and(
              eq(schema.locationMaster.company_id, cfg.compId),
              eq(schema.locationMaster.location_code, feedmillCode),
            ),
          )
          .limit(1);
        ...
        if (!feedmillWh) {
          ...
          await db.insert(schema.locationMaster).values({
            ...
            location_code:
              (await seriesCodeFor(
                db,
                { tenantId, companyId: cfg.compId },
                'LOCATION',
                {
                  parent_location_id: farmLoc?.location_code ?? null,
                  location_type: 'STORE',
                },
                (
                  await db
                    ...
                    .from(schema.locationMaster)
                    .where(eq(schema.locationMaster.company_id, cfg.compId))
        ...
        await db.insert(schema.goodsReceipt).values({
          ...
          tenant_id: tenantId,
          company_id: cfg.compId,
          receipt_no: `GRN-2026-${cfg.tag}-0001`,
          posting_date: todayMinus(20),
          warehouse_id: cfg.warehouse.warehouse_id,
          remarks: 'Bulk maize procurement',
          status: 'POSTED',
          posted_at: tsMinus(20),
          posted_by: cfg.admin.user_id,
          created_by: cfg.admin.user_id,
        });
        ...
        await db.insert(schema.goodsReceiptLine).values({
          ...
          quantity: d4(rawQty),
          uom: rawItem.uom_primary,
          rate: rawRate.toFixed(6),
          amount: d4(rawQty * rawRate),
        });
        const grn1LedgerId = randomUUID();
        await db.insert(schema.inventoryLedger).values({
          ledger_id: grn1LedgerId,
          tenant_id: tenantId,
          company_id: cfg.compId,
          item_id: rawItem.item_id,
          item_code: rawItem.item_code,
          item_description: rawItem.item_name,
          document_type: 'GOODS_RECEIPT',
          document_no: `GRN-2026-${cfg.tag}-0001`,
          document_line_id: grn1LineId,
          posting_date: todayMinus(20),
          entry_type: 'POSITIVE',
          transaction_type: 'PURCHASE',
          quantity: d4(rawQty),
          remaining_quantity: d4(rawQty),
          uom: rawItem.uom_primary,
          rate: rawRate.toFixed(6),
          amount: d4(rawQty * rawRate),
          warehouse_id: cfg.warehouse.warehouse_id,
        ...
        await postJournal(
          cfg.compId,
          cfg.glMap,
          `JV-2026-${cfg.tag}-0001`,
          todayMinus(20),
          'GOODS_RECEIPT',
          `GRN-2026-${cfg.tag}-0001`,
          grn1LedgerId,
          '1010',
          '2010',
          rawQty * rawRate,
          `Maize procurement — ${rawItem.item_name}`,
          cfg.admin.user_id,
        );
        ...
        await db.insert(schema.goodsReceipt).values({
          ...
          tenant_id: tenantId,
          company_id: cfg.compId,
          receipt_no: `GRN-2026-${cfg.tag}-0002`,
          posting_date: todayMinus(18),
          warehouse_id: cfg.warehouse.warehouse_id,
          remarks: 'Compound feed procurement',
          status: 'POSTED',
          posted_at: tsMinus(18),
          posted_by: cfg.admin.user_id,
          created_by: cfg.admin.user_id,
        });
        ...
        await db.insert(schema.goodsReceiptLine).values({
          ...
          quantity: d4(feedQty),
          uom: feedItem.uom_primary,
          rate: feedRate.toFixed(6),
          amount: d4(feedQty * feedRate),
        });
        const grn2LedgerId = randomUUID();
        await db.insert(schema.inventoryLedger).values({
          ledger_id: grn2LedgerId,
          tenant_id: tenantId,
          company_id: cfg.compId,
          item_id: feedItem.item_id,
          item_code: feedItem.item_code,
          item_description: feedItem.item_name,
          document_type: 'GOODS_RECEIPT',
          document_no: `GRN-2026-${cfg.tag}-0002`,
          document_line_id: grn2LineId,
          posting_date: todayMinus(18),
          entry_type: 'POSITIVE',
          transaction_type: 'PURCHASE',
          quantity: d4(feedQty),
          remaining_quantity: d4(feedQty),
          uom: feedItem.uom_primary,
          rate: feedRate.toFixed(6),
          amount: d4(feedQty * feedRate),
          warehouse_id: cfg.warehouse.warehouse_id,
          nob_id: nobId,
          lob_id: lobId,
          category_id: feedItem.category_id,
          created_by: cfg.admin.user_id,
        });
        await postJournal(
          cfg.compId,
          cfg.glMap,
          `JV-2026-${cfg.tag}-0002`,
          todayMinus(18),
          'GOODS_RECEIPT',
          `GRN-2026-${cfg.tag}-0002`,
          grn2LedgerId,
          '1010',
          '2010',
          feedQty * feedRate,
          `Feed procurement — ${feedItem.item_name}`,
          cfg.admin.user_id,
        );
        ...
        const costCenterId = cfg.ccMap.get(
          cfg.tag === 'APX' ? 'CC-FEEDMILL-A' : 'CC-FEEDMILL-H',
        );
        await db.insert(schema.goodsIssue).values({
          issue_id: issueId,
          tenant_id: tenantId,
          company_id: cfg.compId,
          issue_no: `ISS-2026-${cfg.tag}-0001`,
          posting_date: todayMinus(14),
          warehouse_id: cfg.warehouse.warehouse_id,
          cost_center_id: costCenterId,
          remarks: 'Issued to feed mill for ration mixing',
          status: 'POSTED',
          posted_at: tsMinus(14),
          posted_by: cfg.admin.user_id,
          created_by: cfg.admin.user_id,
        });
        await db.insert(schema.goodsIssueLine).values({
          line_id: randomUUID(),
          issue_id: issueId,
          line_no: 1,
          item_id: rawItem.item_id,
          quantity: d4(issueQty),
          uom: rawItem.uom_primary,
        });
        const issueLedgerId = randomUUID();
        await db.insert(schema.inventoryLedger).values({
          ledger_id: issueLedgerId,
          tenant_id: tenantId,
          company_id: cfg.compId,
          item_id: rawItem.item_id,
          item_code: rawItem.item_code,
          ...
          document_no: `ISS-2026-${cfg.tag}-0001`,
          posting_date: todayMinus(14),
          entry_type: 'NEGATIVE',
        ...
        await db.insert(schema.inventoryApplication).values({
          ...
          company_id: cfg.compId,
          item_id: rawItem.item_id,
          inbound_ledger_id: grn1LedgerId,
          outbound_ledger_id: issueLedgerId,
          applied_qty: d4(issueQty),
          applied_cost_amount: d4(issueQty * rawRate),
          application_date: todayMinus(14),
          created_by: cfg.admin.user_id,
        });
        await db
          .update(schema.inventoryLedger)
          .set({ remaining_quantity: d4(rawQty - issueQty) })
          .where(eq(schema.inventoryLedger.ledger_id, grn1LedgerId));
        await postJournal(
          cfg.compId,
          cfg.glMap,
          `JV-2026-${cfg.tag}-0003`,
          todayMinus(14),
          'GOODS_ISSUE',
          `ISS-2026-${cfg.tag}-0001`,
          issueLedgerId,
          '5020',
          '1010',
          issueQty * rawRate,
          `Feed mill issue — ${rawItem.item_name}`,
          cfg.admin.user_id,
        );
        ...
        await db.insert(schema.stockTransfer).values({
          transfer_id: stTransferId,
          tenant_id: tenantId,
          company_id: cfg.compId,
          transfer_no: `TRF-2026-${cfg.tag}-0001`,
          posting_date: todayMinus(10),
          from_warehouse_id: cfg.warehouse.warehouse_id,
          to_warehouse_id: feedmillWhId!,
          remarks: 'Pre-positioning stock at feed mill store',
          status: 'POSTED',
          posted_at: tsMinus(10),
          posted_by: cfg.admin.user_id,
          created_by: cfg.admin.user_id,
        });
        await db.insert(schema.stockTransferLine).values({
          line_id: randomUUID(),
          transfer_id: stTransferId,
          line_no: 1,
          item_id: rawItem.item_id,
          quantity: d4(transferQty),
          uom: rawItem.uom_primary,
        });
        const shipLedgerId = randomUUID();
        await db.insert(schema.inventoryLedger).values({
          ledger_id: shipLedgerId,
          tenant_id: tenantId,
          company_id: cfg.compId,
          item_id: rawItem.item_id,
          item_code: rawItem.item_code,
          ...
          document_no: `TRF-2026-${cfg.tag}-0001`,
          posting_date: todayMinus(10),
          entry_type: 'NEGATIVE',
        ...
        await db.insert(schema.inventoryApplication).values({
          ...
          item_id: rawItem.item_id,
          inbound_ledger_id: grn1LedgerId,
          outbound_ledger_id: shipLedgerId,
          applied_qty: d4(transferQty),
          applied_cost_amount: d4(transferQty * rawRate),
          application_date: todayMinus(10),
          created_by: cfg.admin.user_id,
        });
        await db
          .update(schema.inventoryLedger)
          .set({ remaining_quantity: d4(remainingAfterIssue - transferQty) })
          .where(eq(schema.inventoryLedger.ledger_id, grn1LedgerId));
        const receiveLedgerId = randomUUID();
        await db.insert(schema.inventoryLedger).values({
          ledger_id: receiveLedgerId,
          tenant_id: tenantId,
          company_id: cfg.compId,
          item_id: rawItem.item_id,
          item_code: rawItem.item_code,
          ...
          quantity: d4(transferQty),
          remaining_quantity: d4(transferQty),
          uom: rawItem.uom_primary,
          rate: rawRate.toFixed(6),
          amount: d4(transferQty * rawRate),
        ...
        await postJournal(
          cfg.compId,
          cfg.glMap,
          `JV-2026-${cfg.tag}-0004`,
          todayMinus(10),
          'TRANSFER',
          `TRF-2026-${cfg.tag}-0001`,
          shipLedgerId,
          '1040',
          '1010',
          transferQty * rawRate,
          'In-transit transfer to feed mill store',
          cfg.admin.user_id,
        );
        ...
        await db.insert(schema.stockAdjustment).values({
          ...
          tenant_id: tenantId,
          company_id: cfg.compId,
          adjustment_no: `ADJ-2026-${cfg.tag}-0001`,
          posting_date: todayMinus(7),
          warehouse_id: cfg.warehouse.warehouse_id,
          reason: 'Physical count variance — found unrecorded stock',
          status: 'POSTED',
          posted_at: tsMinus(7),
          posted_by: cfg.admin.user_id,
          created_by: cfg.admin.user_id,
        });
        await db.insert(schema.stockAdjustmentLine).values({
          line_id: randomUUID(),
          adjustment_id: adjId,
          line_no: 1,
          item_id: medItem.item_id,
          quantity: d4(adjQty),
          uom: medItem.uom_primary,
          rate: medRate.toFixed(6),
        });
        const adjLedgerId = randomUUID();
        await db.insert(schema.inventoryLedger).values({
          ledger_id: adjLedgerId,
          tenant_id: tenantId,
          company_id: cfg.compId,
          item_id: medItem.item_id,
          item_code: medItem.item_code,
          item_description: medItem.item_name,
          document_type: 'ADJUSTMENT',
          document_no: `ADJ-2026-${cfg.tag}-0001`,
          posting_date: todayMinus(7),
          entry_type: 'POSITIVE',
          transaction_type: 'VARIANCE_POSITIVE',
          quantity: d4(adjQty),
          remaining_quantity: d4(adjQty),
          uom: medItem.uom_primary,
          rate: medRate.toFixed(6),
          amount: d4(adjQty * medRate),
          warehouse_id: cfg.warehouse.warehouse_id,
          nob_id: nobId,
          lob_id: lobId,
          category_id: medItem.category_id,
          created_by: cfg.admin.user_id,
        });
        await postJournal(
          cfg.compId,
          cfg.glMap,
          `JV-2026-${cfg.tag}-0005`,
          todayMinus(7),
          'ADJUSTMENT',
          `ADJ-2026-${cfg.tag}-0001`,
          adjLedgerId,
          '1010',
          '4030',
          adjQty * medRate,
          `Physical count variance — ${medItem.item_name}`,
          cfg.admin.user_id,
        );

        console.log(
          '  2 GRNs, 1 issue (FIFO-applied), 1 transfer (FIFO-applied), 1 adjustment, 5 journals posted.',
        );
      });
    }
    ...
    for (const cfg of [
      ...
      await run(`Section 6/7: QC + QR (${cfg.tag})`, async () => {
        const existing = await db
          ...
          .where(eq(schema.qcBatchDetail.company_id, cfg.compId))
          .limit(1);
        if (existing.length > 0) {
        ...
        const [param] = await db
          ...
          .where(and(eq(schema.qcParameterMaster.lob_id, lobId)))
          .limit(1);
        ...
        if (!param) {
          ...
          await db.insert(schema.qcParameterMaster).values({
            ...
            company_id: null,
            lob_id: lobId,
            param_code: 'QC-CARCASS-WT',
            param_name: 'Carcass Weight',
            param_type: 'NUMERIC',
            uom: 'KG',
            min_value: '60.0000',
            max_value: '110.0000',
            pass_criteria: 'Within range',
            fail_criteria: 'Below 60kg or above 110kg',
            is_mandatory: true,
            is_active: true,
            created_by: cfg.admin.user_id,
          });
          ...
          await db.insert(schema.qcParameterMaster).values({
            ...
            company_id: null,
            lob_id: lobId,
            param_code: 'QC-VISUAL-GRADE',
            param_name: 'Visual Grade',
            param_type: 'GRADE',
            grade_scale: { A: 'Premium', B: 'Standard', C: 'Economy' },
            is_mandatory: false,
            is_active: true,
            created_by: cfg.admin.user_id,
          });
        ...
        await db.insert(schema.qcBatchDetail).values({
          ...
          tenant_id: tenantId,
          company_id: cfg.compId,
          source_batch_id: cfg.batch.batch_id,
          qc_date: todayMinus(3),
          inspector_id: cfg.admin.user_id,
          total_qty_received: '20.0000',
          pass_qty: '18.0000',
          fail_qty: '1.0000',
          hold_qty: '1.0000',
          grade_a_qty: '12.0000',
          ...
          overall_result: 'PASS',
          disposition: 'ACCEPT',
          qc_notes: 'Routine batch inspection — within tolerance.',
          created_by: cfg.admin.user_id,
        });
        await db.insert(schema.qcParamResult).values({
        ...
        await db.insert(schema.qcBatchDetail).values({
          ...
          company_id: cfg.compId,
          source_batch_id: cfg.batch.batch_id,
          qc_date: todayMinus(1),
          inspector_id: cfg.admin.user_id,
          total_qty_received: '8.0000',
          pass_qty: '5.0000',
          fail_qty: '3.0000',
          hold_qty: '0.0000',
          grade_a_qty: '2.0000',
          ...
          disposition: 'REWORK',
          qc_notes:
            'Weight variance above threshold on 3 units — sent for rework/regrading.',
          created_by: cfg.admin.user_id,
        });
        await db.insert(schema.qcParamResult).values({
          result_id: randomUUID(),
        ...
        const bioItem =
          cfg.itemMap.get(cfg.bioCode) || Array.from(cfg.itemMap.values())[0];
        ...
        await db.insert(schema.qrCodeMaster).values({
          ...
          qc_id: qc1Id,
          item_id: bioItem.item_id,
          lot_no: `LOT-${cfg.tag}-2026-08`,
          pack_no: packNo,
          production_date: todayMinus(3),
          expiry_date: todayMinus(-90),
          net_weight: '92.5000',
          gross_weight: '95.0000',
          pack_uom: 'KG',
          warehouse_id: cfg.warehouse.warehouse_id,
          grade: 'A',
          breed: cfg.breed,
          generated_by: cfg.admin.user_id,
          qr_data: {
            pack_no: packNo,
            item_code: bioItem.item_code,
            item_name: bioItem.item_name,
            lot_no: `LOT-${cfg.tag}-2026-08`,
            batch_no: cfg.batch.batch_no,
            net_weight: '92.5000',
            gross_weight: '95.0000',
            pack_uom: 'KG',
            production_date: todayMinus(3),
            expiry_date: todayMinus(-90),
            facility_code: cfg.tag,
            breed: cfg.breed,
            grade: 'A',
            origin_batch_chain: [{ batch_no: cfg.batch.batch_no }],
            qc: {
              qc_id: qc1Id,
              overall_result: 'PASS',
              disposition: 'ACCEPT',
    ...
    for (const [compId, batch, tag] of [
      ...
      await run(`Section 8: Notifications (${tag})`, async () => {
        const existing = await db
          ...
          .where(eq(schema.notificationAlertLog.company_id, compId))
          .limit(1);
        if (existing.length > 0) {
        ...
        const alerts = [
          {
            severity: 'WARNING',
            title: 'Feed consumption above KPI band',
            message: `Batch ${batch!.batch_no}: daily feed consumption exceeded the scheduled band by 12%.`,
            param: 'Feed Consumption',
            expected: '2.20',
            actual: '2.47',
            devPct: '12.27',
            isRead: true,
          ...
          },
          {
            ...
            message: `Batch ${batch!.batch_no}: mortality rate crossed the critical threshold for the past 3 days.`,
          ...
          {
            severity: 'WARNING',
            title: 'Water intake below expected range',
            message: `Batch ${batch!.batch_no}: water intake trending below the expected daily range.`,
            param: 'Water Intake',
            expected: '15.00',
            actual: '11.20',
            devPct: '-25.33',
        ...
        for (const a of alerts) {
          await db.insert(schema.notificationAlertLog).values({
            ...
            title: a.title,
            message: a.message,
            activity_name: a.param,
            kpi_mode: 'BAND',
            expected_value: a.expected,
            actual_value: a.actual,
            deviation_pct: a.devPct,
            is_read: a.isRead,
            read_at: a.isRead ? tsMinus(a.daysAgo - 1) : null,
            created_at: tsMinus(a.daysAgo),
          });
        }
    ...
    for (const cfg of [
      ...
      await run(`Section 9: Approvals (${cfg.tag})`, async () => {
        const existing = await db
          ...
          .where(eq(schema.approvalRequest.company_id, cfg.compId))
          .limit(1);
        if (existing.length > 0) {
          console.log('  already seeded, skipping.');
          return;
        ...
        const reqs = [
          {
            ...
            doc_no: `REQ-RAT-2026-${cfg.tag}-0001`,
            title: 'Increase gestation ration by 5%',
            urgency: 'MEDIUM',
            status: 'PENDING',
            qty: '2.31',
            uom: 'KG',
            cost: '5040.00',
            justification:
              'Body condition scoring indicates under-conditioned sows this cycle.',
          },
          {
            doc_type: 'GRN_RECEIPT',
            doc_no: `REQ-GRN-2026-${cfg.tag}-0001`,
            title: 'Approve emergency feed procurement',
            urgency: 'HIGH',
            status: 'APPROVED',
            qty: '1000',
            uom: 'KG',
            cost: '31000.00',
            justification:
              'Stock projected to run out before next scheduled delivery.',
          },
          {
            doc_type: 'STOCK_TRANSFER',
            doc_no: `REQ-TRF-2026-${cfg.tag}-0001`,
            title: 'Transfer surplus feed to feed mill store',
            urgency: 'LOW',
            status: 'REJECTED',
            qty: '500',
            uom: 'KG',
            cost: '11000.00',
            justification: 'Requested to consolidate stock ahead of audit.',
            rejectReason:
              'Feed mill store already at capacity — hold until next cycle.',
          },
        ];
        let i = 0;
        for (const r of reqs) {
          ...
          await db.insert(schema.approvalRequest).values({
            ...
            tenant_id: tenantId,
            company_id: cfg.compId,
            operational_area_id: cfg.areaId,
            doc_type: r.doc_type,
            doc_no: r.doc_no,
            title: r.title,
            requested_by: cfg.admin.user_id,
            requestor_label: cfg.admin.full_name,
            requestor_role: 'Company Admin',
            location_label: cfg.batch.batch_no,
            batch_id: cfg.batch.batch_id,
            urgency: r.urgency,
            item_or_stage: r.title,
            requested_qty: r.qty,
            uom: r.uom,
            cost_impact: r.cost,
            justification: r.justification,
            status: r.status,
            submitted_at: tsMinus(8 - i),
            decided_at: decided ? tsMinus(6 - i) : null,
            decided_by: decided ? cfg.admin.user_id : null,
            decider_label: decided ? cfg.admin.full_name : null,
            rejection_reason: (r as any).rejectReason || null,
            created_by: cfg.admin.user_id,
          });
        }
        console.log(
          `  ${reqs.length} approval requests seeded (pending/approved/rejected).`,
        );
      });
    }
    ...
    // ═══════════════════════════════════════════════════════════════════
    for (const cfg of [
      { compId: comp1Id, admin: c1Admin, batch: batch1a!, tag: 'APX' },
      { compId: comp2Id, admin: c2Admin, batch: batch2a!, tag: 'HGH' },
    ] as const) {
      await run(`Section 10: Audit log (${cfg.tag})`, async () => {
        const existing = await db
          .select()
          .from(schema.auditLog)
          .where(eq(schema.auditLog.company_id, cfg.compId))
          .limit(1);
        if (existing.length > 0) {
          console.log('  already seeded, skipping.');
          return;
        ...
        const entries: {
        ...
        }[] = [
          {
            action: 'CREATE',
            entity: 'batch_header',
            entityId: cfg.batch.batch_id,
            daysAgo: 20,
            newValues: { batch_no: cfg.batch.batch_no, status: 'ACTIVE' },
          },
          {
            action: 'CREATE',
          ...
          {
            action: 'UPDATE',
            entity: 'batch_transfer',
            entityId: cfg.batch.batch_id,
            daysAgo: 5,
          },
          {
            action: 'CREATE',
            entity: 'qc_batch_detail',
            entityId: cfg.batch.batch_id,
            daysAgo: 3,
          },
          {
            action: 'CREATE',
            entity: 'qr_code_master',
            entityId: cfg.batch.batch_id,
            daysAgo: 3,
          },
          {
        ...
        for (const e of entries) {
          await db.insert(schema.auditLog).values({
            ...
            tenant_id: tenantId,
            company_id: cfg.compId,
            user_id: cfg.admin.user_id,
            action: e.action,
            entity_name: e.entity,
            entity_id: e.entityId,
            new_values: e.newValues || null,
            created_at: tsMinus(e.daysAgo),
          });
        }
