...
import {
  ...
  STARTER_GL_MAPPINGS,
  STARTER_WAREHOUSE,
} from '../modules/system/setup-wizard/seed/starter-master-data.seed-data';
...
const DEV_USERS = [
  {
    ...
    email: 'tenant.admin@triplec.local',
    scope: 'every company in the tenant',
  },
  {
    key: 'company',
    type: 'COMPANY_ADMIN',
    name: 'Company Administrator',
    email: 'company.admin@triplec.local',
    scope: 'one company',
  },
  {
    key: 'area',
    type: 'OPERATIONAL_ADMIN',
    name: 'Operational Administrator',
    email: 'area.admin@triplec.local',
    scope: 'one operational area',
  },
  {
    key: 'standard',
    type: 'STANDARD_USER',
    name: 'Standard User',
    email: 'user@triplec.local',
    scope: 'no admin rights',
  },
] as const;
const userBy = (key: (typeof DEV_USERS)[number]['key']) =>
  DEV_USERS.find((u) => u.key === key)!;
...
export async function seedDevTenant() {
  if (!/^[a-z0-9-]{3,30}$/.test(tenantCode)) {
    throw new Error(
      `DEV_TENANT_CODE '${tenantCode}' must be 3-30 lowercase letters/numbers/hyphens.`,
    );
  }
  if (DEV_PASSWORD.length < 8)
    throw new Error('DEV_PASSWORD must be at least 8 characters.');
  ...
  try {
    const [platformNob] = await masterDb
      .select()
      .from(master.nobMaster)
      .limit(1);
    if (!platformNob) {
      throw new Error(
        'Platform is not bootstrapped yet — run `pnpm nx run api:db-bootstrap` first.',
      );
    }

    const [plan] = await masterDb
      .select()
      .from(master.planMaster)
      .where(eq(master.planMaster.plan_id, 'PLAN_PRO'))
      .limit(1);
    if (!plan)
      throw new Error("Plan 'PLAN_PRO' not found — run db-bootstrap first.");
    ...
    const defaultPrefix = masterDatabase.startsWith('piggery_')
      ? 'piggery_tenant_'
      : 'tenant_';
    const dbName = assertDatabaseName(
      existingTenant?.db_name || `${defaultPrefix}${tenantCode}`,
    );
    ...
    try {
      ...
      const masterTimezones = await masterDb
        .select()
        .from(master.timezoneMaster);
      const masterCountries = await masterDb
        .select()
        .from(master.countryMaster);
      const masterStates = await masterDb.select().from(master.stateProvince);
      const masterCostingMethods = await masterDb
        .select()
        .from(master.costingMethodConfig);
      ...
      for (const row of masterLangs)
        await tenantDb
          .insert(tenant.languageMaster)
          .values(row)
          .onDuplicateKeyUpdate({ set: { ...row } });
      for (const row of masterCurrs)
        await tenantDb
          .insert(tenant.currencyMaster)
          .values(row)
          .onDuplicateKeyUpdate({ set: { ...row } });
      for (const row of masterTimezones)
        await tenantDb
          .insert(tenant.timezoneMaster)
          .values(row)
          .onDuplicateKeyUpdate({ set: { ...row } });
      for (const row of masterCountries)
        await tenantDb
          .insert(tenant.countryMaster)
          .values(row)
          .onDuplicateKeyUpdate({ set: { ...row } });
      for (const row of masterStates)
        await tenantDb
          .insert(tenant.stateProvince)
          .values(row)
          .onDuplicateKeyUpdate({ set: { ...row } });
      for (const row of masterCostingMethods)
        await tenantDb
          .insert(tenant.costingMethodConfig)
          .values(row)
          .onDuplicateKeyUpdate({ set: { ...row } });
      for (const row of masterSteps)
        await tenantDb
          .insert(tenant.setupStepMaster)
          .values(row)
          .onDuplicateKeyUpdate({ set: { ...row } });
      for (const row of masterNobs)
        await tenantDb
          .insert(tenant.nobMaster)
          .values(row)
          .onDuplicateKeyUpdate({ set: { ...row } });
      for (const row of masterLobs)
        await tenantDb
          .insert(tenant.lobMaster)
          .values(row)
          .onDuplicateKeyUpdate({ set: { ...row } });

      const nobIdByCode = new Map(
        masterNobs.map((n) => [n.nob_code, n.nob_id]),
      );
      const lobIdByCode = new Map(
        masterLobs.map((l) => [l.lob_code, l.lob_id]),
      );
      ...
      const existingSeriesCodes = new Set(
        (
          await tenantDb
            .select({ c: tenant.noSeriesMaster.series_code })
            .from(tenant.noSeriesMaster)
        ).map((r) => r.c),
      );
      for (const series of SYSTEM_NO_SERIES_SEED) {
        if (existingSeriesCodes.has(series.series_code)) continue;
        const seriesNobId = series.nob_code
          ? nobIdByCode.get(series.nob_code)
          : undefined;
        const seriesLobId = series.lob_code
          ? lobIdByCode.get(series.lob_code)
          : undefined;
        if (
          (series.nob_code && !seriesNobId) ||
          (series.lob_code && !seriesLobId)
        )
          continue;
      ...
      if (!existingUom) {
        await tenantDb
          .insert(tenant.uomMaster)
          .values(SYSTEM_UOM_SEED.map((u) => ({ ...u, tenant_id: tenantId })));
      }
      ...
      if (!existingSpecies) {
        await tenantDb
          .insert(tenant.speciesMaster)
          .values(
            SYSTEM_SPECIES_SEED.map((s) => ({ ...s, tenant_id: tenantId })),
          );
      }
      const [existingItemType] = await tenantDb
        .select()
        .from(tenant.itemTypeMaster)
        .limit(1);
      if (!existingItemType) {
        await tenantDb.insert(tenant.itemTypeMaster).values(
          SYSTEM_ITEM_TYPE_SEED.map((t) => ({
            ...t,
            tenant_id: tenantId,
            is_system: true,
          })),
      ...
      const existingLocationTypeCodes = new Set(
        (
          await tenantDb
            .select({ c: tenant.locationTypeMaster.type_code })
            .from(tenant.locationTypeMaster)
        ).map((r) => r.c),
      );
      for (const locationType of SYSTEM_LOCATION_TYPE_SEED) {
        if (existingLocationTypeCodes.has(locationType.type_code)) continue;
        await tenantDb
          .insert(tenant.locationTypeMaster)
          .values({ ...locationType, tenant_id: tenantId, is_system: true });
      }
      ...
      const speciesIdByCode = new Map(
        speciesRows.map((s) => [s.species_code, s.species_id]),
      );

      const existingBreedCodes = new Set(
        (
          await tenantDb
            .select({ c: tenant.breedMaster.breed_code })
            .from(tenant.breedMaster)
        ).map((r) => r.c),
      );
      for (const breed of forSeededLobs(SYSTEM_BREED_SEED)) {
        ...
        await tenantDb.insert(tenant.breedMaster).values({
          ...
          avg_litter_size_weaned:
            breed.avg_litter_size_weaned?.toString() ?? null,
          avg_weaning_weight_kg:
            breed.avg_weaning_weight_kg?.toString() ?? null,
          ...
          boar_productive_life_months:
            breed.boar_productive_life_months ?? null,
        });
      }
      ...
      const seedItemCodeByName = new Map(
        SYSTEM_ITEM_SEED.map((i) => [i.item_name, i.item_code]),
      );
      for (const row of await tenantDb.select().from(tenant.itemMaster)) {
        itemIdByCode.set(
          seedItemCodeByName.get(row.item_name) ?? row.item_code,
          row.item_id,
        );
      }
      for (const item of forSeededLobs(SYSTEM_ITEM_SEED)) {
        ...
        await tenantDb.insert(tenant.itemMaster).values({
          ...
          item_code: await seedCode(
            tenantDb,
            tenantId,
            null,
            'ITEM',
            tenant.itemMaster,
            tenant.itemMaster.item_code,
            item.item_code,
      ...
      const existingParameterCodes = new Set(
        (
          await tenantDb
            .select({ c: tenant.parameterMaster.parameter_code })
            .from(tenant.parameterMaster)
        ).map((r) => r.c),
      );
      for (const param of forSeededLobs(SYSTEM_PARAMETER_SEED)) {
        ...
        if (!nobId || !lobId) continue;
        const itemId = param.item_code
          ? itemIdByCode.get(param.item_code)
          : undefined;
        await tenantDb.insert(tenant.parameterMaster).values({
          ...
          default_qty_per_unit:
            param.default_qty_per_unit != null
              ? param.default_qty_per_unit.toString()
              : null,
          default_qty_per_batch:
            param.default_qty_per_batch != null
              ? param.default_qty_per_batch.toString()
              : null,
      ...
      const existingStageCodes = new Set(
        (
          await tenantDb
            .select({ c: tenant.stageMaster.stage_code })
            .from(tenant.stageMaster)
        ).map((r) => r.c),
      );
      if (pigLobId && pigNobId) {
        ...
        const stageIdByCode = new Map(
          SYSTEM_STAGE_SEED.map((s) => [s.stage_code, randomUUID()]),
        );
        ...
        for (const stage of SYSTEM_STAGE_SEED) {
          ...
          await tenantDb
            .update(tenant.stageMaster)
            .set({
              next_stage_id: stage.next_stage_code
                ? stageIdByCode.get(stage.next_stage_code) || null
                : null,
              alt_next_stage_id: stage.alt_next_stage_code
                ? stageIdByCode.get(stage.alt_next_stage_code) || null
                : null,
            })
            .where(
              eq(
                tenant.stageMaster.stage_id,
                stageIdByCode.get(stage.stage_code)!,
              ),
            );
        }
        ...
        const breedIdByCode = new Map(
          (
            await tenantDb
              .select({
                code: tenant.breedMaster.breed_code,
                id: tenant.breedMaster.breed_id,
              })
              .from(tenant.breedMaster)
          ).map((r) => [r.code, r.id]),
        );
        const liveStageIdByCode = new Map(
          (
            await tenantDb
              .select({
                code: tenant.stageMaster.stage_code,
                id: tenant.stageMaster.stage_id,
              })
              .from(tenant.stageMaster)
          ).map((r) => [r.code, r.id]),
        );
        ...
        const existingLifecycleKeys = new Set(
          (
            await tenantDb
              .select({
                b: tenant.breedLifecycleStages.breed_id,
                s: tenant.breedLifecycleStages.stage_id,
              })
              .from(tenant.breedLifecycleStages)
          ).map((r) => `${r.b}:${r.s}`),
        );
        for (const lc of SYSTEM_BREED_LIFECYCLE_SEED) {
          ...
          const feedItemId = lc.feed_item_code
            ? itemIdByCode.get(lc.feed_item_code)
            : undefined;
          const outputItemId = lc.output_item_code
            ? itemIdByCode.get(lc.output_item_code)
            : undefined;
          await tenantDb.insert(tenant.breedLifecycleStages).values({
            lifecycle_id: randomUUID(),
            tenant_id: tenantId,
            lifecycle_code: await seedCode(
              tenantDb,
              tenantId,
              null,
              'BREED_LIFECYCLE_STAGE',
              tenant.breedLifecycleStages,
              tenant.breedLifecycleStages.lifecycle_code,
              `${lc.breed_code}-${lc.stage_code}`,
              { breed_id: lc.breed_code, stage_id: lc.stage_code },
            ),
            breed_id: breedId,
            stage_id: stageId,
            ...
            feed_qty_per_head_per_day_kg:
              lc.feed_qty_per_head_per_day_kg != null
                ? lc.feed_qty_per_head_per_day_kg.toString()
                : null,
            feed_wastage_pct:
              lc.feed_wastage_pct != null
                ? lc.feed_wastage_pct.toString()
                : null,
            std_body_weight_kg:
              lc.std_body_weight_kg != null
                ? lc.std_body_weight_kg.toString()
                : null,
            std_adg_gpd:
              lc.std_adg_gpd != null ? lc.std_adg_gpd.toString() : null,
            std_fcr: lc.std_fcr != null ? lc.std_fcr.toString() : null,
            std_mortality_rate_pct:
              lc.std_mortality_rate_pct != null
                ? lc.std_mortality_rate_pct.toString()
                : null,
            output_item_id: outputItemId || null,
            output_uom: lc.output_uom || null,
            std_output_qty:
              lc.std_output_qty != null ? lc.std_output_qty.toString() : null,
            kpi_lower_limit:
              lc.kpi_lower_limit != null ? lc.kpi_lower_limit.toString() : null,
            kpi_upper_limit:
              lc.kpi_upper_limit != null ? lc.kpi_upper_limit.toString() : null,
      ...
      const defaultLangId =
        masterLangs.find((l) => l.is_system_default)?.lang_id ||
        masterLangs[0]?.lang_id;
      ...
      const [existingCompany] = await tenantDb
        .select()
        .from(tenant.companyMaster)
        .where(eq(tenant.companyMaster.company_code, 'TRIPLEC'))
        .limit(1);
      ...
      for (const cc of companyConfigs) {
        ...
        const [existingGl] = await tenantDb
          .select()
          .from(tenant.glAccountMaster)
          .where(eq(tenant.glAccountMaster.company_id, cc.id))
          .limit(1);
        if (!existingGl) {
          ...
          for (const mapping of STARTER_GL_MAPPINGS) {
            const debitAccountId = accountIdByCode.get(
              mapping.debit_account_code,
            );
            const creditAccountId = accountIdByCode.get(
            ...
            await tenantDb.insert(tenant.glMappingMaster).values({
              ...
              tenant_id: tenantId,
              company_id: cc.id,
              mapping_code: await seedCode(
                tenantDb,
                tenantId,
                cc.id,
                'GL_MAPPING',
                tenant.glMappingMaster,
                tenant.glMappingMaster.mapping_code,
        ...
        const farmLoc = await seedLocation(tenantDb, locCtx, {
          key: cc.farmCode,
          name: cc.farmName,
          type: 'FARM',
        ...
        const shedLoc = await seedLocation(tenantDb, locCtx, {
          key: cc.shedCode,
          name: cc.shedName,
          type: 'SHED',
          parent: farmLoc,
        ...
        await seedLocation(tenantDb, locCtx, {
          key: `WH-${cc.code}-MAIN`,
          name: `${cc.name} Central Warehouse`,
          type: 'STORE',
          parent: farmLoc,
          storageType: 'STORE',
          subType: STARTER_WAREHOUSE.warehouse_type,
        });
        await seedLocation(tenantDb, locCtx, {
          key: `SILO-${cc.code}-01`,
          name: `${cc.shedName} Feed Silo`,
          type: 'SILO',
          parent: shedLoc,
          storageType: 'SILO',
          siloCapacityKg: 25000,
          siloReorderDays: 7,
        });
        ...
        const [existingArea] = await tenantDb
          .select()
          .from(tenant.operationalAreaMaster)
          .where(eq(tenant.operationalAreaMaster.company_id, cc.id))
          .limit(1);
      ...
      const [existingTenantAdmin] = await tenantDb
        .select()
        .from(tenant.userMaster)
        .where(eq(tenant.userMaster.email, userBy('tenant').email))
        .limit(1);
      ...
      if (!existingTenantAdmin) {
        ...
        await tenantDb
          .update(tenant.userMaster)
          .set({ password_hash: commonPasswordHash, is_active: true })
          .where(eq(tenant.userMaster.user_id, tAdminId));
      }

      // 2. Company Admin — sees one company.
      const [existingComp1Admin] = await tenantDb
        .select()
        .from(tenant.userMaster)
        .where(eq(tenant.userMaster.email, userBy('company').email))
        .limit(1);
      ...
      if (!existingComp1Admin) {
        ...
        await tenantDb
          .update(tenant.userMaster)
          .set({ password_hash: commonPasswordHash, is_active: true })
          .where(eq(tenant.userMaster.user_id, c1AdminId));
      }

      // 3. Operational Admin — sees one operational area, not the whole company.
      const opAdminUserId = randomUUID();
      const [existingOpAdmin] = await tenantDb
        .select()
        .from(tenant.userMaster)
        .where(eq(tenant.userMaster.email, userBy('area').email))
        .limit(1);
      ...
      if (!existingOpAdmin) {
        ...
        await tenantDb
          .update(tenant.userMaster)
          .set({ password_hash: commonPasswordHash, is_active: true })
          .where(eq(tenant.userMaster.user_id, opAdminId));
      }
      ...
      const [existingStandard] = await tenantDb
        .select()
        .from(tenant.userMaster)
        .where(eq(tenant.userMaster.email, userBy('standard').email))
        .limit(1);
      ...
      if (!existingStandard) {
        ...
        await tenantDb
          .update(tenant.userMaster)
          .set({ password_hash: commonPasswordHash, is_active: true })
          .where(eq(tenant.userMaster.user_id, stdId));
      }
      ...
      const [existingRole] = await tenantDb
        .select()
        .from(tenant.roleMaster)
        .where(eq(tenant.roleMaster.company_id, COMPANY_1_ID))
        .limit(1);
        ...
        ? {
            superAdminRoleId: (
              await tenantDb
                ...
                .where(
                  and(
                    eq(tenant.roleMaster.company_id, COMPANY_1_ID),
                    eq(tenant.roleMaster.role_code, 'SUPER_ADMIN'),
                  ),
                )
                .limit(1)
            )[0]?.role_id,
            managerRoleId: (
              await tenantDb
                ...
                .where(
                  and(
                    eq(tenant.roleMaster.company_id, COMPANY_1_ID),
                    eq(tenant.roleMaster.role_code, 'MANAGER'),
                  ),
                )
                .limit(1)
            )[0]?.role_id,
            operatorRoleId: (
              await tenantDb
                ...
                .where(
                  and(
                    eq(tenant.roleMaster.company_id, COMPANY_1_ID),
                    eq(tenant.roleMaster.role_code, 'OPERATOR'),
                  ),
                )
                .limit(1)
            )[0]?.role_id,
      ...
      for (const ur of userRoleMap) {
        ...
        const [existingURA] = await tenantDb
          .select()
          .from(tenant.userRoleAssignment)
          .where(
            and(
              eq(tenant.userRoleAssignment.user_id, ur.userId),
              eq(tenant.userRoleAssignment.role_id, ur.roleId),
            ),
          )
          .limit(1);
        if (!existingURA) {
      ...
      for (const uc of userCompanyMap) {
        const [existingUCA] = await tenantDb
          .select()
          .from(tenant.userCompanyAssignments)
          .where(
            and(
              eq(tenant.userCompanyAssignments.user_id, uc.userId),
              eq(tenant.userCompanyAssignments.company_id, uc.companyId),
            ),
          )
          .limit(1);
        if (!existingUCA) {
    ...
    console.log('==================');
    console.log(
      `Tenant code:      ${tenantCode}  (send as x-tenant-id header, or ?tenant=${tenantCode} on the login page)`,
    );
    console.log(`Company:          ${companyName} (TRIPLEC)`);
    console.log('');
    for (const u of DEV_USERS) {
      console.log(
        `  ${u.email.padEnd(30)} ${DEV_PASSWORD.padEnd(10)} ${u.type.padEnd(19)} ${u.scope}`,
      );
    }
    console.log('');
    console.log(
      `Login:            http://localhost:3001/login?tenant=${tenantCode}`,
    );
    console.log(
      'Starter GL accounts, GL mappings, one warehouse, and one farm/shed are already seeded.',
    );
    console.log('');
  } finally {
