...
import {
  FARM_LOCATION_SEED,
  FarmLocationSeedRow,
} from './lib/farm-location-seed-data';
...
async function run() {
  ...
  try {
    const [[lock]] = await db.query<RowDataPacket[]>(
      "SELECT GET_LOCK('navfarm-farm-locations', 5) acquired",
    );
    if (Number(lock.acquired) !== 1)
      throw new Error('Another farm location seed run is active.');
    ...
    if (!scopeRows.length)
      throw new Error('location_master is empty — no scope to copy.');
    ...
    for (const u of UOM_ADDITIONS) {
      const [existing] = await db.query<RowDataPacket[]>(
        'SELECT uom_id FROM uom_master WHERE tenant_id = ? AND uom_code = ?',
        [scope.tenant_id, u.code],
      );
        ...
        if (write) {
          await db.query(
            ...
            [
              ...
              t.name,
              t.prefix,
              JSON.stringify(t.allowedParents),
            ],
          );
        }
    ...
    plan.locationTypesAdded = typesAdded.length
      ? typesAdded
    ...
    for (const r of sorted) {
      ...
      const parentId =
        r.type === 'FARM'
          ? null
          : parentCode
            ? (idByCode.get(parentCode) ?? null)
            : (farmIdByFarm.get(r.farm) ?? null);
      if (r.type !== 'FARM' && !parentId) {
        throw new Error(
          `Parent for ${r.code} (${r.parent}) was not inserted first — check the sort.`,
        );
      }
      const farmId =
        r.type === 'FARM' ? null : (farmIdByFarm.get(r.farm) ?? null);
      ...
      const values = [
        ...
        r.storageType ?? null,
        r.storageName ?? null,
        num(r.siloCapacityKg),
        num(r.siloReorderDays),
        num(r.downtimeDays),
        r.feedInBags === undefined ? null : r.feedInBags ? 1 : 0,
        farmId,
      ];
    ...
    plan.locations = {
      ...
      byFarm: ['MULTIPLIER', 'PORTA'].map((f) => ({
        farm: f,
        rows: FARM_LOCATION_SEED.filter((r) => r.farm === f).length,
      })),
      blockedAwaitingTripleC:
        'docs/triple-c-location-code-queries.md — 453 of 703 rows',
    };
    ...
    const [areas] = await db.query<RowDataPacket[]>(
      'SELECT area_id, area_code, farm_id FROM operational_area_master WHERE tenant_id = ?',
      [scope.tenant_id],
    );
    ...
    for (const a of areas) {
      ...
      if (write) {
        await db.query(
          'UPDATE operational_area_master SET farm_id = ?, updated_at = NOW() WHERE area_id = ?',
          [multiplierId, a.area_id],
        );
      }
      repointed.push(`${a.area_code} → MULTIPLIER`);
    }
    plan.operationalAreas = repointed.length
      ? repointed
      : 'already pointing at MULTIPLIER';

    const [after] = await db.query<RowDataPacket[]>(
      `SELECT location_type, COUNT(*) n, SUM(is_active = 1) active FROM location_master
        WHERE tenant_id = ? GROUP BY location_type ORDER BY location_type`,
      [scope.tenant_id],
    );
    plan.locationMasterAfter = after.map(
      (r) => `${r.location_type}: ${r.active}/${r.n} active`,
    );
