...
async function run() {
  ...
  const [[tenantRow]] = await server.query<any[]>(
    'SELECT tenant_id, db_name FROM tenant_master WHERE tenant_code = ?',
    [tenantCode],
  );
  await server.end();
  if (!tenantRow)
    throw new Error(`Tenant '${tenantCode}' not found in ${masterDatabase}.`);
  ...
  try {
    const batches = await db
      ...
      .where(eq(schema.batchHeader.status, 'ACTIVE'));
    ...
    for (const batch of batches) {
      ...
      if (existing) {
        console.log(
          `  [${batch.batch_no}] skip — scheduler_header already exists for its current stage.`,
        );
        continue;
      }
      ...
      if (!stage) {
        console.log(
          `  [${batch.batch_no}] skip — stage_id does not resolve to a real Stage Master row.`,
        );
        continue;
      }
      ...
      const animalCount =
        liveCount > 0
          ? String(liveCount)
          : (batch.closing_quantity ?? batch.opening_quantity);
      ...
      if (batch.breed_id) {
        ...
        if (lifecycle) {
          ...
          let seq = 1;
          const itemIds = [
            lifecycle.feed_item_id,
            lifecycle.output_item_id,
          ...
          if (
            lifecycle.feed_item_id &&
            lifecycle.feed_qty_per_head_per_day_kg
          ) {
            lines.push({
              line_id: randomUUID(),
              scheduler_id: schedulerId,
          ...
          if (lifecycle.output_item_id && lifecycle.std_output_qty) {
            lines.push({
              ...
              item_description: itemNames.get(lifecycle.output_item_id) ?? null,
      ...
      console.log(
        `  [${batch.batch_no}] created scheduler_header for stage '${stage.stage_name}' with ${lineCount} line(s).`,
      );
    }
