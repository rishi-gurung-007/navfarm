...
type LineSpec =
  | { line_type: 'CONSUMPTION'; activity_name: string; itemCode: string; standard_qty: string; is_mandatory?: boolean }
  | { line_type: 'DESCRIPTIVE'; activity_name: string; kpi_metric: string; kpi_uom: string; capture_per: string; is_mandatory?: boolean }
...
const LINE_TEMPLATE: LineSpec[] = [
  ...
  { line_type: 'RESOURCE', activity_name: 'Farm Labour Hours', resourceCode: 'RES-APX-LAB01' },
  { line_type: 'CONSUMPTION', activity_name: 'Lactation Diet Top-Up', itemCode: 'FEED-LACT-SOW', standard_qty: '1.5000' },
  { line_type: 'DESCRIPTIVE', activity_name: 'Barn Temperature', kpi_metric: 'TEMPERATURE', kpi_uom: 'CELSIUS', capture_per: 'AVERAGE' },
  { line_type: 'CONSUMPTION', activity_name: 'Maize/Corn Supplement', itemCode: 'RAW-MAIZE-CORN', standard_qty: '0.5000' },
  { line_type: 'CONSUMPTION', activity_name: 'Deworming Dose', itemCode: 'MED-IVERMECTIN', standard_qty: '1.0000' },
  { line_type: 'CONSUMPTION', activity_name: 'Antibiotic Cover', itemCode: 'MED-PENICILLIN', standard_qty: '1.0000' },
];

function lineKey(spec: LineSpec, itemIds: Map<string, string>, resourceIds: Map<string, string>): string {
  if (spec.line_type === 'CONSUMPTION') return `CONSUMPTION:${itemIds.get(spec.itemCode)}`;
  if (spec.line_type === 'DESCRIPTIVE') return `DESCRIPTIVE:${spec.kpi_metric}`;
  if (spec.line_type === 'OVERHEAD') return `OVERHEAD:${spec.overhead_category}`;
...
async function run() {
  ...
  if (flags.some((a) => !['--apply', '--verify'].includes(a)) || (apply && verify)) {
  ...
  try {
    const [[lock]] = await db.query<RowDataPacket[]>("SELECT GET_LOCK('navfarm-seed-batch-schedulers', 5) acquired");
    ...
    if (!stages.length) throw new Error(`No active stage_master rows for this batch's LOB/company.`);
    ...
    const itemCodes = [...new Set(LINE_TEMPLATE.filter((l): l is Extract<LineSpec, { line_type: 'CONSUMPTION' }> => l.line_type === 'CONSUMPTION').map((l) => l.itemCode))];
    const resourceCodes = [...new Set(LINE_TEMPLATE.filter((l): l is Extract<LineSpec, { line_type: 'RESOURCE' }> => l.line_type === 'RESOURCE').map((l) => l.resourceCode))];
    ...
    for (const code of itemCodes) {
      ...
      if (!row) throw new Error(`Template item_code '${code}' not found in item_master for this tenant.`);
    ...
    for (const code of resourceCodes) {
      ...
      if (!row) throw new Error(`Template resource_code '${code}' not found in resource_master for this tenant.`);
      resourceIds.set(code, row.resource_id);
    }

    type PlannedHeader = { stage_id: string; stage_code: string; scheduler_id: string; isNew: boolean; existingLineCount: number; animal_count: number; linesToInsert: Array<LineSpec & { line_seq: number }> };
    ...
    for (const stage of stages) {
      ...
      const existingKeys = new Set(
        existingLines.map((l) => {
          ...
          if (l.line_type === 'DESCRIPTIVE') return `DESCRIPTIVE:${l.kpi_metric}`;
          if (l.line_type === 'OVERHEAD') return `OVERHEAD:${l.overhead_category}`;
          if (l.line_type === 'RESOURCE') return `RESOURCE:${l.resource_id}`;
          return `${l.line_type}:${l.line_seq}`;
        }),
      );
      let nextSeq = existingLines.length ? Math.max(...existingLines.map((l) => l.line_seq)) + 1 : 1;
      ...
      if (linesToInsert.length < need) {
        throw new Error(`Template exhausted for stage ${stage.stage_code} — needed ${need} new lines, only found ${linesToInsert.length} non-duplicate template entries. Widen LINE_TEMPLATE.`);
      }
    ...
    console.log(JSON.stringify({
      ...
      toppedUpHeaders: plan.filter((p) => !p.isNew && p.linesToInsert.length > 0).length,
      alreadySatisfied: plan.filter((p) => p.existingLineCount >= MIN_LINES_PER_SCHEDULER).length,
      plan: plan.map((p) => ({
        stage: p.stage_code,
        scheduler_id: p.scheduler_id,
        newHeader: p.isNew,
        existingLines: p.existingLineCount,
        linesToAdd: p.linesToInsert.map((l) => `${l.line_seq}:${l.line_type}:${l.activity_name}`),
    ...
    if (apply || verify) {
      ...
      for (const p of plan) {
        if (p.isNew) {
          await db.query(
            ...
            [p.scheduler_id, batch.tenant_id, batch.company_id, batch.batch_id, p.stage_id, batch.breed_id, batch.lob_id, batch.nob_id, locationId, batch.start_date, p.animal_count, now, now],
          );
        }
        for (const line of p.linesToInsert) {
          ...
          if (line.line_type === 'CONSUMPTION') {
            await db.query(
              ...
              [base.line_id, base.scheduler_id, base.line_seq, base.line_type, base.activity_name, base.stage_id, base.occurrence, base.start_day, base.is_mandatory, base.source, base.nob_id, base.lob_id, itemIds.get(line.itemCode), line.standard_qty],
            );
            ...
            await db.query(
              `INSERT INTO scheduler_line
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              [base.line_id, base.scheduler_id, base.line_seq, base.line_type, base.activity_name, base.stage_id, base.occurrence, base.start_day, base.is_mandatory, base.source, base.nob_id, base.lob_id, line.kpi_metric, line.kpi_uom, line.capture_per],
            );
            ...
            await db.query(
              `INSERT INTO scheduler_line
                 (line_id, scheduler_id, line_seq, line_type, activity_name, stage_id, occurrence, start_day, is_mandatory, source, nob_id, lob_id,
                  overhead_category)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              [base.line_id, base.scheduler_id, base.line_seq, base.line_type, base.activity_name, base.stage_id, base.occurrence, base.start_day, base.is_mandatory, base.source, base.nob_id, base.lob_id, line.overhead_category],
            );
          } else {
            await db.query(
              `INSERT INTO scheduler_line
                 (line_id, scheduler_id, line_seq, line_type, activity_name, stage_id, occurrence, start_day, is_mandatory, source, nob_id, lob_id,
                  resource_id)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              [base.line_id, base.scheduler_id, base.line_seq, base.line_type, base.activity_name, base.stage_id, base.occurrence, base.start_day, base.is_mandatory, base.source, base.nob_id, base.lob_id, resourceIds.get(line.resourceCode)],
            );
          }
        }
      }
      ...
      console.log('Read-only. No changes attempted. Re-run with --verify, then --apply.');
    }
  } finally {
