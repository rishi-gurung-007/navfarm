...
export const planMaster = mysqlTable('plan_master', {
  ...
  storage_limit_gb: decimal('storage_limit_gb', { precision: 8, scale: 2 })
    .default('5.00')
    .notNull(),
...
export const tenantMaster = mysqlTable('tenant_master', {
  ...
  plan_id: varchar('plan_id', { length: 30 }).references(
    () => planMaster.plan_id,
    { onDelete: 'restrict' },
  ),
...
export const tenantSubscription = mysqlTable('tenant_subscription', {
  sub_id: varchar('sub_id', { length: 36 })
    .primaryKey()
    .$defaultFn(() => randomUUID()),
  tenant_id: varchar('tenant_id', { length: 36 })
    .notNull()
    .unique()
    .references(() => tenantMaster.tenant_id, { onDelete: 'cascade' }),
  plan_code: varchar('plan_code', { length: 30 })
    .notNull()
    .references(() => planMaster.plan_id, { onDelete: 'restrict' }),
  feature_flags: json('feature_flags'),
  storage_limit_gb: decimal('storage_limit_gb', { precision: 8, scale: 2 })
    .default('5.00')
    .notNull(),
  support_tier: varchar('support_tier', { length: 20 })
    .default('STANDARD')
    .notNull(),
  sla_uptime_pct: decimal('sla_uptime_pct', { precision: 5, scale: 2 })
    .default('99.50')
    .notNull(),
...
export const tenantMasterRelations = relations(
  tenantMaster,
  ({ one, many }) => ({
...
export const tenantSubscriptionRelations = relations(
  tenantSubscription,
  ({ one }) => ({
