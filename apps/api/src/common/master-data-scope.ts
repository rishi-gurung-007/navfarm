...
export const MASTER_TABLES: Record<string, AnyMySqlTable> = {
  ...
  farm: schema.locationMaster,
  warehouse: schema.locationMaster,
  shed: schema.locationMaster,
  location: schema.locationMaster,
  'location-type': schema.locationTypeMaster,
  stage: schema.stageMaster,
  'number-series': schema.noSeriesMaster,
  animal: schema.animalRegister,
  item: schema.itemMaster,
  'item-type': schema.itemTypeMaster,
  'item-category': schema.itemCategoryMaster,
  'item-attribute': schema.itemAttributeMaster,
  uom: schema.uomMaster,
  ...
  'uom/conversion': schema.uomConversionMaster,
  'uom-conversion': schema.uomConversionMaster,
  species: schema.speciesMaster,
  breed: schema.breedMaster,
  'breed-lifecycle-stage': schema.breedLifecycleStages,
  reason: schema.reasonMaster,
  disease: schema.diseaseMaster,
  'feed-formula': schema.feedFormulaMaster,
  activity: schema.activityMaster,
  supplier: schema.supplierMaster,
  customer: schema.customerMaster,
  resource: schema.resourceMaster,
  'gl-account': schema.glAccountMaster,
  'gl-mapping': schema.glMappingMaster,
  'cost-center': schema.costCenterMaster,
};
...
export const companyCondition = (
  ...
  companyId?: string | null,
): SQL => (companyId ? eq(column, companyId) : isNull(column));
...
export function masterScopeConditions(
  cls: ClsService,
  table: AnyMySqlTable,
  ...
  if (scope && columns.tenant_id)
    conditions.push(eq(columns.tenant_id, scope.tenantId));
  if (columns.company_id && (scope || requestedCompany)) {
    conditions.push(
      companyId
        ? eq(columns.company_id, companyId)
        : isNull(columns.company_id),
    );
  }
  if (scope?.kind === 'OPERATIONAL') {
    if (columns.nob_id && scope.nobId)
      conditions.push(
        or(eq(columns.nob_id, scope.nobId), isNull(columns.nob_id))!,
      );
    if (columns.lob_id && scope.lobId)
      conditions.push(
        or(eq(columns.lob_id, scope.lobId), isNull(columns.lob_id))!,
      );
  }
  return conditions;
}
...
export async function enforceMasterRequest(
  cls: ClsService,
  request: any,
  ...
  const conversion =
    controllerPath === 'uom' &&
    (request.route?.path || request.path || '').includes('conversion');
  const table = MASTER_TABLES[conversion ? 'uom/conversion' : controllerPath];
  if (!table) return;
  const user = request.user;
  const area = cls.get<{ area_id: string; nob_id: string; lob_id: string }>(
    'activeOperationalArea',
  );
  const activeArea = area?.area_id ? area : undefined;
  const kind =
    request.headers['x-workspace-scope'] ||
    (activeArea
      ? 'OPERATIONAL'
      : request.headers['x-active-company-id']
        ? 'COMPANY'
        : ['TENANT_ADMIN', 'SYSTEM_ADMIN'].includes(user.userType)
          ? 'TENANT'
          : 'COMPANY');
  if (!['TENANT', 'COMPANY', 'OPERATIONAL'].includes(kind))
    throw new BadRequestException('Invalid workspace scope.');
  if (
    kind === 'TENANT' &&
    !['TENANT_ADMIN', 'SYSTEM_ADMIN'].includes(user.userType)
  )
    throw new ForbiddenException(
      'Tenant templates require tenant administration access.',
    );
  const companyId =
    kind === 'TENANT'
      ? null
      : request.headers['x-active-company-id'] || user.companyId;
  if (kind !== 'TENANT' && !companyId)
    throw new BadRequestException('Select a company workspace first.');
  if (kind === 'OPERATIONAL' && !activeArea)
    throw new BadRequestException('Select an operational area first.');
  if (kind !== 'OPERATIONAL' && activeArea)
    throw new BadRequestException(
      'Area context conflicts with workspace scope.',
    );
  const scope: MasterScope = {
    kind,
    companyId,
    tenantId: request.tenantId || user.tenantId,
    nobId: activeArea?.nob_id,
    lobId: activeArea?.lob_id,
  };
  ...
  for (const supplied of [
    ...
    if (supplied && supplied !== companyId)
      throw new ForbiddenException('Master company must match the workspace.');
  }
  const db = cls.get<MySql2Database<typeof schema>>('tenantDb');
  const columns = getTableColumns(table) as Record<string, AnyMySqlColumn>;
  const checkRecord = async (
    target: AnyMySqlTable,
    column: AnyMySqlColumn,
    value: unknown,
    field?: string,
  ) => {
    ...
    if (typeof value !== 'string' || value.trim() === '') {
      throw new BadRequestException(
        `'${field ?? column.name}' must be a single identifier.`,
      );
    }
    const constraints = masterScopeConditions(cls, target);
    const targetColumns = getTableColumns(target) as Record<
      string,
      AnyMySqlColumn
    >;
    if (targetColumns.tenant_id)
      constraints.push(eq(targetColumns.tenant_id, scope.tenantId));
    const [row] = await db
      .select()
      .from(target)
      .where(and(eq(column, value), ...constraints))
      .limit(1);
    if (!row)
      throw new NotFoundException(
        'Master record is not available in this workspace.',
      );
    return row;
  };
  ...
  if (id) {
    ...
    if (primary) {
      ...
      if (
        table === schema.breedLifecycleStages &&
        typeof row.breed_id === 'string'
      )
        await checkRecord(
          schema.breedMaster,
          schema.breedMaster.breed_id,
          row.breed_id,
        );
    }
  }
  ...
  const isCreate =
    ...
    (!request.route?.path ||
      request.route.path
        .replace(/\/$/, '')
        .endsWith(`/${conversion ? 'uom/conversion' : controllerPath}`));
  if (isCreate && columns.company_id) {
    if (!companyId && columns.company_id.notNull)
      throw new BadRequestException(
        'This record requires a company workspace.',
      );
  ...
  if (kind === 'OPERATIONAL') {
    for (const [key, value] of [
      ...
      if (body[key] && body[key] !== value)
        throw new BadRequestException(
          'NOB/LOB must match the operational area.',
        );
  ...
  for (const fk of getTableConfig(table).foreignKeys) {
    ...
    if (!Object.values(MASTER_TABLES).includes(reference.foreignTable))
      continue;
    for (let i = 0; i < reference.columns.length; i++) {
      ...
      if (value)
        await checkRecord(
          ...
          reference.foreignColumns[i],
          value,
          reference.columns[i].name,
        );
    }
  }
